        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (result.status == 'success') {
                    ...
                    showPopupMessage('error', result.message + ' (Error Code: 142)');
                }
            },
            error: function () {
                showPopupMessage('error', 'Previous request not completed. (Error Code: 143)');
            }
        });
    }
});
...
function ItemPageNumberSet(page,pagefolder){  
		var folder_id = pagefolder.split(",");  
		  $.ajax({
			...
			error: function () {
				showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 144)');
			}
		});   
	}
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (result.status == 'success') {
                        ...
                        showPopupMessage('error', result.message + ' (Error Code: 145)');
                    }
                },
                error: function () {
                    showPopupMessage('error', 'Previous request not completed. (Error Code: 146)');
                }
            });
	}
});
...
$(document).on('click', '#share_item_button', function(){
        if($("#share_items_front").valid() &&  $('#field_share_item').val() ==''){
            ...
            var search_text = '<?php  echo base64_encode((isset($_REQUEST['search_text']) && $_REQUEST['search_text'] !='')?$_REQUEST['search_text']:''); ?>';
            var username = [];
...
//            });
            $.ajax({
                ...
                error: function () {
                    showPopupMessage('error','Something went wrong, Please try again. (Error Code: 147)');
                }
	}); 
}
 ...
 function getRecordTreedata(folder_id,itemId,share,flg,previousId,ref,detailPagecontent,pageNumber='',pagefolder='',start='',total_item_page='',action='',imgFlg='',searchString = '',recordIdsArr = '',scrapbook_item = ''){
    if(recordIdsArr !='' && typeof(recordIdsArr) != 'undefined'){
		...
		ItemPageNumberSet(pageNumber,pagefolder);
            ...
            error: function () {
                $('.loading-div').hide();
                showPopupMessage('error','Something went wrong, Please try again. (Error Code: 148)');
            }
        });
}
 ...
 function getRecordIds(id){
     $.ajax({
		...
		/************* Fix end Issue Id 2143  18-jan-2023 ***************/
            ...
            error: function () {
                showPopupMessage('error','Something went wrong, Please try again. (Error Code: 149)');
            }
	}); 
 }  
 function imageLoad(){
...
//    if(pr_file_id !=''){
        ...
        error: function () {
            showPopupMessage('error','Something went wrong, Please try again. (Error Code: 150)');
        }
   });
//    }
