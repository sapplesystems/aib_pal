...
<script>
    ...
    $(document).ready(function () {
        ...
        //Fix start for Issue ID 0002427 on 12-Feb-2025
        var previousValue = $('#previous-item-id').val();
        // Validate before parsing
        if (previousValue && previousValue.trim() !== "" && previousValue.startsWith("[") && previousValue.endsWith("]")) {
            var previous_id = JSON.parse(previousValue);

            // Check if the array is empty
            if (Array.isArray(previous_id) && previous_id.length === 0) {
                $('#go-back-button').hide();
            } else {
                $('#go-back-button').show();
            }
        } else {
            $('#go-back-button').hide(); // Hide if it's invalid JSON or empty
        }
        //Anil code ends here
        ...
        //     $('#go-back-button').show();
        // }
        //Fix end for Issue ID 0002427 on 12-Feb-2025
    ...
    $('#go-back-button').click(function () {
        ...
        if (previousValue && previousValue.trim() !== "" && previousValue.startsWith("[") && previousValue.endsWith("]")) {
            ...
            // Ensure it's an array and has elements before popping
            if (Array.isArray(previous_id) && previous_id.length > 0) {
