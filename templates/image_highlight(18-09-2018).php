 ...
 if($window_height == ''){ 
     ...
     ?>
        ...
        <?php }else{  ?>
            <img class="show_large_image" src="<?php echo THUMB_URL.'?id='.$pr_file_id.'&show_text=yes&search_text='.$searchString; ?>" alt="Slider Image" onload="largeHighLightImage()"/>
        <?php }  ?>
        <?php 
        if(!empty($searchHighLightData['rect'])){
            $imageUrl = THUMB_URL.'?id='.$pr_file_id;
...
$(function() {
    $('#full-image-content').one('load',function() {
        if(url_image == ''){
