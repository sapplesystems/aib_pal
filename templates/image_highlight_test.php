<?php
	...
	if ($window_height == '')
	{
		if (empty($searchHighLightData['rect']) == false)
		{
			print("<div class='imgCenterSlider'>");		// width: 338px;height: 480px; position:absolute; left:50%; top:50%; transform:translateX(-50%) translateY(-50%);
		}
		else
		{
			print("<div class='imgCenterSlider'>");
		}

		if ($item_type == 'url')
		{
			print("<a href=\"$data_url\" target='_blank'><img src=\"".IMAGE_PATH."external_url.jpg\" alt='Slider Image' /></a>");
		}
		else
		{
			print("<img class='show_large_image' src=\"".THUMB_URL."?id=".$pr_file_id."&record_item_id=".$record_item_id."&show_text=yes&search_text=".$searchString." alt='Slider Image' onload=\"largeHighLightImage()\" />");
		}
