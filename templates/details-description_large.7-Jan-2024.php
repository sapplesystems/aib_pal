    ...
    visibleMarkers.forEach(record => {
        const marker = new google.maps.Marker({
            position: new google.maps.LatLng(record.lat, record.lon),
						map: mapLarge,
            icon: record.item_id_c === item_id_c ? "<?php echo HOST_ROOT_IMAGE_PATH; ?>map-icon.png" : null
        });
    ...
    geocoder.geocode({ 'address': address }, function (results, status) {
        if (status === 'OK' && results && results.length > 0) {
            ...
            if (lat === null || lng === null) {
                console.log("No lat lng from metadata - set to geocode");
            ...
            if (!isNaN(lat) && !isNaN(lng)) {
                ...
                // Listen for changes and generate updated link
                panorama.addListener('position_changed', function () {
                    const updatedPosition = panorama.getPosition();
                    const updatedHeading = panorama.getPov().heading;

                    // Generate updated link
                    const googleMapsLink = `https://www.google.com/maps/@${updatedPosition.lat()},${updatedPosition.lng()},0a,${panorama.getZoom()}y,${updatedHeading}h,90t/data=!3m4!1e1`;
                    console.log("Updated Google Maps Link:", googleMapsLink);

                    // Optional: Display or use the link in your UI
                    document.getElementById('dynamicGoogleLink').href = googleMapsLink;
                });
            } else {
            ...
            if (isNaN(heading)) {
                alert("Heading is NaN");
            }
        } else {
            console.error('Failed to get geocode information:', status);
            alert("Failed to get geocode information: " + status);
        }
    });
}

        function computeHeadingAndShowMap_OLD(lat,lng) {
...
//                        };
				...
				} else 
				{
					...
					// Compute the heading from the panorama location to the bounds center
						...
						if (!isNaN(lat1) && !isNaN(lng1)) {
							...
							map.setStreetView(panorama);
							// Add event listener to handle link updates dynamically
    panorama.addListener('pano_changed', function () {
        const panoId = panorama.getPano();
        console.log('Panorama ID changed:', panoId);

        // (Optional) Debug the new panorama link
        const position = panorama.getPosition();
        console.log(
            `Updated position: ${position.lat()}, ${position.lng()}`
        );
    });
    ...
    panorama.addListener('position_changed', function () {
        const position = panorama.getPosition();
        console.log(
            `Position updated to: ${position.lat()}, ${position.lng()}`
        );
    });
...
// fetch the metadata , and if meta data is present , set the lat1 and lat2 to the metadata location lat and lng
        fetch(metadataUrl)
            ...
            .then(data => {
                if (data.status === 'OK') {
					...
					console.log("displayMap function ="+location);
                    if (location) {
        ...
        alert('No address provided.');
		 ...
		 let panorama; 
		  function initPano() { console.log("3115");
