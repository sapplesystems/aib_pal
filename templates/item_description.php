...
<?php if ($themeName == 'custom') { ?>
    <div class="row">
        <div class="col-md-4"><span class="itemHeading"><?php echo ($apiResponse['info']['records'][0]['item_type'] == 'RE') ? 'Record Title :' : 'Item Title :'; ?></span></div>
        <div class="col-md-8">
            <span class="itemDesc">
    ...
    if (count($apiResponse['info']['records'][0]['fields']) > 0 || !empty($apiTagResponse['info']['records'])) {
        foreach ($apiResponse['info']['records'][0]['fields'] as $key => $recordsArray) {
            if ($recordsArray['field_value'] != '' && $recordsArray['field_title'] != 'OCR Text') {
                ?>
                <div class="row">
                    <div class="col-md-4"><span class="itemHeading"><?php echo $recordsArray['field_title']; ?> :</span></div>
                    <div class="col-md-8">
                        <span class="itemDesc">
            ...
            ?> 
            <div class="row">
                <div class="col-md-4"><span class="itemHeading">Tags :</span></div>
                <div class="col-md-8">
                    <span class="itemDesc">
...
<div class="padding15 borderBtmNone maxHeight400">
	...
	<!--div class="seemore_details"><span>See more details</span></div-->
</div>
<?php } else { 
    $description_count++;
    ...
    <span class="tableSection">
		...
		<?php if($apiResponse['info']['records'][0]['item_type']=='IT'){?>
		
		<h4> 
            <span class="desccription_item_details paddLeftNone">
				<strong>Record Title:</strong>
				<label><?php echo $itemParents[count($itemParents)-2]['item_title'];?></label>
			</span>
		</h4>	
		<?php }?>
		...
		<!--// Fix Start SS Bug Id 2318  13-Sep-2023/////////////////-->
		<?php if($apiResponse['info']['records'][0]['item_type']=='RE'){?>
		
		<h4> 
            <span class="desccription_item_details paddLeftNone">
				<strong>Item Title:</strong>
				<label><?php echo $apiResponseItems['info']['records'][0]['item_title'];?></label>
			</span>
		</h4>	
		<?php }?>
		<!--// Fix End SS Bug Id 2318  13-Sep-2023/////////////////-->
...
<script>
    $(document).ready(function () {
        $('.item_title').text('<?php echo addslashes($item_title); ?>');
        //$('#desc_count').html('(<?php echo $description_count; ?>)');
    });
</script>