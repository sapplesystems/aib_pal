...
<div class="footer">
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-lg-8 col-md-12 text-center footerText col-sm-12 col-xs-12 marginTopBottom20">AIB™PAL Copyright &copy; <?php echo date('Y'); ?>. All Rights Reserved. "ArchiveInABox" and box device, and "AIB" are registered trademarks of SmallTownPapers, Inc. <br> <span class="footerBottom-none"> <a href="terms_condition.html"
...
<div class="modal fade" id="request_removal">
...
<div class="modal fade" id="report_content">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
...
<div class="modal fade" id="front_contact_us">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
...
<div class="modal fade" id="start_trouble_ticket_popup">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
...
<link rel="stylesheet" href="<?php echo CSS_PATH . 'jstree-style.min.css'; ?>" />
<!-- jsTree JS -->
...
<div class="dsd" id="define_search_folder_home" style="display: none;">
    <div class="define_search_popup">
        <h4
            style="margin-top:0px;font-size:18px;line-height:30px;background: var(--first-color);padding: 5px 10px;font-weight: bold;">
            Select the folders you want to search<span class="popup_close" aria-hidden="true">&times;</span></h4>
        <div class="row">
            <div class="col-md-6">
                <div id="tree-container"></div>
                <input type="hidden" id="selected-ids" readonly style="width: 100%; padding: 5px;">
                <!-- <div id="custom-archive-tree"></div> -->
                <?php //include_once 'show_archive_tree.php'; ?>
            </div>
            <div class="col-md-6">
                <div class="form-group" style="margin-bottom: 5px;">
                    <input type="text" class="form-control" name="define_search_text" id="define_search_text" value=""
                        placeholder="Enter Search Here">
                </div>
                <span style="color: red;font-size: 12px;">Note: Use quote marks for phrase.</span><br />
                <button class="btn search-button" id="define_search_popup_btn" style="margin-top: 10px;">Search</button>
            </div>
        </div>
    </div>
</div>
...
} elseif (isset($folder_d) && $folder_d != '') {
    $parentNodeId = $folder_d;
}

if (isset($_REQUEST['define_search_ids']) && $_REQUEST['define_search_ids'] != '') {
    $define_search_ids = $_REQUEST['define_search_ids'];
} else {
    $define_search_ids = '';
}
...
<script>
    $(document).ready(function () {
        // Define the node path to be opened and pre-selected nodes.
        const nodePath = [<?php echo implode($item_tree_path, ','); ?>]; // This is the path to open.
        const preselectedNodeIds = [<?php echo $define_search_ids; ?>];//["92515", "92516"];         // Additional nodes to be pre-selected.
        ...
        $('#tree-container').jstree({
            'core': {
                ...
                'data': function (node, callback) {
                    // For the root, send a fixed parent_id (for example, 91006)
                    const defaultParentId = <?= json_encode($parentNodeId ?: 0); ?>;
                    const parentId = (node.id === "#") ? defaultParentId : node.id;
                    $.ajax({
                        url: "services.php",
                        type: "POST",
                        data: { mode: 'getDefineTreeData', parent_id: parentId },
                        dataType: "json",
                        success: function (response) {
                            // Pass data to jsTree and then try to open our defined path.
                            callback(response);
                            // Using a timeout to ensure the AJAX-loaded data is rendered before we open the path.
                            setTimeout(function () {
                                openPathAndSelectNodes();
                            }, 500);
                        }
                    });
                }
            ...
            .on("changed.jstree", function (e, data) {
                updateSelectedIds(data.selected);
            });

        // Recursively open nodes along the given path.
        function openPathAndSelectNodes() {
            let tree = $('#tree-container').jstree(true);
            let index = 0;

            function openNext() {
                if (index < nodePath.length) {
                    let nodeId = nodePath[index];
                    let node = tree.get_node(nodeId);
                    if (node) {
                        tree.open_node(node, function () {
                            index++;
                            // Recursively call for the next node in the path.
                            openNext();
                        });
                    } else {
                    ...
                    // Once the path is fully opened, select the final node from the path.
                    let finalNode = nodePath[nodePath.length - 1];
                    //   tree.select_node(finalNode);
                    // Also select (and open) additional pre-selected nodes.
                    preselectedNodeIds.forEach(function (id) {
                        if (tree.get_node(id)) {
                            tree.select_node(id);
                            tree.open_node(id);
                        }
                    });
                    updateSelectedIds(tree.get_selected());
                }
            }
            openNext();
        }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (result.status == 'success') {
                        ...
                        showPopupMessage('error', result.message + ' (Error Code: 1103)');
                    }
                },
                error: function () {
                    showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 1104)');
                }
            });
            <?php
        } else {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (result.status == 'success') {
                            ...
                            showPopupMessage('error', result.message + ' (Error Code: 1105)');
                        }
                    },
                    error: function () {
                        showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 1106)');
                    }
                });
            }
            <?php
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (result.status == 'success') {
                    ...
                    showPopupMessage('error', 'error', 'Something went wrong, Please try again. (Error Code: 1107)');
                }
                $('.loading-div').hide();
            },
            error: function () {
                showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 1108)');
            }
        });
    }
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (result.status == 'success') {
                            ...
                            showPopupMessage('error', result.message + ' (Error Code: 1109)');
                        }
                    },
                    error: function () {
                        showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 1110)');
                    }
                });
            }
            <?php
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (result.status == 'success') {
                            ...
                            showPopupMessage('error', result.message + ' (Error Code: 1111)');
                        }
                    },
                    error: function () {
                        showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 1112)');
                    }
                });
            }
            <?php
...
<script type="text/javascript">
    function getAllHelpMessage(location) {
        ...
        $.ajax({
            ...
            error: function () {
                showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 1113)');
                $('.admin-loading-image').hide();
            }
        });
    }
