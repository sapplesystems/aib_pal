...
<div class="content-wrapper">
    <section class="content-header">
        <h1>Manage Reprint Request</h1>
        <ol class="breadcrumb">
            <li><a href="index"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">Manage Reprint Request</li>
        </ol>
        <h4 class="list_title text-center"><span class="pull-left">Manage Reprint Request</span> <span class="headingNameDesign"><?php echo $_SESSION['aib']['user_data']['item_title'];?></span></h4>
    </section>
    <section class="content bgTexture">
        ...
        <div class="row" id="dataTableDiv">
            <div class="col-md-12 tableStyle">
                 <?php if($loginUserType == 'R'){ ?><div class="archive-list custom-dropdown left35">
                ...
                <table id="reprint_request_table" class="display table" width="100%" cellpadding="0" cellspacing="0">  
                    <thead>  
                        <tr>  
                            <th width="10%" class="text-center">Status</th>
                            <th width="15%" class="text-center">Name</th> 
                            <th width="15%" class="text-center">Email</th> 
                            <th width="15%" class="text-center">Item Link</th>
    ...
    <div class="download-image" style="display:none;"></div>
</div>
...
<script type="text/javascript">
    ...
    $(document).on('click','.view_request_data', function(){
     ...
     var table = $('#reprint_request_table').DataTable({"pageLength": 100,"sDom":'<"H"lfrp>t<"F"ip>'});
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                for (i = 0; i < record.length; i++) {
                    if(record[i].image_link == 'not found'){
                        image = '';
                    }else{
                        image = '<a class="download_request_data" href='+record[i].image_link +' id=download_'+record[i].req_item +' download><img src="'+image_path+'download.png" alt="" /></a>';
                    }
                    ...
                    table.row.add([
                        ...
                        record[i].req_name,
                        record[i].req_email,
                        "<a target='_blank' style='color:#3c8dbc;' href="+record[i].item_link+">"+record[i].item_link+"</a>",
                        record[i].created,
                        record[i].comment, 
						'<span class="view_request_data" data-title="View" data-form-request-id='+ record[i].req_id +'><img src="'+image_path+'view.png" alt="" /></span>'+image+'<span data-field-item-id="'+record[i].req_item +'"><a href="download-pdf.php?item_id='+record[i].req_item +'" title="Download PDF" target="_blank"><img title="pdf" src="'+image_path+'pdf_icon.gif" alt="" /></a></span>'
                    ]).draw(false);
                    select: true;
                }
                $("#dataTableDiv").show();
    ...
    $(document).on('click','.delete_request_data', function(){
        if(confirm('Are you sure you want to delete this? This cannot be undone')){
            ...
            var delete_link = $(this).attr('data-form-request-id'); 
			delete_link = delete_link.split('folder_id=');
			
			var item_id = delete_link[1].split('&');
            $('.admin-loading-image').show();
			
			$.ajax({
				type: 'POST',
				url: 'services_admin_api.php',
					data: {mode:'change_request_status',request_id: request_id, status: 'COMPLETED',folder_id : item_id[0]},
					success: function (response) {
						var record = JSON.parse(response);
						if(record.status == 'success'){
							deleteFolderItem(item_id[0]);
						}
					}
			});
	...
	function deleteFolderItem(item_id){  
		 $.ajax({
                type: 'POST',
                url: 'services_admin_api.php',
