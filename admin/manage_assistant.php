...
<script type="text/javascript">
    ...
    function get_assistant_list(parent_id = ''){
        ...
        $.ajax({
            ...
            error: function () {
                showPopupMessage('error','Something went wrong, Please try again. (Error Code: 490)');
            }
        });
    }
    ...
    $(document).on('click', '#update_assistant_btn', function (){
        if ($("#assistantForm").valid()) {
            ...
            $.ajax({
                ...
                error: function () {
                    showPopupMessage('error','Something went wrong, Please try again. (Error Code: 491)');
                }
            });
        }
    });
    ...
    $(document).on('click','.delete_assistant',function(){
        if(confirm("Are you sure to delete the assistant? This cannot be undone")){
            ...
            $.ajax({
                ...
                error: function () {
                    showPopupMessage('error','Something went wrong, Please try again. (Error Code: 492)');
                    $('.admin-loading-image').hide();
                }
            });
        }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(record.status == 'success'){
						...
						showPopupMessage('error', record.message + ' (Error Code: 493)');
                    }
                }
            });
        }else{
            showPopupMessage('error','Please select a sub-group');
            return false;
        }
    });
