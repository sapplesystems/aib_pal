...
if (isset($_REQUEST['folder_id']) && $_REQUEST['folder_id'] != '') {
    ...
    $flag = 0;
    if (isset($_REQUEST['flg']) && $_REQUEST['flg'] == 'people') {
        $flag = 1;
    }
    if (isset($_REQUEST['flg']) && $_REQUEST['flg'] == 'scrapbook') {
        $flag = 2;
    }
...
include_once COMMON_TEMPLATE_PATH . 'details-custom-header.php';
...
<?php
include_once COMMON_TEMPLATE_PATH . 'details-custom-footer.php';
$backURLSearch = '';
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (result.status == 'success') {
                        ...
                        showPopupMessage('error', result.message + ' (Error Code: 383)');
                    }
                },
                error: function () {
                    showPopupMessage('error', 'Previous request not completed. (Error Code: 384)');
                }
            });
        }
    });
...
//$('#details-back-button').click(function(){
    ...
    function ItemPageNumberSet(page, pagefolder) {
        ...
        $.ajax({
            ...
            error: function () {
                showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 385)');
            }
        });
    }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (result.status == 'success') {
                        ...
                        showPopupMessage('error', result.message + ' (Error Code: 386)');
                    }
                },
                error: function () {
                    showPopupMessage('error', 'Previous request not completed. (Error Code: 387)');
                }
            });
        }
    });
...
//            });
            $.ajax({
                ...
                error: function () {
                    showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 388)');
                }
            });
        }
...
//    var total_thumb_page = $('.sapple-slider-thumb').length;
        ...
        if (current_value == total_item_page && $("#thumb-list-" + row_val).length == 0) {
            ...
            var detailPagecontent = 'Y';
            var action = 'Next';
            if (total_items > total_item_page * 3) {
                getRecordTreedata(folder_id, itemId, share, flg, previousId, ref, detailPagecontent, pageNumber, pagefolder, start, total_item_page, action, '', '', '', scrapbook_item);
            }
        } else {
            current_value = parseInt(parseInt(current_value) + 1);
...
//        var thumb_url = '<?php echo THUMB_URL; ?>';
    ...
    function getRecordTreedata(folder_id, itemId, share, flg, previousId, ref, detailPagecontent, pageNumber = '', pagefolder = '', start = '', total_item_page = '', action = '', imgFlg = '', searchString = '', recordIdsArr = '', scrapbook_item = '') {
        ...
        $.ajax({
            ...
            error: function () {
                $('.loading-div').hide();
                showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 389)');
            }
        });
    }
    ...
    function getRecordIds(id) {
        $.ajax({
            ...
            error: function () {
                showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 390)');
            }
        });
    }
    function imageLoad() {
    ...
    function fullImageWithHighLight(pr_file_id, item_id, height, searchString) {
        $.ajax({
            ...
            error: function () {
                showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 391)');
            }
        });
    }
</script>
