			...
			if ($ResultList != false)
			{
				...
				if ($OptionSortOrder == "STPA")
				{
					...
					foreach($ResultList as $TempResultRecord)
					{
						...
						if ($STPSort == false)
						{
							$STPSort = browse_urldecode($TempResultRecord["item_title"]).",".$TempResultRecord["item_id"];
						}
						else
						{
				...
				foreach($ResultList as $TempResultRecord)
				{
					...
					if ($OptionHide == true)
					{
						$EntryTypeProperty = ftree_get_property($GLOBALS["aib_db"],$ResultRecord["item_id"],AIB_FOLDER_PROPERTY_VISIBLE);
					...
					if ($OptionDerefLinks == false)
					{
						...
						if ($TempResultRecord["item_type"] == FTREE_OBJECT_TYPE_LINK)
						{
							...
							$LinkData["link_title"] = browse_urldecode($TempResultRecord["item_title"]);
						}
					}
					else
					{
						...
						if ($TempResultRecord["item_type"] == FTREE_OBJECT_TYPE_LINK)
						{
							...
							if ($DerefRecord != false)
							{
								...
								$LinkData["final_deref_item_title"] = browse_urldecode($DerefRecord["item_title"]);
							...
							if ($TempRootInfo["archive_group"] !== false)
							{
								...
								if ($RootPropertyList != false)
								{
									foreach($RootPropertyList as $RootPropertyName => $RootPropertyValue)
									{
										...
										$RootInfo["archive_group_properties"][$RootLocalName] = $RootLocalValue;
									}
								}
							}
							...
							if ($TempRootInfo["archive"] !== false)
							{
								...
								if ($RootPropertyList != false)
								{
									foreach($RootPropertyList as $RootPropertyName => $RootPropertyValue)
									{
										$RootLocalName = urldecode($RootPropertyName);
										$RootLocalValue = urldecode($RootPropertyValue);
										$RootInfo["archive_properties"][$RootLocalName] = $RootLocalValue;
									}
								}
							}
						}
...
//						$FieldList = ftree_field_get_item_fields_ext($GLOBALS["aib_db"],$ResultRecord["item_id"]);
					...
					$OutRecord = array(
						...
						"item_title" => browse_urldecode($ResultRecord["item_title"]),
					...
					if ($IsLink == true)
					{
						...
						if ($OptionLinkSourceProperties == true)
						{
							...
							if ($LinkProperties != false)
							{
								...
								{
									$TempName = urldecode($LocalName);
									$OutRecord["link_properties"][$TempName] = urldecode($LocalValue);
								}
...
*/
		...
		case "get_archive_and_group":
			...
			{
				$ArchiveInfo["archive"]["item_title"] = browse_urldecode($ArchiveInfo["archive"]["item_title"]);
			}

			if (isset($ArchiveInfo["archive_group"]["item_title"]) == true)
			{
				$ArchiveInfo["archive_group"]["item_title"] = browse_urldecode($ArchiveInfo["archive_group"]["item_title"]);
			}
...
//								$FieldList = ftree_field_get_item_fields_ext($GLOBALS["aib_db"],$TempChildRecord["item_id"]);
							...
							$TempChildRecord["item_title"] = browse_urldecode($TempChildRecord["item_title"]);
...
//					$FieldList = ftree_field_get_item_fields_ext($GLOBALS["aib_db"],$ItemInfo["item_id"]);
				...
				$OutData["info"]["records"][] = array(
					...
					"item_type" => $LocalEntryType,
					"item_title" => browse_urldecode($ItemInfo["item_title"]),
		...
		case "list_item_ref":
			...
			foreach($Result as $Record)
			{
				$OutData["records"][] = array(
					...
					"item_title" => browse_urldecode($Record["item_title"]),
