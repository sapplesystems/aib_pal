...
// #########
	...
	switch($OpCode)
	{
		case "addrecord":
			...
			$RecordFileMode = aib_get_with_default($FormData,"record_mode","");
			...
			if ($AttachFlagValue == "all" || count($BatchList) < 1)
			{
				...
				foreach($ParentFolderList as $SelectedFolderID)
				{
					...
					// Set link class property, if needed

					if ($LinkClass != false)
					{
						ftree_set_property($GLOBALS["aib_db"],$NewRecordID,"link_class",$LinkClass,true);
					}
					...
					// Store property for record mode

					ftree_set_property($GLOBALS["aib_db"],$NewRecordID,AIB_FOLDER_PROPERTY_RECORDFILEMODE,$RecordFileMode,true);
					...
					if ($SelectedFolderID == $ParentFolderID)
					{
						foreach($BatchList as $BatchRecord)
						{
							...
							ftree_set_property($GLOBALS["aib_db"],$ItemResult[1],AIB_FOLDER_PROPERTY_VISIBLE,$VisibleSetting,true);
							ftree_set_property($GLOBALS["aib_db"],$ItemResult[1],AIB_FOLDER_PROPERTY_PRIVATE,$PrivacySetting,true);
		...
		case "updaterecord":
			...
			// Get OCR languages, if any.  Default is English-only.

			$Languages = aib_get_with_default($FormData,"languages","eng");
			...
			if ($ObjectClass == AIB_ITEM_TYPE_RECORD)
			{
				...
				modify_links($TempFormData,$ObjectID);
				...
				// Get child items and update private/public settings

				$LocalChildItemList = ftree_list_child_objects($GLOBALS["aib_db"],$ObjectID,false,false,false,false,false,true);
				foreach($LocalChildItemList as $LocalChildRecord)
				{
					ftree_set_property($GLOBALS["aib_db"],$LocalChildRecord["item_id"],AIB_FOLDER_PROPERTY_VISIBLE,$VisibleSetting,true);
					ftree_set_property($GLOBALS["aib_db"],$LocalChildRecord["item_id"],AIB_FOLDER_PROPERTY_PRIVATE,$PrivacySetting,true);
				}
			...
			else
			{
				...
				$OCRText = aib_get_with_default($FormData,"itemrecord_default_ocr",false);
				if ($OCRText != false && $OCRFieldDef != false)
				{
					$OCRValues = array($OCRFieldDef["field_id"] => $OCRText);
					ftree_field_store_item_fields($GLOBALS["aib_db"],$ObjectID,$OCRValues);
				}

				// Submit OCR for item if requested

				if ($OCRFlag == "Y")
				{
					$BaseName = fetch_stored_image_for_ocr($ObjectID);
					if ($BaseName != false)
					{
						$InfoArray = array("tree_item=$ObjectID","profile=$OCRProfile","languages=$Languages","source=$BaseName");
						$InfoString = join("\t",$InfoArray);
						aib_store_file_batch_entry($GLOBALS["aib_db"],AIB_BATCH_RECORD_TYPE_OCR_REQUEST,$InfoString,$ObjectID);
					}
				}
		...
		// Clear OCR flags for one or more items
		case "clearocr":
			$ItemListString = aib_get_with_default($FormData,"item_id_list","");
			$OptRecursive = aib_get_with_default($FormData,"opt_recursive",false);
			$LanguagesList = aib_get_with_default($FormData,"languages","eng");
			$OCRProfile = aib_get_with_default($FormData,"ocr_profile","NULL");
			$ChildItemsFlag = aib_get_with_default($FormData,"child_items","N");

			// Set flags

			if (preg_match("/[Yy]/",$ChildItemsFlag) != false)
			{
				$ChildItemsFlag = true;
			}
			else
			{
				$ChildItemsFlag = false;
			}
			...
			// If recursive is set, get the list of items in all subgroups/records/etc.
			...
			{
				// If not recursive, the initial list is the supplied item ID values

				$ItemList = explode(",",$ItemListString);

				// If the child items are requested, get the immediate child items of the supplied item ID values

				if ($ChildItemsFlag == true)
				{
					$ItemMap = array();
					...
					foreach($ItemList as $ItemID)
					{
						...
						$LocalChildItemList = ftree_list_child_objects($GLOBALS["aib_db"],$ItemID,false,false,false,false,false,true);
						foreach($LocalChildItemList as $ChildItemRecord)
						{
							$LocalID = $ChildItemRecord["item_id"];

							// Get AIB folder type property for the retrieved record

							$ChildItemType = ftree_get_property($GLOBALS["aib_db"],$LocalID,AIB_FOLDER_PROPERTY_FOLDER_TYPE);

							// Process based on type.  If we need to, grab child items

							switch($ChildItemType)
							{
								// Only get child items for records

								case AIB_ITEM_TYPE_RECORD:
									$RecordChildList = ftree_list_child_objects($GLOBALS["aib_db"],$LocalID,false,false,false,false,false,true);
									foreach($RecordChildList as $RecordChildRecord)
									{
										$RecordChildLocalID = $RecordChildRecord["item_id"];
										$RecordChildItemType = ftree_get_property($GLOBALS["aib_db"],$RecordChildLocalID,AIB_FOLDER_PROPERTY_FOLDER_TYPE);

										// Only process things that will have attached images.  Ignore archives, collections, archive groups,
										// comments and scrapbooks

										switch($RecordChildItemType)
										{
											case AIB_ITEM_TYPE_ARCHIVE:
											case AIB_ITEM_TYPE_ARCHIVE_GROUP:
											case AIB_ITEM_TYPE_COLLECTION:
											case AIB_ITEM_TYPE_SUBGROUP:
											case AIB_ITEM_TYPE_SCRAPBOOK_ENTRY:
											case AIB_ITEM_TYPE_SCRAPBOOK:
											case AIB_ITEM_TYPE_SCRAPBOOK_SET:
											case AIB_ITEM_TYPE_COMMENT:
											case AIB_ITEM_TYPE_COMMENT_THREAD:
											case AIB_ITEM_TYPE_COMMENT_SET:
												break;

											default:
												if (isset($ItemMap["$RecordChildLocalID"]) == false)
												{
													$ItemMap["$RecordChildLocalID"] = true;
												}
							...
							if (isset($ItemMap["$LocalID"]) == false)
							{
								$ItemMap["$LocalID"] = true;
							}
						}
					}

					$ItemList = array_keys($ItemMap);
				}
			}

			// Process each item in the item list constructed from child lists, recursive lists, etc.

			foreach($ItemList as $ItemID)
			{
				ftree_delete_property($GLOBALS["aib_db"],$ItemID,"ocr_update");
			}
		...
		case "markocr":
			...
			$OCRProfile = aib_get_with_default($FormData,"ocr_profile","NULL");
			$ChildItemsFlag = aib_get_with_default($FormData,"child_items","N");
			$SkipProcessed = aib_get_with_default($FormData,"skip_processed","Y");
			...
			if (preg_match("/[Yy]/",$SkipProcessed) != false)
			{
				$SkipProcessed = true;
			}
			else
				...
				if ($ChildItemsFlag == true)
				{
					...
					foreach($ItemList as $ItemID)
					{
						...
						foreach($LocalChildItemList as $ChildItemRecord)
						{
								...
								case AIB_ITEM_TYPE_RECORD:
									...
									foreach($RecordChildList as $RecordChildRecord)
									{
										...
										switch($RecordChildItemType)
										{
											...
											default:
												if ($SkipProcessed == true)
												{
													$RecordProcessedFlag = ftree_get_property($GLOBALS["aib_db"],$RecordChildLocalID,"ocr_update");
													if ($RecordProcessedFlag == true)
													{
														break;
													}
												}
							...
							if ($SkipProcessed == true)
							{
								$RecordProcessedFlag = ftree_get_property($GLOBALS["aib_db"],$LocalID,"ocr_update");
								if ($RecordProcessedFlag == true)
								{
									continue;
								}
			...
			foreach($ItemList as $ItemID)
			{
				...
				switch($ItemType)
				{
					...
					default:
						...
						{
							// Do not process folders or links
							...
							// Process if real record or item

							default:
								// Fetch stored image and place in path for OCR server

								$BaseName = fetch_stored_image_for_ocr($ItemID);
								if ($BaseName == false)
								{
									break;
								}
								...
								$InfoArray = array("tree_item=$ItemID","profile=$OCRProfile","languages=$Languages","source=$BaseName");
								$InfoString = join("\t",$InfoArray);
								aib_store_file_batch_entry($GLOBALS["aib_db"],AIB_BATCH_RECORD_TYPE_OCR_REQUEST,$InfoString,$ItemID);
								break;
						}
