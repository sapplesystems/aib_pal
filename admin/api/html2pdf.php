...
function html2pdf_debug($Msg)
{
	$Handle = fopen("/tmp/html2pdf.txt","a+");
	if ($Handle != false)
	{
		fputs($Handle,$Msg."\n");
		fclose($Handle);
...
class PDF extends FPDF
{
	...
	function __construct($orientation='P',$unit='mm',$format='A4',$_title,$_url,$_debug=false)
	{
		...
		$this->debug=$_debug;
		$this->AliasNbPages();
		$this->LocalUnits = $unit;
	}

	function convert_pixel_units($In)
	{
		switch($this->LocalUnits)
		{
			case 'pt':
				return($In);
	...
	function convert_percentage($In,$Orientation)
	{
		if (preg_match("/[\%]$/",$In) != false)
		{
			$Temp = preg_replace("/[\%]/","",$In);
			$Temp = $Temp / 100.0;
			$XSize = $this->CurPageSize[0];
			$YSize = $this->CurPageSize[1];
			if ($Orientation == "x")
			{
				return($Temp * $XSize);
			}
	...
	function WriteHTML($html,$bi)
	{
		...
		$this->bi=$bi;
		if ($bi)
			$html=strip_tags($html,"<a><img><p><br><font><tr><blockquote><h1><h2><h3><h4><pre><red><blue><ul><li><hr><b><i><u><strong><em><gotoxy>"); 
		else
			$html=strip_tags($html,"<a><img><p><br><font><tr><blockquote><h1><h2><h3><h4><pre><red><blue><ul><li><hr><gotoxy>"); 
		...
		switch($tag){
			case 'GOTOXY':
				if(isset($attr['X']) && isset($attr['Y']))
				{
					$LocalWidth = $attr['X'];
					$LocalHeight = $attr['Y'];
					$LocalWidth = $this->convert_percentage($LocalWidth,'x');
					$LocalHeight = $this->convert_percentage($LocalHeight,'y');
					$this->SetXY($LocalWidth,$LocalHeight);
				}
...
//				$this->Ln(3);
			...
			case 'A':
				...
				break;
			case 'IMG':
				if (isset($attr['SRC']) == true)
				{
					if(!isset($attr['WIDTH']))
						$attr['WIDTH'] = 0;
					if(!isset($attr['HEIGHT']))
						$attr['HEIGHT'] = 0;
					$LocalWidth = $attr['WIDTH'];
					$LocalHeight = $attr['HEIGHT'];
					$LocalWidth = $this->convert_percentage($LocalWidth,'x');
					$LocalHeight = $this->convert_percentage($LocalHeight,'y');
					$this->Image($attr['SRC'], $this->GetX(), $this->GetY(), $LocalWidth,$LocalHeight);
//					$this->Ln(2);
				}
