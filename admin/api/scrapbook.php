...
// Log debug
// ---------
function aib_scrapbook_log_debug($Msg)
{
	$Handle = fopen("/tmp/aib_scrapbook_log_debug.txt","a+");
	if ($Handle != false)
	{
		fputs($Handle,$Msg."\n");
		fclose($Handle);
...
// #########
	...
	switch($OpCode)
	{
		...
		case "scrpbk_del":
			...
			// Get the entry requested for delete and confirm that it is the correct type of entry

			$ItemRecord = ftree_get($GLOBALS["aib_db"],$ItemID);
			if ($ItemRecord == false)
			{
				$OutData["status"] = "ERROR";
				$OutData["info"] = "NO SUCH RECORD";
				break;
			}

			$ItemProp = ftree_get_property($GLOBALS["aib_db"],$ItemID,AIB_FOLDER_PROPERTY_FOLDER_TYPE);
			if ($ItemProp == false)
			{
				$OutData["status"] = "ERROR";
				$OutData["info"] = "NOT A SCRAPBOOK";
				break;
			}

			if ($ItemProp != AIB_ITEM_TYPE_SCRAPBOOK)
			{
				$OutData["status"] = "ERROR";
				$OutData["info"] = "NOT A SCRAPBOOK";
				break;
...
//						$FieldList = ftree_field_get_item_fields_ext($GLOBALS["aib_db"],$ResultRecord["item_id"]);
		...
		case "scrpbk_delent":
			...
			{
				$OutData["status"] = "ERROR";
				$OutData["info"] = "NOT A SCRAPBOOK ENTRY";
				break;
			}

			if ($ItemProp != AIB_ITEM_TYPE_SCRAPBOOK_ENTRY)
			{
				$OutData["status"] = "ERROR";
				$OutData["info"] = "NOT A SCRAPBOOK ENTRY";
				break;
