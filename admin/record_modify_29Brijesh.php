...
*/
				...
				default:
					...
					if ($FieldsSaved != false)
					{
						...
						{
							$LocalFieldDef = ftree_field_get_field($GLOBALS["aib_db"],$FieldID);
							if ($LocalFieldDef != false)
							{
								$FieldsUsed[$FieldID] = array("def" => $LocalFieldDef, "value" => $FieldValue);
							}
						}
					}
					...
					$OutBuffer[] = "</td><td class='parent-folder-info-link-cell' align='right'><a href=\"/admin/manage_my_archive.php?folder_id=".$ParentFolderID."\">Return To List</a></td></tr>";
					$OutBuffer[] = "</table>";
...
//						"existing_fields" => $FieldsUsed,
					...
					$OutBuffer[] = aib_gen_form_header("pageform","/admin/record_modify.php",false,"validate_form");
...
*/
			...
			switch($OpCode)
			{
				case "save_edit":
					...
					$OutBuffer[] = '<script>window.location.href="/admin/manage_my_archive.php?folder_id='.$ParentFolderID.'";</script>';
				...
				case false:
					...
					switch($ObjectClass)
					{
						case AIB_ITEM_TYPE_RECORD:
							...
							$FieldDef["itemrecord_title"]["value"] = urldecode($ItemDef["item_title"]);
							break;
					...
					if ($OCRFieldDef != false)
					{
						...
						if ($OCRValue != false)
						{
							$FieldDef["itemrecord_default_ocr"]["value"] = $OCRValue;
						}
					}
...
//							$OutBuffer[] = aib_draw_radio_field($FieldDef["itemrecord_attachall"]);
					...
					if ($ObjectRecord["item_type"] == FTREE_OBJECT_TYPE_LINK && $ObjectRecord["item_source_type"] == FTREE_SOURCE_TYPE_URL)
//					if (ObjectClass == AIB_ITEM_TYPE_ITEM && $ObjectRecord["item_)
					{
...
//include('template/common_footer.php');
	...
	{
		print("
				var CheckedTreeItems = {".$TreeNavInfo["init_item"]."};
		");
//		print("
