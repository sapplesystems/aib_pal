...
include("template/top_menu_admin_data.php");
	...
	switch($OpCode)
	{
		...
		default:
			...
			$TargetFields = join("&",array("src=records","parent=$ParentFolderID","srckey=$LocalSourceKey","searchval=$LocalSearchValue","srcmode=$LocalSourceMode","srcpn=$LocalSourcePageNumber"));
			switch($FolderType)
			{
				// Collection
				case AIB_ITEM_TYPE_COLLECTION:
					...
					$MenuCell .= " &nbsp; ";
					$MenuCell .= "<a href='#' class='aib-folder-menu-link' onclick=\"assign_data_entry('$ParentFolderID','$LocalSourceKey','$LocalSearchValue','$LocalSourceMode','$LocalSourcePageNumber');\">Assignments</a>";
					break;
			...
			// If this is the admin, add a scrolling list for the assistants with checkboxes and a button if we're in
			// an archive.
/*
			if ($UserType == FTREE_USER_TYPE_ADMIN && $FolderType != AIB_ITEM_TYPE_ARCHIVE_GROUP)
			{
				// Get the current archive and archive group, and then get the list of assistants
				...
				if ($ArchiveInfo["archive"] != false)
				{
					...
					$AssistantList = ftree_list_users_for_parent($GLOBALS["aib_db"],$ArchiveID,FTREE_USER_TYPE_SUBADMIN);
					if ($AssistantList == false)
					{
						$AssistantList = array();
					}
				}
				...
				if (count($AssistantList) > 0)
				{
					$DivSpec = array(
						"check_field" => "user_id",
						"check_title" => "",
						"check_prefix" => "user_checkbox",
						"field_list" => array("user_title"),
						"title_list" => array("Assistant Name"),
						"div_style" => "border:5px solid grey; width:100%; display:block;",
						"button_title" => "Assign Records To Selected Assistant(s)",
						"button_action" => "",
						);
					$ListParam["extra_title_rows"][] = "<tr><td colspan='99'>".aib_generate_scrolling_record_div($GLOBALS["aib_db"],$DivSpec,$AssistantList)."</td></tr>";
				}
			}
*/
			// Generate list frame
	...
	function assign_data_entry(ParentFolderID,SourceKey,SearchValue,SourceMode,PageNumber)
	{
		...
		URL = '/assign_data_entry_form.php?opcode=assign&parent=' + ParentFolderID +
			...
			'&srcpn=' + PageNumber + '&idlist=' + ListOfIDString;
