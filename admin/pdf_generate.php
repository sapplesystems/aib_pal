...
//	echo '<pre>';
//	print_r($resultPdfData);die;
    if($resultPdfData['status'] == 'OK'){
//        file_put_contents('tmp/'.$item_id.'.pdf', $resultPdfData['info']);

	// Create a temporary base file name using the item ID and the current POSIX time stamp; forces
	// the browser to load the image instead of attempting to retrieve from cache.

	$TempID = $item_id.sprintf("%d",time());

	// Header settings:  Public content, file transfer, PDF file (attachment) with MIME and encoding.

        header("Cache-Control: public");
        header("Content-Description: File Transfer");
        header("Content-Disposition: attachment; filename=".$TempID.".pdf");
        header("Content-Type: application/pdf");
        header("Content-Transfer-Encoding: binary");

	// Send the PDF data to the browser, then exit

	print($resultPdfData["info"]);
