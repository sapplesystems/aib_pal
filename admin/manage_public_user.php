...
<div class="content-wrapper">
    <section class="content-header">
       ...
       <!-- <h4 class="list_title"><span class="pull-left top-padding">Manage Public User</span> <button class="btnDownload" onClick="get_administrator_data_export()"><i class="fa fa-download"></i>&nbsp; Download Report</button>
...
<script type="text/javascript">
    ...
    $(document).on('click', '#update_administrator_form', function (){
        if ($("#administrator_form").valid()) {
            ...
            $.ajax({
                ...
                error: function () {
                    showPopupMessage('error','Something went wrong, Please try again. (Error Code: 560)');
                }
            });
        }
    });
	...
	function get_administrator_data_export(){
        ...
        $.ajax({
            url: "services_admin_api.php",
            type: "post",
            data: {mode: 'assistant_list_generate_report',type:'U'},
            success: function (response) { $('.admin-loading-image').hide();
				downloadFile( '<?php echo HOST_PATH;?>tmp/user_list_report.csv');
				
            },
            error: function () {
                showPopupMessage('error','Something went wrong, Please try again. (Error Code: 561)');
            }
        });
    }
  ...
  a.href = url;
  a.download = 'people_data.csv'; // optional: leave blank to use the original filename
  ...
  a.remove();
    ...
    function get_administrator_data(){
        ...
        $.ajax({
            ...
            error: function () {
                showPopupMessage('error','Something went wrong, Please try again. (Error Code: 562)');
            }
        });
    }
    ...
    $(document).on('click','.delete_administrator',function(){
        if(confirm("Are you sure to delete the public user ? This cannot be undone")){
            ...
            $.ajax({
                ...
                error: function () {
                    showPopupMessage('error','Something went wrong, Please try again. (Error Code: 563)');
                    //$('.admin-loading-image').hide();
                }
            });
        }
