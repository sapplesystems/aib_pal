    ...
    $(document).on('click', '.delete_item_from_scrapbook', function(){
        ...
        if(confirm("Are you sure to delete the entry from scrapbook? This cannot be undone")){
            ...
            $.ajax({
                ...
                error: function () {
                    $('.admin-loading-image').hide();
                    showPopupMessage('error','Something went wrong, Please try again. (Error Code: 564)');
                }
            });
        }
    });
    ...
    function getAllScrapbookEntries(scrapbook_id){
        if(scrapbook_id){
            ...
            $.ajax({
                ...
                error: function () {
                    showPopupMessage('error','Something went wrong, Please try again. (Error Code: 565)');
                }
            });
        }
    }
