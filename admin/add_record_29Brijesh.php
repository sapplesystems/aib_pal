		...
		<td align='left' valign='top'>
			...
			switch($OpCode)
			{
				...
				case "edit":
					...
					$OutBuffer[] = "</td><td class='parent-folder-info-link-cell' align='right'><a href=\"/admin/manage_my_archive.php?folder_id=".$_REQUEST['parent']."\">Return To List</a></td></tr>";
					$OutBuffer[] = "</table>";
					...
					$OutBuffer[] = aib_gen_form_header("pageform","/record_form_2018_updated.php",false,"validate_form");
					...
					// Save any option values as hidden fields

					foreach($FormData as $FormFieldName => $FormFieldValue)
					{
						if (preg_match("/^opt[\_]/",$FormFieldName) != false)
						{
							$OutBuffer[] = "<input type='hidden' name='$FormFieldName' value='$FormFieldValue'>";
						}
					}
...
//						"existing_fields" => $FieldsUsed,
			...
			switch($OpCode)
			{
				...
				case "save":
					...
					$TargetSpec[] = array( "url" => "/record_form_2018_updated.php", "title" => "Add Another Record","fields" => $TempFields);
					$OutBuffer[] = aib_chain_link_set($TargetSpec);
					break;
