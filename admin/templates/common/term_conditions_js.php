...
$(document).ready(function(){
	...
	$(document).on('click','#agree_term_condition',function(){
		...
		$.ajax({
			...
			eroor:function(){
				showPopupMessage('error','Something went wrong, Please try again. (Error Code: 887)');
			}
		});
	});
});
	...
	$.ajax({
		...
		success: function (data){
			...
			$('#term_cond_data_value').html(result.message); 
				...
				showPopupMessage('error', 'error','Something went wrong, Please try again. (Error Code: 888)');
			}
			$('.admin-loading-image').hide();
		},
		error: function () {
			showPopupMessage('error','Something went wrong, Please try again. (Error Code: 889)');
		}
	});
}
</script>
