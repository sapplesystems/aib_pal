<section class="content-header"><h5 class="list_title text-center"><span class="pull-left">Tasks To Be Completed </span> <span class="headingNameDesign"><?php echo $_SESSION['aib']['user_data']['item_title'];?></span></h5></section>
        ...
        if (!empty($uncompleteResponseData)) {
            foreach ($uncompleteResponseData as $unCompleteKey=>$uncompleteData) {
                if($uncompleteData['sub_group'] != ''){
					$uncompleteData_count = count($uncompleteData['list']);
					$incomplete_data['sub_group_id'][] = $unCompleteKey;
					$incomplete_data['count'][] = $uncompleteData_count;
					//$incomplete_data[$unCompleteKey] = $uncompleteData_count;
					//$incomplete_data[] = array('sub_group_id'=>$unCompleteKey,'count'=>$uncompleteData_count);
                ?>
                <tr>
                    ...
                    <td align="left">
						<span id="display_count_<?php echo $unCompleteKey; ?>"><?php echo $uncompleteData_count; ?></span>
						&nbsp;&nbsp;&nbsp;
						<a class="btn btn-info btn-xs" href="javascript:void(0);" onclick="markComplete(event,'<?php echo $unCompleteKey; ?>');">Mark Complete</a>
					</td>
                </tr>
            <?php } 
			}
...
<section class="content-header"><h5 class="list_title">Completed Tasks</h5></section>
        ...
        if (!empty($completeResponseData)) {
            foreach ($completeResponseData as $completeKey=>$completeData) {
                if($completeData['sub_group'] != ''){
					$completeData_count = count($completeData['list']);
					$complete_data['sub_group_id'][] = $completeKey;
					$complete_data['count'][] = $completeData_count;
					//$complete_data[$completeKey] = $completeData_count;
					//$complete_data[] = array('sub_group_id'=>$completeKey,'count'=>$completeData_count);
                ?>
                <tr id="complete_tr_<?php echo $completeKey; ?>">
                    ...
                    <td align="left">
						<?php echo $completeData_count; ?>
						&nbsp;&nbsp;&nbsp;
						<a class="btn btn-info btn-xs" href="javascript:void(0);" onclick="markInComplete(event,'<?php echo $_SESSION['aib']['user_data']['user_id']; ?>','<?php echo $completeKey; ?>');">Mark Incomplete</a>
					</td>
                </tr>
            <?php } 
			}
...
<script type="text/javascript">
	// set the data count display code start here
	var incomplete_data = '<?php echo json_encode($incomplete_data); ?>';
	var complete_data = '<?php echo json_encode($complete_data); ?>';
	var incomplete_data_J = JSON.parse(incomplete_data);
	var complete_data_J = JSON.parse(complete_data);
	if(incomplete_data_J.sub_group_id){
		var incomplete_data_len = incomplete_data_J.sub_group_id.length;
		for(var i = 0; i < incomplete_data_len; i++){
			var sgid = incomplete_data_J.sub_group_id[i];
			var count = incomplete_data_J.count[i];
			var indexof = -2;
			if(complete_data_J.sub_group_id){
				indexof = complete_data_J.sub_group_id.indexOf(sgid);
			}
			var complete_count = 0;
			if(indexof > -1){
				complete_count = complete_data_J.count[indexof];
				var total_count = (count + complete_count);
				$('#display_count_'+sgid).html(complete_count + '/' + total_count);
				$('#complete_tr_'+sgid).remove();
			}else{
				$('#display_count_'+sgid).html(0 + '/' + count);
			}
		}
	}
	// set the data count display code end here
