...
<div class="content-wrapper">
    <section class="content-header"> 
        <h4 class="list_title">Add DMCA</h4>
    </section>
        ...
        <div class="row">
            <div class="col-md-12">
			 ...
			 <div class="clearfix"></div>
                <form class="marginBottom30 formStyle form-group" action="" method="POST" id="dmca_field_id_form" name="dmca_field_id_form">
                       <input type="hidden" name="timestamp" id="timestamp" value="<?php echo time(); ?>">
                    <div class="row">
                        <div class="col-md-2 text-right"><strong>DMCA :</strong></div>
                        <div class="col-md-10">
							 <textarea class="form-control" rows="20" id="dmca_field_id" name="dmca_field_id"></textarea>
...
$(document).ready(function(){
    ...
    type: "post",
    data: {mode: 'get_term_and_condition', user_id: 1,form_type : 'DMCA'},
    success: function (data){
         var result = JSON.parse(data);
         if(result.status == 'success'){ 
             setTimeout(function(){ 
                tinymce.get("dmca_field_id").setContent(result.message);  
                $('.admin-loading-image').hide();
            }, 3000); 
         }else{
            showPopupMessage('error', 'error','Something went wrong, Please try again. (Error Code: 454)');
         }
    },
    error: function () {
        showPopupMessage('error','Something went wrong, Please try again. (Error Code: 455)');
    }
});
	
	
    $('#addtermCondButton').click(function(){ 
        if($("#dmca_field_id_form").valid()){ 
            //var termForm = $("#dmca_field_id_form").serialize();
            var terms_conditions = tinymce.get("dmca_field_id").getContent();
            $('.admin-loading-image').show();
            $.ajax({
                url: "services_admin_api.php",
                type: "post",
                data: {mode: 'term_condition_add_form', terms_conditions: terms_conditions ,form_type : 'DMCA'},
                success: function (data){
                     var result = JSON.parse(data);
                     if(result.status == 'success'){ 
                        showPopupMessage('success', result.message);
                     }else{
						showPopupMessage('error', result.message + ' (Error Code: 456)');
                     }
                     $('.admin-loading-image').hide();
                      location.reload();
                },
                error: function () {
                    showPopupMessage('error','Something went wrong, Please try again. (Error Code: 457)');
                }
            });
			}
    }); 
 
    $("#dmca_field_id_form").validate({
        rules: {
            dmca_field_id:{
                required: true 
            }           
...
{	
    var elem = tinymce.get("dmca_field_id").getContent();
    var mywindow = window.open('', 'PRINT', 'height=400,width=600');

    mywindow.document.write('<html><head><title> DMCA</title>');
    mywindow.document.write('</head><body >');
    mywindow.document.write('<h1>DMCA</h1>');
