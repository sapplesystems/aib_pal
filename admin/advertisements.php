...
include('include/advertisements.php');
...
// #########
	...
	$AIBKey = aib_get_with_default($FormData,"aibads-key",false);
...
include("template/top_menu_admin_data.php");
	...
	switch($OpCode)
	{
		...
		default:
			if (aib_open_db() != false)
			{
				...
				$PreloadBuffer = aib_generate_list_preload("aibads",$TotalListPages);
	...
	switch($OpCode)
	{
		...
		default:
			...
			// Get the list of archives and the current archive group

			aib_open_db();
			$ArchiveInfoList = array();
			...
			if ($ArchiveGroupList != false)
			{
				foreach($ArchiveGroupList as $ArchiveGroupRecord)
				{
					...
					$ArchiveInfoList["ag:".$ArchiveGroupID] = $ArchiveGroupTitle." ($ArchiveGroupCode)";
					$TempArchiveList = aib_get_archive_group_archive_list($GLOBALS["aib_db"],$ArchiveGroupID);
					foreach($TempArchiveList as $ArchiveRecord)
					{
						$ArchiveID = $ArchiveRecord["item_id"];
						$ArchiveTitle = $ArchiveRecord["item_title"];
						$ArchiveInfoList["ar:$ArchiveID"] = "&#9492; $ArchiveTitle";
					}
				}
			}
			...
			$OutBuffer[] = "<div name='aibads' id='aibads-data-div' class='aib-generic-list-div'>";
...
//				);
			...
			if (count($ArchiveInfoList) > 0)
			{
				$ListParam["extra_title_rows"][] = "Field Owner / Type Of Field: <select name='key' id='aibads-key'>";
				foreach($ArchiveInfoList as $InfoListName => $InfoListValue)
				{
					$ListParam["extra_title_rows"][] = "<option value=\"$InfoListName\">$InfoListValue</option>";
				}
			...
			$MenuCell .= "<a href='/field_form.php?opcode=add&$TargetFields' class='aib-folder-menu-link'>Create Advertisement</a>";
			$TitleCell .= "</td>";
			$TitleRow .= $TitleCell;
			$MenuCell .= "</td>";
			$TitleRow .= $MenuCell;
			...
			$OutBuffer[] = aib_generate_generic_list_frame_html("aibads",$ListParam);
			aib_close_db();
			...
			$Spec["extra_init_code"] = "
				\$('#aibads-key').change(function(Event) {
					listquery_aibads();
				});
			";
			...
			$ListData = aib_generate_scroll_table_handler($UserID,"aibads",$Spec);
