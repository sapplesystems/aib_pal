...
record_form_debug("Can't find zip in batch");
	...
	{
record_form_debug("Can't find csv in batch $SourceZIPName,$TempCSVFileName");
		return(false);
	}
	...
	// If row list is FALSE, we couldn't open the file
...
record_form_debug("Can't open CSV file");
	...
	// No CSV rows to check; fails.  CSV must contain data for import to work.
...
record_form_debug("File batch is $FileBatchID");
...
record_form_debug("File list for batch is ".var_export($FileList,true));
	$FirstZIP = false;
	...
	if ($FirstZIP == false)
	{
record_form_debug("No zip file in batch");
		return(false);
	}
...
function get_column_name_list($FileBatchID)
{
	...
	if ($FirstZIP == false)
	{
record_form_debug("No zip file in batch for get_column_name_list");
		return(false);
	}
	...
	if ($FirstCSV == false)
	{
record_form_debug("No CSV file in ZIP file $FirstZIP for get_column_name_list");
		return(false);
	}
	...
	{
record_form_debug("Can't extract CSV file $FirstCSV to $TempCSVFileName in ZIP file $FirstZIP for get_column_name_list");
		return(false);
	}
...
function get_csv_from_zip($SourceZIPName,$TargetCSVFileName)
{
	...
	if ($FirstCSV == false)
	{
record_form_debug("No CSV file in ZIP file $FirstZIP for get_csv_from_zip");
		return(false);
	}
	...
	if ($ExtractResult == false)
	{
record_form_debug("Can't extract CSV file $FirstCSV to $TempCSVFileName in ZIP file $FirstZIP for get_csv_from_zip");
		return(false);
	}
...
record_form_debug("No user data in aib session");
...
*/
					...
					$OutBuffer[] = "<input type='hidden' name='itemrecord_visible' value='$VisibilityFlag'>";
					$OutBuffer[] = "<input type='hidden' name='itemrecord_private' value='$PrivacyFlag'>";
					$OutBuffer[] = "<input type='hidden' name='import_format' value='$ImportType'>";
					$OutBuffer[] = "<input type='hidden' name='parent' value='".aib_get_with_default($FormData,"parent","-1")."'>";
					$OutBuffer[] = "<input type='hidden' name='archive_code' value='".aib_get_with_default($FormData,"archive_code","-1")."'>";
