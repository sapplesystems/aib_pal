...
/* FIX TO ALLOW LOGIN WITHOUT CSRF CHECK */
if ($_REQUEST['mode']=='login_user' || ($csrf_token && isValidCsrfToken($csrf_token))) {
...
switch ($_REQUEST['mode']) {
        ...
        switch ($_POST['type']) {
            ...
            case 'A':
                ...
                foreach ($responseData as $key => $value) {
                    if (isset($_POST['type2']) && $_POST['type2'] == 'claimed') {
                        if (!$value['_properties']['claimed_user'] || empty($value['_properties']['claimed_user'])) {
                            continue;
                        }
                    }
                    if (isset($_POST['type2']) && $_POST['type2'] == 'unclaimed') {
                        if (($value['_properties']['claimed_user'] || $value['_properties']['claimed_user2']) && ($value['_properties']['claimed_user'] != '2' && $value['_properties']['claimed_user2'] != '2')) {
                            continue;
                        }
                    }
                    $archiveData = getItemData($value['user_top_folder']);
                    $society_name_new = '';
                    if (json_decode($archiveData['properties']['temp_claim_data_' . $value['user_login']]) != '') {

                        $arr = json_decode($archiveData['properties']['temp_claim_data_' . $value['user_login']], true);
                        $found_key = array_search('society_name', array_column($arr, 'name'));
                        if ($found_key) {
                            $society_name_new = $arr[$found_key]['value'];
                        }
                    }


                    if (!empty($archiveData)) {
                        $value['item_id'] = $archiveData['item_id'];
                        $value['item_title'] = $archiveData['item_title'];
                        $value['user_pro_type'] = $value['_properties']['type'];
                        $value['username'] = $value['user_login'];
                        $value['society_name_new'] = $society_name_new;
	...
	case 'get_tnc_version':
		...
		$tc_resp = aibServiceRequest($tc_data, 'browse');
		if($tc_resp['status'] == 'OK'){
			if($tc_resp['info']['records'][$tnc_id]){
				$responseData = array('status' => 'success', 'message' => $tc_resp['info']['records'][$tnc_id]);
			}else{
				$responseData = array('status' => 'error', 'message' => 'Something went wrong');
			}
		}
		...
		if($tc_resp['status'] == 'OK'){
			...
			if($user_info_resp['status'] == 'OK'){
				...
				if(!empty($accept_term_service)){
					...
					$decoded_accept_term_service_arr = json_decode($decoded_accept_term_service, true);
					$html = '';
					for($i = 1; $i <= $tnc_version; $i++){
						$datetime = 'Not Accepted';
						if(array_key_exists('terms_and_conditions_'.$i,$decoded_accept_term_service_arr)){
							$datetime = date('m-d-Y H:i:s', strtotime($decoded_accept_term_service_arr['terms_and_conditions_'.$i]));
						}
						$html .= '<tr>';
						$html .= '<td class="tnc_version" id="terms_and_conditions_'.$i.'">Term and Condition Version '.$i.'</td>';
						$html .= '<td>'.$datetime.'</td>';
						$html .= '</tr>';
					}
					$responseData = array('status' => 'success', 'html' => $html);
				}else{
					$html = '';
					for($i = 1; $i <= $tnc_version; $i++){
						$datetime = 'Not Accepted';
    ...
    case 'forms-list':
        ...
        print json_encode($responseData);
        break;
    case 'save_help_message':
        $element_id = $_POST['data']['element_id'];
        $language = $_POST['data']['language'];
        $text = addslashes($_POST['data']['text']);
        ...
        $APIResponse = aibServiceRequest($PostData, "ui_messages");
        print json_encode($APIResponse);
        break;
    case 'list_all_help_message':
        $PostData = array(
            "_key" => APIKEY,
            "_session" => $sessionKey,
            "_user" => "1",
            "_op" => "list_by_location",
    ...
    case 'delete_help_message':
        ...
        print json_encode($APIResponse);
        break;
    case 'get_help_message':
        $PostData = array(
            "_key" => APIKEY,
            "_session" => $sessionKey,
            "_user" => "1",
            "_op" => "get_ui_element_nameloc",
            "name" => $_POST['name'],
            "location" => $_POST['location'],
        );
        ...
        if ($type != 'manage') {
            ...
            foreach ($responseData as $responseKey => $newresponseData) {
                ...
                if ($newresponseData['field_owner_id'] != '' && $newresponseData['field_owner_id'] != '-1' && $newresponseData['field_owner_id'] != '-2') {
                    ...
                    $newresponseData['display_owner_title'] = 'Root';
                }
    ...
    case 'update_profile':
        ...
        if ($apiResponse['status'] == 'OK') {
            ...
            if (isset($postData['term_service']) && !empty($postData['term_service'])) {
                $userProperty[$ui]['name'] = 'term_service';
                $userProperty[$ui]['value'] = $postData['term_service'];
				$ui++;
				// term and condition versioning start here
				$tc_data = array(
					'_key' => APIKEY,
				...
				if (isset($postData['occasional_update']) && $postData['occasional_update'] == 'Y') {
                    $userProperty[$ui]['name'] = 'occasional_update';
                    $userProperty[$ui]['value'] = $postData['occasional_update'];
                }
...
//            "field_format" => $postData['field_format_detail'],
        ...
        if ($apiResponse['status'] == 'OK') {
			if(isset($postData['ddvalues']) and $postData['ddvalues']!=''){
				...
				$PostData = array(
					...
					"_op" => "set_item_prop",
					//"opt_long" => 'Y',
					"obj_id" => CUSTOM_FIELD_DD,
					"propname_1" => $apiResponse['info'],
					"propval_1" => ($postData['ddvalues']),
					
				);
				
				$apiResponseF = aibServiceRequest($PostData, "browse");
    ...
    case 'get_fields':
		...
		if($recordget[0]['field_data_type']=='DD'){
			...
			$PostData = array(
				...
				$apiResponse = aibServiceRequest($PostData, "browse");
				$dd_values='';
				if(isset($apiResponse['info']['records'][$recordget[0]['field_id']])){
					 $dd_values=$apiResponse['info']['records'][$recordget[0]['field_id']];
				}
				//print_r($apiResponse);
    ...
    case'get_temp_claim_data':
        ...
        $apiResponseProps = aibServiceRequest($apiRequestData, 'browse');
        $responseData = $apiResponseProps['info']['records']['temp_claim_data_' . $username];
        print $responseData;
        break;
    ...
    case 'update-manage-field':
        ...
        if ($apiResponse['status'] == 'OK') {
			...
			if(isset($_POST['ddvalues']) and $_POST['ddvalues']!=''){
				...
				$PostData = array(
					...
					//"opt_long" => 'Y',
					"obj_id" => CUSTOM_FIELD_DD,
					"propname_1" => $_POST['update_field_id'],
					"propval_1" => ($_POST['ddvalues']),
    ...
    case 'list_tree_items':
		 //Fix start for Issue ID 0002305 on 10-Oct-2023
		if(!isset($_SESSION['aib']['user_data']['user_id']) or $_SESSION['aib']['user_data']['user_id']==0){
        ...
        $search_filter = array(array('name' => 'aibftype', "value" => "rec"), array('name' => 'aibftype', "value" => "sg"), array('name' => 'aibftype', "value" => "col"), array('name' => 'aibftype', "value" => "IT"), array('name' => 'link_class', "value" => "public"), array('name' => 'visible_to_public', "value" => 0), array('name' => 'aib:visible', "value" => "N"), array('name' => 'link_class', "value" => "historical_connection"));
        $postData = array(
            "_key" => APIKEY,
            "_session" => $sessionKey,
            "_user" => $_SESSION['aib']['user_data']['user_id'],
            ...
            if ($_SESSION['aib']['user_data']['user_type'] == 'U') {
                ...
                if ($itemData['item_type'] == 'SG') {
                    if ($itemDataArray['item_type'] == 'SG') {
                        ...
                        $apiResponse['info']['records'][$itemKey]['child_count'] = $itemDataArray['property_counts'][4]['count'] - ($itemDataArray['property_counts'][5]['count'] + $itemDataArray['property_counts'][8]['count']); //+$visileFalseCount//$disabledCount
                    }
                    if ($itemDataArray['property_counts'][2]['count'] != 0) {
                        $apiResponse['info']['records'][$itemKey]['sg_count'] = $itemDataArray['property_counts'][2]['count'] - $disabledCount;
                    }
    ...
    case 'create_new_items':
        ...
        switch ($postData['item_request_type']) {
            case 'ar':
                ...
                $ItemProp = getItemProp($postData['parent_id']);
                $ItemPropRecord = $ItemProp['info']['records'];
                setItemProp($postData['parent_id'], [
                    'propname_1' => 'total_archive',
                    'propval_1' => (!empty($ItemPropRecord['total_archive'])) ? ($ItemPropRecord['total_archive'] + 1) : 1
                ]);
                break;
            case 'col':
                ...
                //Fix start for Issue ID 2140 on 23-Feb-2023
                $getPathData = getPath($postData['parent_id']);
                $archive_group_id = $getPathData['info']['records'][1]['item_id'];
                $ItemProp = getItemProp($archive_group_id);
                $ItemPropRecord = $ItemProp['info']['records'];
                setItemProp($archive_group_id, [
                    'propname_1' => 'total_collection',
                    'propval_1' => (!empty($ItemPropRecord['total_collection'])) ? ($ItemPropRecord['total_collection'] + 1) : 1
                ]);
                break;
            case 'sg':
                ...
                $aditionalProp['propname_5'] = 'download_to_public_item';
                $aditionalProp['propval_5'] = $postData['download_to_public_item'];
                /*<!----------- Issue Id 2108 Fix End date 12-Dec-2022--------------->*/
                if ($_SESSION['aib']['user_data']['user_type'] != 'U') {
                ...
                $ItemPropRecord = $ItemProp['info']['records'];
                setItemProp($archive_group_id, [
                    'propname_1' => 'total_sub_group',
                    'propval_1' => (!empty($ItemPropRecord['total_sub_group'])) ? ($ItemPropRecord['total_sub_group'] + 1) : 1
                ]);
                break;
            default :
                $item_title = $postData['item_title'];
                ...
                $aditionalProp['propname_3'] = 'status';
                $aditionalProp['propval_3'] = 1;
                break;
        }
        ...
        break;
    //Fix start for Issue ID 2140 on 23-Feb-2023
    case 'add_total_item_count':
        $item_count = (!empty($_POST['item_count'])) ? $_POST['item_count'] : 0;
        $getPathData = getPath($_POST['parent_id']);
        $archive_group_id = $getPathData['info']['records'][1]['item_id'];
        $ItemProp = getItemProp($archive_group_id);
        $ItemPropRecord = $ItemProp['info']['records'];
        setItemProp($archive_group_id, [
            'propname_1' => 'total_item',
            'propval_1' => (!empty($ItemPropRecord['total_item'])) ? ($ItemPropRecord['total_item'] + $item_count) : $item_count
        ]);
        $responseData = array('status' => 'success', 'message' => 'Item count saved successfully.');
        print json_encode($responseData);
        break;
    case 'download_report_list':
        $postData = array(
            "_key" => APIKEY,
            "_session" => $sessionKey,
            "_user" => 1,
        ...
        if($apiResponse['status'] == 'OK'){
            ...
            if(!empty($_POST['download_csv'])){
                $path = ROOT_PATH . '/tmp/';
                $filename = 'archive_report_' . time() . '.csv';
                $fp = fopen($path . $filename, 'w');

                $CsvColumn[] = 'Title';
                $CsvColumn[] = 'Total Archive';
                $CsvColumn[] = 'Total Collection';
                $CsvColumn[] = 'Total Sub Group';
                $CsvColumn[] = 'Total Record';
                $CsvColumn[] = 'Total Item';
                fputcsv($fp, $CsvColumn);

                foreach($records as $key => $val){
                    $total_archive = ($val['properties']['total_archive']) ? intval($val['properties']['total_archive']) : 0;
                    $total_collection = ($val['properties']['total_collection']) ? intval($val['properties']['total_collection']) : 0;
                    $total_sub_group = ($val['properties']['total_sub_group']) ? intval($val['properties']['total_sub_group']) : 0;
                    $total_record = ($val['properties']['total_record']) ? intval($val['properties']['total_record']) : 0;
                    $total_item = ($val['properties']['total_item']) ? intval($val['properties']['total_item']) : 0;

                    $CsvRow = [];
                    $CsvRow[] = $val['item_title'];
                    $CsvRow[] = $total_archive;
                    $CsvRow[] = $total_collection;
                    $CsvRow[] = $total_sub_group;
                    $CsvRow[] = $total_record;
                    $CsvRow[] = $total_item;
                    fputcsv($fp, $CsvRow);
                }
                ...
                $download_url = 'https://' . HOST_ROOT_PATH . '/tmp/' . $filename;
				/* fix end for 2274 10-july */
                $response = array('status' => 'success', 'message' => '', 'data_type' => 'url', 'data' => $download_url);
            }else{
                $html = '';
                $TrRows = '';
                $TrTotal = '';
                ...
                foreach($records as $key => $val){
                    ...
                    $total_archive_count = ($total_archive_count + $total_archive);
                    $total_collection_count = ($total_collection_count + $total_collection);
                    $total_sub_group_count = ($total_sub_group_count + $total_sub_group);
                    $total_record_count = ($total_record_count + $total_record);
                    $total_item_count = ($total_item_count + $total_item);
    
                    $TrRows .= '<tr>';
                    $TrRows .= '<td class="text-center">' . $val['item_title'] . '</td>';
                    $TrRows .= '<td class="text-center">' . $total_archive . '</td>';
                    $TrRows .= '<td class="text-center">' . $total_collection . '</td>';
                    $TrRows .= '<td class="text-center">' . $total_sub_group . '</td>';
                    $TrRows .= '<td class="text-center">' . $total_record . '</td>';
                    $TrRows .= '<td class="text-center">' . $total_item . '</td>';
                    $TrRows .= '</tr>';
                }
                $TrTotal += '<tr>';
                $TrTotal += '<th class="text-center"></th>';
                $TrTotal += '<th class="text-center">' . $total_archive_count . '</th>';
                $TrTotal += '<th class="text-center">' . $total_collection_count . '</th>';
                $TrTotal += '<th class="text-center">' . $total_sub_group_count . '</th>';
                $TrTotal += '<th class="text-center">' . $total_record_count . '</th>';
                $TrTotal += '<th class="text-center">' . $total_item_count . '</th>';
                $TrTotal += '</tr>';
    
                $html = $TrTotal . $TrRows . $TrTotal;
                $response = array('status' => 'success', 'message' => '', 'data_type' => 'html', 'data' => $html);
            }
        }else{
    ...
    case 'run_ocr_onfolder_flag':
		...
		$PostData = array(
            ...
            "_session" => $sessionKey,
            "_user" => $_SESSION['aib']['user_data']['user_id'],
            "_op" => "set_item_prop_recurse",
            "opt_long" => 'Y',
            "parent_item" => $_POST['obj_id'],
            "prop_name" => "ocr_flag",
            "prop_value" => "1",
            "depth" => "-1",
            "include_parent" => "Y",
            "valid_types" => "F,I,L,LF,LI",
                //"delete_flag" => "Y",
                //"obj_type_list" => "CO,SG",
        );
        $apiResponse = aibServiceRequest($PostData, "browse");
        ...
        if ($_POST['form_type'] == 'PC') {
			...
			if($tc_resp['status'] == 'OK'){
				...
				if(empty($tnc_version)){
					...
					$tnc_version = $tnc_version + 1;
				}
			}
            $apiRequestDataItem['propname_2'] = 'terms_and_conditions';
            $apiRequestDataItem['propval_2'] = $termCond;
			$apiRequestDataItem['propname_3'] = 'tnc_version';
            $apiRequestDataItem['propval_3'] = $tnc_version;
			$apiRequestDataItem['propname_4'] = 'terms_and_conditions_'.$tnc_version;
            $apiRequestDataItem['propval_4'] = $termCond;
    ...
    case 'set_claimed_popup_message':
        $responseData = array('status' => 'error', 'message' => 'Something went wrong, Please try again');
        $claimed_message = addslashes($_POST['claimed_message']);
        $apiRequestDataItem = array(
            '_key' => APIKEY,
            '_user' => $_SESSION['aib']['user_data']['user_id'],
            '_op' => 'set_item_prop',
        ...
        $apiRequestDataItem['propname_2'] = 'claimed_message';
        $apiRequestDataItem['propval_2'] = ($claimed_message && $claimed_message != '') ? $claimed_message : 'Are you the manager or owner of this society?';
        $message = 'Claimed message added successfully';
        ...
        if (!empty($postData)) {
            ...
            if ($apiResponse['status'] == 'OK') {
                ...
                if ($result['status'] == 'OK') {
                    ...
                    $email_data['subject'] = 'ArchiveInABox � update user details';
    ...
    case 'update_archive_group_details':
        ...
        if (isset($postData['archive_id']) && $postData['archive_id'] != '') {
            ...
            if(!empty($postData['archive_watermark_text'])){
                $PostData = array(
                    "_key" => APIKEY,
                    "_session" => $sessionKey,
                    "_user" => $_SESSION['aib']['user_data']['user_id'],
                    "_op" => "set_item_prop_recurse",
                    "opt_long" => 'Y',
                    "parent_item" => $archive_id,
                    "prop_name" => "archive_watermark_text",
                    "prop_value" => $postData['archive_watermark_text'],
                    "depth" => "-1",
                    "include_parent" => "Y",
                    "valid_types" => "F",
                        //"delete_flag" => "Y",
    ...
    case 'save_archive_watermark_text':
        $responseData = array('status' => 'error', 'message' => 'Something went wrong! Please try again');
        $archive_id = $_POST['wm_item_id'];
        $archive_watermark_text = $_POST['archive_watermark_text'];
		
        //$archive_watermark_text = str_pad($archive_watermark_text, 30, ' ', STR_PAD_BOTH);
        /* $apiRequestDataItem = array(
          '_key' => APIKEY,
          '_user' => $_SESSION['aib']['user_data']['user_id'],
          '_op' => 'set_item_prop',
          ...
          'obj_id' => $archive_id,
          'opt_long' => 'Y',
          'propname_1' => 'archive_watermark_text',
          'propval_1' => $archive_watermark_text
          );
          $apiResponse = aibServiceRequest($apiRequestDataItem, 'browse'); */
        //if ($apiResponse['status'] == 'OK') {
		/************* SS FIX START 0002309   21-Sep-2023 ************************/
		if(isset($_POST['archive_download_watermark_text']) and trim($_POST['archive_download_watermark_text'])!=''){
			$archive_download_watermark_text = ($_POST['archive_download_watermark_text'])?$_POST['archive_download_watermark_text']:'';
		 $PostData = array(
            "_key" => APIKEY,
            "_session" => $sessionKey,
            "_user" => $_SESSION['aib']['user_data']['user_id'],
            ...
            "opt_long" => 'Y',
            "parent_item" => $archive_id,
            "prop_name" => "archive_download_watermark_text",
            "prop_value" => $archive_download_watermark_text,
            "depth" => "-1",
            "include_parent" => "Y",
            "valid_types" => "F",
                //"delete_flag" => "Y",
        ...
        $PostData = array(
            ...
            "parent_item" => $archive_id,
            "prop_name" => "archive_watermark_text",
            "prop_value" => $archive_watermark_text,
            "depth" => "-1",
            "include_parent" => "Y",
            "valid_types" => "F",
                //"delete_flag" => "Y",
    ...
    case 'fetch_archive_watermark_text':
        $responseData = array('status' => 'error', 'message' => 'Something went wrong! Please try again');
        $archive_id = $_POST['item_id'];
        $apiRequestDataItem = array(
            '_key' => APIKEY,
            '_user' => $_SESSION['aib']['user_data']['user_id'],
            '_op' => 'get_item_prop',
    ...
    case 'get_archive_prop_details':
        ...
        if (isset($user_id) && !empty($user_id)) {
			/********* Issue fix to update archive_user_id pro in archive 11-dec-2024 *******************/
			if(!isset($itemDetails['properties']['archive_user_id']) || $itemDetails['properties']['archive_user_id']==''){
				 $apiRequestDataItem = array(
					'_key' => APIKEY,
					'_user' => ($_SESSION['aib']['user_data']['user_id']) ? $_SESSION['aib']['user_data']['user_id'] : 1,
					'_op' => 'set_item_prop',
					'_session' => $sessionKey,
					'obj_id' => $archive_id,
			...
			$user_id = $itemDetails['properties']['archive_user_id'];
    ...
    case 'get_user_details':
        ...
        if ($apiResponse['status'] == 'OK') {
            ...
            if ($terms_condition['timestamp'] > $userProfileData['timestamp']) {
                $userProfileData['term_service'] = '';
            }
        }
    ...
    case 'update_items_by_id':
        ...
        switch ($postData['item_type']) {
            ...
            case 'SG':
                ...
                $aditionalProp['propval_3'] = $postData['display_archive_level_advertisements'];
				/*<!----------- Issue Id 2108 Fix End date 12-Dec-2022--------------->*/
				$aditionalProp['propname_4'] = 'download_to_public_item';
                $aditionalProp['propval_4'] = $postData['download_to_public_item'];
				/*<!----------- Issue Id 2108 Fix End date 12-Dec-2022--------------->*/
                $item_class = 'sg';
                break;
            case 'RE':
    ...
    case 'delete_item_by_id':
        ...
        if ($item_id) {
            //Fix start for Issue ID 2140 on 23-Feb-2023
            $to_be_delete_result = getTotalNumberOfChildInTheItem($item_id);

            $prop_count = 1;
            $prop_array = [];

            if(!empty($to_be_delete_result) && count($to_be_delete_result)){
                ...
                $getitemRequestDataResp = aibServiceRequest($getitemRequestData, 'browse');
                if(!empty($getitemRequestDataResp) && $getitemRequestDataResp['status'] == 'OK'){
                    $getitemRequestDataRecord = $getitemRequestDataResp['info']['records'];
                    $current_item_type = $getitemRequestDataRecord[0]['item_type'];
                }
                //delete knowlegde base item

                $ar_count = (!empty($current_item_type) && ($current_item_type == 'AR' || $current_item_type == 'AG')) ? ($to_be_delete_result['ar_count'] + 1) : $to_be_delete_result['ar_count'];
                $co_count = (!empty($current_item_type) && $current_item_type == 'CO') ? ($to_be_delete_result['co_count'] + 1) : $to_be_delete_result['co_count'];
                $sg_count = (!empty($current_item_type) && $current_item_type == 'SG') ? ($to_be_delete_result['sg_count'] + 1) : $to_be_delete_result['sg_count'];
                $re_count = (!empty($current_item_type) && $current_item_type == 'RE') ? ($to_be_delete_result['re_count'] + 1) : $to_be_delete_result['re_count'];
                $it_count = (!empty($current_item_type) && $current_item_type == 'IT') ? ($to_be_delete_result['it_count'] + 1) : $to_be_delete_result['it_count'];

                $getPathData = getPath($item_id);
                $archive_group_id = $getPathData['info']['records'][1]['item_id'];
                $ItemProp = getItemProp($archive_group_id);
                $ItemPropRecord = $ItemProp['info']['records'];

                if(!empty($ItemPropRecord['total_archive']) && !empty($ar_count)){
                    $ar_count = ($ItemPropRecord['total_archive'] - $ar_count);
                    if($ar_count <= 0){
                        $ar_count = 0;
                    }
                    $prop_array['propname_' . $prop_count] = 'total_archive';
                    $prop_array['propval_' . $prop_count] = $ar_count;
                    $prop_count++;
                }
                if(!empty($ItemPropRecord['total_collection']) && !empty($co_count)){
                    $co_count = ($ItemPropRecord['total_collection'] - $co_count);
                    if($co_count <= 0){
                        $co_count = 0;
                    }
                    $prop_array['propname_' . $prop_count] = 'total_collection';
                    $prop_array['propval_' . $prop_count] = $co_count;
                    $prop_count++;
                }
                if(!empty($ItemPropRecord['total_sub_group']) && !empty($sg_count)){
                    $sg_count = ($ItemPropRecord['total_sub_group'] - $sg_count);
                    if($sg_count <= 0){
                        $sg_count = 0;
                    }
                    $prop_array['propname_' . $prop_count] = 'total_sub_group';
                    $prop_array['propval_' . $prop_count] = $sg_count;
                    $prop_count++;
                }
                if(!empty($ItemPropRecord['total_record']) && !empty($re_count)){
                    $re_count = ($ItemPropRecord['total_record'] - $re_count);
                    if($re_count <= 0){
                        $re_count = 0;
                    }
                    $prop_array['propname_' . $prop_count] = 'total_record';
                    $prop_array['propval_' . $prop_count] = $re_count;
                    $prop_count++;
                }
                if(!empty($ItemPropRecord['total_item']) && !empty($it_count)){
                    $it_count = ($ItemPropRecord['total_item'] - $it_count);
                    if($it_count <= 0){
                        $it_count = 0;
                    }
                    $prop_array['propname_' . $prop_count] = 'total_item';
                    $prop_array['propval_' . $prop_count] = $it_count;
                    $prop_count++;
                }

                setItemProp($archive_group_id, $prop_array);
            }
            //Fix end for Issue ID 2140 on 23-Feb-2023
	...
	case 'mark_complete_record':
		...
		if(!empty($folder_id)){
			...
			$apiResponse = aibServiceRequest($postData, 'browse');
			if($apiResponse['status'] == 'OK'){
				foreach($apiResponse['info']['records'] as $v){
					if($v['item_type'] == 'RE' && $v['item_id']){
						markRecordAsComplete($v['item_id']);
					}
				}
				$response = array('status' => 'success', 'message' => 'completed');
			}
		}
		echo json_encode($response);
		break;
	case 'mark_incomplete_record':
		...
		if ($assistant_id && $sub_group_id) {
			...
			if ($apiResponseStatus['status'] == 'OK') {
				...
				$apiResponse = aibServiceRequest($apiRequestData, 'dataentry');
				if ($apiResponse['status'] == 'OK') {
					$responseData = array('status' => 'success', 'message' => 'Incomplete');
				}
			}
		}
        print json_encode($responseData);
    ...
    case 'edit_assistant_uncomplete_data':
		...
		if(!empty($item_id)){
            ...
            $apiResponse = aibServiceRequest($apiRequestData, 'browse');
			if($apiResponse['status'] == 'OK'){
				$item_property = $apiResponse['info']['records'];
			}
		}
        if ($item_type == 'RE') {
    ...
    case 'update_assistant_item':
        ...
        if ($apiResponse['status'] == 'OK') {
			if(!empty($postData['parent_id'])){
				$apiRequestDataItem = array(
					...
					'_op' => 'set_item_prop',
					'_session' => $sessionKey,
					'obj_id' => $postData['parent_id'],
					'propname_1' => 'last_updated_on',
					'propval_1' => date('Y-m-d H:i:s'),
				);
				$response = aibServiceRequest($apiRequestDataItem, 'browse');
			}
			
		//	<!------- SS Fix Start for Issue ID 2333 on 17-Jan-2024 ---->
			if(!empty($postData['itemrecord_address_line'])){
				$lng=$postData['itemrecord_lat'];
				$lat=$postData['itemrecord_lng'];	
					if(trim($postData['itemrecord_address_line'])!='' || trim($postData['itemrecord_address_city'])!='' || trim($postData['itemrecord_address_state'])!='' || trim($postData['itemrecord_address_pin_code'])!='')
					{
						$address=trim($postData['itemrecord_address_line'])." ".trim($postData['itemrecord_address_city'])." ".trim($postData['itemrecord_address_state'])." ".trim($postData['itemrecord_address_pin_code']);
					...
					$SetItemLocParam["obj_id"] =$postData['parent_id']; 
					$SetItemLocParam["lat"] = $lat;
					$SetItemLocParam["lon"] = $lng;
					$SetItemLocParam["alt"] =  '';
					$SetItemLocResponseAdd = aibServiceRequest($SetItemLocParam,"locationsearch");
				...
				$apiRequestDataItem = array(
					...
					'_session' => $sessionKey,
					'obj_id' => $postData['parent_id'],
					'propname_1' => 'itemrecord_address_line',
					'propval_1' => $postData['itemrecord_address_line'],
					'propname_2' => 'itemrecord_address_city',
					'propval_2' => $postData['itemrecord_address_city'],
					'propname_3' => 'itemrecord_address_state',
					'propval_3' => $postData['itemrecord_address_state'],
					'propname_4' => 'itemrecord_address_pin_code',
					'propval_4' => $postData['itemrecord_address_pin_code'],
					'propname_5' => 'itemrecord_lat',
					'propval_5' => $lat,
					'propname_6' => 'itemrecord_lng',
					'propval_6' => $lng,
				);
				$response = aibServiceRequest($apiRequestDataItem, 'browse');
			}
			//<!------- SS end  Start for Issue ID 2333 on 17-Jan-2024 ---->
			if(!empty($postData['item_id'])){
				$apiRequestDataItem = array(
					...
					'_session' => $sessionKey,
					'obj_id' => $postData['item_id'],
					'propname_1' => 'item_updated_by_assistant',
					'propval_1' => '1', //$ad_sorting_order_value, 
					'propname_2' => 'last_updated_on',
					'propval_2' => date('Y-m-d H:i:s'),
				);
				$response = aibServiceRequest($apiRequestDataItem, 'browse');
			}
			...
			if(!empty($postData['is_skipped'])){
				//echo '<pre>';print_r($_SESSION['aib']['current_records']);echo '</pre>';
				//echo '<pre>';print_r($_SESSION['aib']['current_records_keys']);echo '</pre>';
				$ind = 0;
				$record_array_count = count($_SESSION['aib']['current_records'][$postData['parent_id']]['item_records']);
				foreach($_SESSION['aib']['current_records'][$postData['parent_id']]['item_records']  as $kk => $vv){
					$ind++;
					//echo '<pre>';print_r($vv['item_records']);echo '</pre>';
					//echo '$kk - '.$kk. 'vv item id - '.$vv['item_id'].', pp - '.$postData['item_id'].'==';
					if($vv['item_id'] == $postData['item_id']){
						$next_item_key = $ind;
						break;
					}
				}
				//echo 'record_array_count-'.$record_array_count.', next_item_key-'.$next_item_key;
				$record_number = array_search($postData['parent_id'],$_SESSION['aib']['current_records_keys']) + 1;
				//echo 'record_number - '.$record_number;
				$next_record_id = $_SESSION['aib']['current_records_keys'][$record_number];
				//echo 'next_record_id - '.$next_record_id;
				if($record_array_count == $next_item_key){
					//$nextItemId = getNextUncompleteItemToEdit();
					if($next_record_id && $_SESSION['aib']['current_records'][$next_record_id]['item_records'][0]['item_id']){
						print json_encode(array('status' => 'success', 'item_id' => $_SESSION['aib']['current_records'][$next_record_id]['item_records'][0]['item_id'], 'parent_id' => $next_record_id, 'is_skipped' => '1'));
					}else{
						$nextItemId = getNextUncompleteItemToEdit();
						print json_encode(array('status' => $nextItemId['status'], 'item_id' => $nextItemId['item_id'], 'parent_id' => $nextItemId['parent_id']));
					}
				}else{
					print json_encode(array('status' => 'success', 'item_id' => $_SESSION['aib']['current_records'][$postData['parent_id']]['item_records'][$next_item_key]['item_id'], 'parent_id' => $postData['parent_id'], 'is_skipped' => '1'));
				}
			}else if ($markCompletedStatus) {
                $nextItemId = getNextUncompleteItemToEdit();
				print json_encode(array('status' => $nextItemId['status'], 'item_id' => $nextItemId['item_id'], 'parent_id' => $nextItemId['parent_id']));
            }
        } else {
        ...
        break;
    case 'set_ad_sorting_order':
        $response = array('status' => 'error', 'msg' => '', 'record_id' => '');
        $item_id = $_POST['item_id'];
        if ($item_id) {
            $sorting_order = $_POST['sorting_order'];
            /* $ad_sorting_order_value = '0';
              if($_POST['sorting_order'] == 'true'){
              $ad_sorting_order_value = '1';
              } */
            $apiRequestDataItem = array(
                '_key' => APIKEY,
                '_user' => 1,
                ...
                '_session' => $sessionKey,
                'obj_id' => $item_id,
                'propname_1' => 'ad_sorting_order',
                'propval_1' => $sorting_order, //$ad_sorting_order_value, 
            );
            $response = aibServiceRequest($apiRequestDataItem, 'browse');
            if ($response['status'] == 'OK') {
                $PostData = array(
                    ...
                    //"opt_long" => 'Y',
                    "parent_item" => $item_id,
                    "prop_name" => "ad_sorting_order",
                    "prop_value" => $sorting_order,
                    "depth" => "-1",
                    "include_parent" => "Y",
                    "valid_types" => "F",
                        //"delete_flag" => "Y",
                ...
                $response = aibServiceRequest($PostData, "browse");
            }
        }
        echo json_encode($response);
        break;
    case 'set_ad_display_type':
        $response = array('status' => 'error', 'msg' => '', 'record_id' => '');
        $item_id = $_POST['item_id'];
        if ($item_id) {
            ...
            $apiRequestDataItem = array(
                ...
                '_session' => $sessionKey,
                'obj_id' => $item_id,
                'propname_1' => 'ad_display_type',
                'propval_1' => $display_type, //$ad_sorting_order_value, 
            );
            $response = aibServiceRequest($apiRequestDataItem, 'browse');
            if ($response['status'] == 'OK') {
                $PostData = array(
                    ...
                    //"opt_long" => 'Y',
                    "parent_item" => $item_id,
                    "prop_name" => "ad_display_type",
                    "prop_value" => $display_type,
                    "depth" => "-1",
                    "include_parent" => "Y",
                    "valid_types" => "F",
                    //"delete_flag" => "Y",
        ...
        echo json_encode($response);
        break;
    case 'set_ad_display_flip_time':
        $response = array('status' => 'error', 'msg' => '', 'record_id' => '');
        $item_id = $_POST['item_id'];
        if ($item_id) {
            $flip_time = intval($_POST['flip_time']);
            /*if($flip_time < 10){
                $flip_time = 10;
            }*/
            /* $ad_sorting_order_value = '0';
            ...
            $apiRequestDataItem = array(
                ...
                '_session' => $sessionKey,
                'obj_id' => $item_id,
                'propname_1' => 'ad_display_flip_time',
                'propval_1' => $flip_time, //$ad_sorting_order_value, 
            );
            $response = aibServiceRequest($apiRequestDataItem, 'browse');
            if ($response['status'] == 'OK') {
                $PostData = array(
                    ...
                    //"opt_long" => 'Y',
                    "parent_item" => $item_id,
                    "prop_name" => "ad_display_flip_time",
                    "prop_value" => $flip_time,
                    "depth" => "-1",
                    "include_parent" => "Y",
                    "valid_types" => "F",
                    //"delete_flag" => "Y",
        ...
        echo json_encode($response);
        break;
    case 'set_google_ad_script':
        $response = array('status' => 'error', 'message' => 'Something went wrong, Please try again');
        $google_ad_script = $_POST['ad_script'];//addslashes($_POST['ad_script']);
        $position1 = $_POST['position1'];
        $position2 = $_POST['position2'];
        $position3 = $_POST['position3'];
        $position4 = $_POST['position4'];
		$switch_off_all_adds = $_POST['switch_off_all_adds'];
        $item_id = $_POST['item_id'];//addslashes($_POST['ad_script']);
        if($item_id){
            $apiRequestDataItem = array(
                '_key' => APIKEY,
                '_user' => $_SESSION['aib']['user_data']['user_id'],
            ...
            $apiRequestDataItem['propname_2'] = 'google_ad_script';
            $apiRequestDataItem['propval_2'] = $google_ad_script;
            $apiRequestDataItem['propname_3'] = 'top_left_position';
            $apiRequestDataItem['propval_3'] = ($position1) ? $position1 : '';
            $apiRequestDataItem['propname_4'] = 'top_right_position';
            $apiRequestDataItem['propval_4'] = ($position2) ? $position2 : '';
            $apiRequestDataItem['propname_5'] = 'bottom_left_position';
            $apiRequestDataItem['propval_5'] = ($position3) ? $position3 : '';
            $apiRequestDataItem['propname_6'] = 'bottom_right_position';
            $apiRequestDataItem['propval_6'] = ($position4) ? $position4 : '';
			$apiRequestDataItem['propname_6'] = 'switch_off_all_adds';
            $apiRequestDataItem['propval_6'] = ($switch_off_all_adds) ? $switch_off_all_adds : '';
            $api_response = aibServiceRequest($apiRequestDataItem, 'browse');
        }
        ...
        print json_encode($response);
        break;
    case 'get_advertisement_profile':
        $response = array('status' => 'error', 'msg' => '', 'record_id' => '');
        if(!empty($_POST['record_id'])){
            $record_id = $_POST['record_id'];
            $advert_get_profile = array(
                "_key" => APIKEY,
                "_session" => $sessionKey,
                "_user" => "1",
                "_op" => "advert_get",
            ...
            $response = aibServiceRequest($advert_get_profile, "advertisement");
        }
        echo json_encode($response);
        break;
    case 'upload_advertisement':
        $response = array('status' => 'error', 'msg' => '', 'record_id' => '');

        if (!empty($_POST['ad_item_id'])) {
            $imageObj = $_FILES['original_file'];
            $x_coordinate = $_POST['advertisement_x'];
            $y_coordinate = $_POST['advertisement_y'];
            $advertisement_w = $_POST['advertisement_w'];
            $advertisement_h = $_POST['advertisement_h'];
            if ($imageObj['name'] && isset($x_coordinate) && isset($y_coordinate)) {
                $uploadedImage = cropAndUploadAdImage($imageObj, $x_coordinate, $y_coordinate, $advertisement_h);
            }
            
            /*if(isset($_POST['uploaded_image'])){
                //$uploadedImage = $_POST['uploaded_image'];
                $PostData['original_file'] = $_POST['uploaded_image'];
            }*/

            /* if(isset($_POST['sorting_order'])){
              $ad_sorting_order_value = '1';
              }else{
              $ad_sorting_order_value = '0';
              }
              ...
              '_op' => 'set_item_prop',
              '_session' => $sessionKey,
              'obj_id' => $_POST['ad_item_id'],
              'propname_1' => 'ad_sorting_order',
              'propval_1' => $ad_sorting_order_value,
              );
              $apiResponse = aibServiceRequest($apiRequestDataItem, 'browse'); */

            $PostData['_key'] = APIKEY;
            $PostData['_session'] = $sessionKey;
            $PostData['_user'] = '1';
            if (!empty($_POST['ad_update_id'])) {
                $PostData['_op'] = 'advert_update';
                $PostData['record_id'] = $_POST['ad_update_id'];
            } else {
                $PostData['_op'] = 'advert_store';
                $PostData['item_id'] = $_POST['ad_item_id'];
            }
            $PostData['title'] = $_POST['ad_title'];
            $PostData['url'] = $_POST['ad_url'];
            $PostData['sort_order'] = ($_POST['ad_sort_order']) ? $_POST['ad_sort_order'] : '1';
            $PostData['alt_title'] = $_POST['ad_alt_title'];
            $PostData['inherit'] = ($_POST['inherit_flag'] == '1') ? 'Y' : 'N';
            $PostData['record_ref'] = ($_POST['record_ref']) ? $_POST['record_ref'] : '-1';
            $PostData['disable'] = $_POST['disable_flag'];
            if (isset($uploadedImage)) {
                //$PostData['filename'] = AIB_AD_STORAGE_PATH . '/' . $uploadedImage;
                $filename = AIB_AD_STORAGE_PATH . '/' . $uploadedImage;
                //$filename = '/home/stparch/virtual_sites/aib_historicals/ad_uploads/' . $uploadedImage;
                $PostData['filename'] = $filename;
                if ($_POST['ad_update_id']) {
                    $advert_set_file = array(
                        "_key" => APIKEY,
                        "_session" => $sessionKey,
                        "_user" => "1",
                        "_op" => "advert_set_file",
                        "record_id" => $_POST['ad_update_id'],
                        "filename" => $filename,
                    );
                    aibServiceRequest($advert_set_file, "advertisement");
                }
            }
    ...
    case 'get_advertisement_references':
        $record_id = $_POST['record_id'];
        $response = array('status' => 'error', 'msg' => '', 'record_id' => '');
        if ($record_id) {
            $PostData = array(
            ...
            if($response['status'] == 'OK'){
                ...
                $responseTitle = aibServiceRequest($PostDataTitle, "advertisement");
                $response['data']['ad_title_name'] = $responseTitle['info']['records'][0]['ad_title'];
            
                $count1 = count($response['data']['records']);
                for($i = 0; $i < $count1; $i++){
                    $item_id = $response['data']['records'][$i]['item_id'];
                    if($item_id){
                        $PostData = array(
                            "_key" => APIKEY,
                            "_session" => $sessionKey,
                        ...
                        $presponse = aibServiceRequest($PostData, "browse");
                        if($presponse['status'] == 'OK'){
                            $count2 = count($presponse['info']['records']);
                            $item_title = [];
                            for($k = 0; $k < $count2; $k++){
                                $item_title[] = $presponse['info']['records'][$k]['item_title'];
                            }
                            $response['data']['records'][$i]['item_path'] = implode(' &#8594; ', $item_title);
                        }
                    }
                }
            }
    ...
    case 'get_advertisement_list':
        ...
        echo json_encode($response);
        break;
    case 'get_visible_advertisement_list':
        $item_id = $_POST['item_id'];
        $response = array('status' => 'error', 'msg' => '', 'record_id' => '');
        if ($item_id) {
            $PostData = array(
        ...
        echo json_encode($response);
        break;
    case 'get_ad_global_properties':
        $item_id = $_POST['item_id'];
        $response = array('status' => 'error', 'msg' => '');
        if ($item_id) {
            $response = getItemDetailsWithProp($item_id);
            $response = array(
                'status' => 'OK', 
                'ad_sorting_order' => $response['properties']['ad_sorting_order'],
                'ad_display_type' => $response['properties']['ad_display_type'],
                'ad_display_flip_time' => $response['properties']['ad_display_flip_time']
                );
        }
        echo json_encode($response);
        break;
    ...
    case 'get_advertisement_detail':
        ...
        if ($record_id) {
            ...
            $response = aibServiceRequest($PostData, "advertisement");
            if ($response['status'] == 'OK') {
                $response = $response['info']['records'][0];
            }
        }
        echo json_encode($response);
        break;
    case 'upload_croped_image':
        ...
        switch ($_REQUEST['type']) {
            ...
            case 'banner':
                ...
                $crop_height = isset($_POST['baner_crop_height']) ? $_POST['baner_crop_height'] : 340;
                ...
                break;
            case 'society_banner':
                $imageObj = $_FILES['society_archive_header_image'];
                $x_coordinate = $_POST['society_archive_header_x'];
                $y_coordinate = $_POST['society_archive_header_y'];
                $crop_height = isset($_POST['baner_crop_height']) ? $_POST['baner_crop_height'] : 600;
                $uploadedImage = cropAndUploadImage($imageObj, $x_coordinate, $y_coordinate, 'society_banner', $crop_height);
                $apiRequestDataItem['propname_1'] = 'society_archive_header_image';
                $apiRequestDataItem['propval_1'] = $uploadedImage;
                $response = array('status' => 'success', 'image' => IMAGE_TARGET_PATH . $uploadedImage, 'type' => 'society_banner');
                break;
            case 'content':
    ...
    case 'upload_default_home_image':
        $response = array('status' => 'error', 'image' => '');
        $apiRequestDataItem = array(
            '_key' => APIKEY,
            '_user' => 1,
        ...
        switch ($_REQUEST['type']) {
            case 'logo':
                $imageObj = $_FILES['home_logo_image'];
                $x_coordinate = $_POST['home_logo_x'];
                $y_coordinate = $_POST['home_logo_y'];
                $uploadedImage = cropAndUploadImage($imageObj, $x_coordinate, $y_coordinate, 'logo');
                $apiRequestDataItem['propname_1'] = 'default_home_logo';
                $apiRequestDataItem['propval_1'] = $uploadedImage;
                $response = array('status' => 'success', 'image' => IMAGE_TARGET_PATH . $uploadedImage, 'type' => 'logo');
                break;
            case 'banner':
                $imageObj = $_FILES['home_header_image'];
                $x_coordinate = $_POST['home_header_x'];
                $y_coordinate = $_POST['home_header_y'];
                $crop_height = isset($_POST['baner_crop_height']) ? $_POST['baner_crop_height'] : 340;
                $uploadedImage = cropAndUploadImage($imageObj, $x_coordinate, $y_coordinate, 'banner', $crop_height);
                $apiRequestDataItem['propname_1'] = 'default_home_header_image';
                $apiRequestDataItem['propval_1'] = $uploadedImage;
                $response = array('status' => 'success', 'image' => IMAGE_TARGET_PATH . $uploadedImage, 'type' => 'banner');
                break;
        }
    ...
    case 'forget_password_email':
        ...
        if ($apiResponse['status'] == 'OK') {
            ...
            if($apiResponse['info']['properties']['security_question_answers'] == 1)
            {
                ...
                {
                    $url = 'id=' . $apiResponse['info']['user_id'] . '&type=forget';
                    $link =  HOST_PATH . 'thank-you.php?' . urlencode($url);
            ...
            if (!empty($listDataArray)) {
                ...
                foreach ($listDataArray as $key => $dataArray) {
                    ...
                    if ($_SESSION['aib']['user_data']['user_top_folder'] > 1 and $_SESSION['aib']['user_data']['user_login'] != 'root' and $_SESSION['aib']['user_data']['user_type'] != 'R') {
                        $item_id_root = $item_id;
                        if ($request_data_type == 'RP') {
                            $item_id_root = $itemArchiveDetails[1]['item_id'];
                        }

                        if ($item_id_root != $_SESSION['aib']['user_data']['user_top_folder']) {
                            continue;
                        }
                    }
...
//            }
    ...
    case 'get_assistant_completed_record_list':
        ...
        if ($parent_id) {
			...
			$temp_array = [];
            if ($apiResponse['status'] == 'OK') {
				foreach($apiResponse['info']['records'] as $val){
					if($val['is_link'] == 'Y'){
						continue;
					}
					$temp_array[] = $val;
				}
                $listDataArray = $temp_array;//$apiResponse['info']['records'];
            }
        }
        if ($type == 'record') {
            $completeRequestData = array(
	...
	case 'send_verification_link_again':
        $user_id = $_POST['user_id'];
        if (!empty($user_id)) {
            $postData = array(
                "_key" => APIKEY,
			...
			if($apiResponse['status'] == 'OK'){
				...
				$email = sendMail($email_data, $email_template);
				if ($email) {
					$responseData = array('status' => 'success', 'message' => 'Verification link has been sent successfully.');
				} else {
					$responseData = array('status' => 'error', 'message' => 'Email could not be sent, please try again.');
				}
			}
        }else{
			$responseData = array('status' => 'error', 'message' => 'User id not found.');
		}
		echo json_encode($responseData);
        break;
		
	case 'change_email_validation_link':
		$user_id = $_POST['user_id'];
        if (!empty($user_id)) {
			$user_info_post_data = array(
				"_key" => APIKEY,
			...
			$user_info_resp = aibServiceRequest($user_info_post_data, 'users');
			if($user_info_resp['status'] == 'OK'){
				$new_email = urldecode($user_info_resp['info']['properties']['new_email']);
				$old_email = urldecode($user_info_resp['info']['properties']['email']);
				$email_template = file_get_contents(EMAIL_PATH . "/email_change.html");
				$email_data['to'] = $new_email;
				$email_data['from'] = ADMIN_EMAIL;
				$email_data['reply'] = ADMIN_EMAIL;
				$email_data['subject'] = 'ArchiveInABox � Change Email-Id';
				$email_template = str_replace('#username#', $user_info_resp['info']['user_login'], $email_template);
				$url = 'id=' . $user_id . '&flg=change_email&' . $new_email . '&' . $old_email;
				...
				// Fixed Start Issue ID 2465 03-Jul-2025
                // $email = sendMail($email_data, $email_template);
				// if ($email) {
				// 	$responseData = array('status' => 'success', 'message' => 'Verification link has been sent successfully.');
				// } else {
				...
				// }
                $responseData = array('status' => 'error', 'message' => 'Email could not be sent, please try again.');
                // Fixed end Issue ID 2465 03-Jul-2025
            }else{
				$responseData = array('status' => 'error', 'message' => 'Something went wrong.');
			}
        }else{
	...
	case 'full_validate_account':
        ...
        if ($archive_id) {
			...
			$postDataLogin = array(
				...
				"_op" => 'update_profile',
				"_user" => 1,
				"user_id" => $archiveUserId['properties']['archive_user_id'],
				"new_user_password" => DEFAULT_PASSWORD
			);
			...
			if ($apiResponse['status'] == 'OK') {
				...
				$userProperty[1]['value'] = 'Y';
				$userProperty[2]['name'] = 'occasional_update';
				$userProperty[2]['value'] = 'Y';
				...
				if ($userPropertyStatus['status'] == 'OK') {
					$response = array('status' => 'success', 'message' => 'Account has been activated successfully.');
				}
			}
        }else{
			$response = array('status' => 'error', 'message' => 'Archive id not found.');
		}
		print(json_encode($response));
        break;
    ...
    case 'update_archive_registration_details':
        ...
        if (isset($postData['archive_id']) && $postData['archive_id'] != '') {
            ...
            if (!empty($postData) && isset($postData['archive_user_id'])) {
                ...
                if ($apiResponseProfile['status'] == 'OK') {
					$ui = 0;
                    $userProperty[$ui]['name'] = 'timestamp';
                    $userProperty[$ui]['value'] = $postData['timestamp'];
					$ui++;
                    if (isset($postData['term_service']) && !empty($postData['term_service'])) {
                        $userProperty[$ui]['name'] = 'term_service';
                        $userProperty[$ui]['value'] = $postData['term_service'];
            ...
            if ($apiResponse['status'] == 'OK') {
                if ($email_condition) {
					$userProperty[0]['name'] = 'new_email';
					$userProperty[0]['value'] = $postData['register_emailId'];
					$postRequestData = array(
						"_key" => APIKEY,
						"_session" => $sessionKey,
						"_user" => 1,
    ...
    case 'get_archive_prop_data':
        ...
        $terms_condition = getSuperAdminAllProperty(1);
        if ($terms_condition['timestamp'] > $user_tiemstamp) {
            $userPropData['term_service'] = '';
        }
    ...
    case 'add_security_questions':
        ...
        $items = !empty($apiResponse['info']['records']) ? $apiResponse['info']['records'] : [];
        // var_dump($items);die;
        if(count($items) < 2){
            // Parse $_POST['formData'] in to $postData variable            
            parse_str($_POST['formData'], $postDataArray);
            $postDataItem = array(
                    ...
                    '_session' => $sessionKey,
                    'parent' => SECURITY_QUESTION_PARENT_ID,
                    'item_title' =>$postDataArray['question_1'],
                    'item_type' => 'rec',
            );
            $apiItemResponse = aibServiceRequest($postDataItem, 'browse');
            if ($apiItemResponse['status'] == 'OK') {
                $responseData = array('status' => 'success', 'message' => 'Security questions set');
            } else {
                $responseData = array('status' => 'error', 'message' => 'Not set security questions');
            }
        }else{
            $responseData = array('status' => 'error', 'message' => 'Two questions already added');
        }
        print json_encode($responseData);
        break;
    case 'get_security_questions':
        ...
        if ($apiResponse['status'] == 'OK') {
            $items = !empty($apiResponse['info']['records']) ? $apiResponse['info']['records'] : [];
            include_once TEMPLATE_PATH . 'security_question.php';
        } else {
            return false;
        }
        break;
    ...
    case 'update_security_question':
        ...
        if (isset($item_id) && $item_id != '') {
            ...
            $apiResponse = aibServiceRequest($postDataItem, 'browse');
            if ($apiResponse['status'] == 'OK') {
                $responseData = array('status' => 'success', 'message' => 'Security question updated!');
            } else {
                $msg = !empty($apiResponse['info'])?$apiResponse['info']:'security question not updated';
                $responseData = array('status' => 'error', 'message' => $msg);
            }
        }else{
            $responseData = array('status' => 'error', 'message' => 'Invalid request for security question update');
        }
        print json_encode($responseData);
        break;
    ...
    case 'remove_society_images':
        ...
        break;
        //  Fix start Issue Id 0002476 07-July-2025
    case 'remove_default_home_images':
        $property_name = filter_input(INPUT_POST, 'property_name', FILTER_SANITIZE_SPECIAL_CHARS);
        $responseArray = ['status' => 'error', 'message' => 'Missing required fields.'];
        if ($property_name) {
            $postData = array(
   ...
   case 'enable_disable_download':
        $download_status = filter_input(INPUT_POST, 'download_status', FILTER_DEFAULT, FILTER_SANITIZE_SPECIAL_CHARS);
         $obj_id = filter_input(INPUT_POST, 'item_id', FILTER_DEFAULT, FILTER_SANITIZE_SPECIAL_CHARS);
        if ($download_status!='' && $obj_id != '') {
            ...
            $responseArray = ['status' => 'error', 'message' => 'Some issue please try again'];
        	}
        ...
        print json_encode($responseArray);
        break;	
		/*<!-- End Fixed Issue ID 2138 02-Jan-2023  -->	*/
   ...
   case 'remove_location':
        ...
        if ( $obj_id != '') {
			...
			$postData = array(
                    ...
                    'propname_1' => 'itemrecord_address_line',
					'propname_2' => 'itemrecord_address_city',
				    'propname_3' => 'itemrecord_address_pin_code',
				    'propname_4' => 'itemrecord_address_state',
				    'propname_5' => 'itemrecord_lat',
                   'propname_6' => 'itemrecord_lng',
                );
                // Service request for set item property
			...
			$postData = array(
                ...
                // Service request for set item property
                $apiResponse = aibServiceRequest($postData, 'locationsearch');
		...
		case 'assistant_list_generate_report_deleted':
		$reoprt_file_path=AIB_USER_TMP.'user_list_report.csv';
		$output = fopen($reoprt_file_path,'w') or die("Can't open php://output");
        //Find the user top folder(Access level), If not present in request get from loggedin user
        $topFolderId = (isset($_POST['parent_id']) && $_POST['parent_id'] != '') ? $_POST['parent_id'] : $_SESSION['aib']['user_data']['user_top_folder'];
        $userId = $_SESSION['aib']['user_data']['user_id'];
        //Check for request type S: Assistant, A: Administrator, U: Public user
        switch ($_POST['type']) {
            ...
            case 'A':
				...
				$valueCSV[] = 'ID';
                        $valueCSV[] = "Archive Group";
                        $valueCSV[] = "User Type";
                        $valueCSV[] = "Name";
                        $valueCSV[] = 'EmailId';
						$valueCSV[] = 'Phone Number';
						$valueCSV[] = 'Physical Address Line1';
						$valueCSV[] = 'Physical Address Line2';
						$valueCSV[] = 'Physical City';
						$valueCSV[] = 'Physical State';
						$valueCSV[] = 'Physical Zip';
						$valueCSV[] = "Mailing Address Line1";
						$valueCSV[] = "Mailing Address Line2";
						$valueCSV[] = 'Mailing City';
						$valueCSV[] = 'Mailing State';
						$valueCSV[] = 'Mailing Zip';
                       
						 fputcsv($output, $valueCSV);
                // Service request for get Administrator list data
                $apiResponse = aibServiceRequest($apiGetData, 'users');
                $responseData = $apiResponse['info']['records'];
                // Get the Archive Group data
                ...
                foreach ($responseData as $key => $value) {
                    ...
                    if (json_decode($archiveData['properties']['temp_claim_data_' . $value['user_login']]) != '') {
                        ...
                        $arr = json_decode($archiveData['properties']['temp_claim_data_' . $value['user_login']], true);
                        $found_key = array_search('society_name', array_column($arr, 'name'));
                        if (array_search('register_emailId', array_column($arr, 'name'))) {
                            $register_emailId = $arr[array_search('register_emailId', array_column($arr, 'name'))]['value'];
                        }
						if (array_search('phoneNumber', array_column($arr, 'name'))) {
                            $phoneNumber = $arr[array_search('phoneNumber', array_column($arr, 'name'))]['value'];
                        }
						if (array_search('physicalAddressLine1', array_column($arr, 'name'))) {
                            $physicalAddressLine1 = $arr[array_search('physicalAddressLine1', array_column($arr, 'name'))]['value'];
                        }
						if (array_search('physicalAddressLine1', array_column($arr, 'name'))) {
                            $physicalAddressLine2 = $arr[array_search('physicalAddressLine1', array_column($arr, 'name'))]['value'];
                        }
						if (array_search('physicalCity', array_column($arr, 'name'))) {
                            $physicalCity = $arr[array_search('physicalCity', array_column($arr, 'name'))]['value'];
                        }
						if (array_search('physicalState', array_column($arr, 'name'))) {
                            $physicalState = $arr[array_search('physicalState', array_column($arr, 'name'))]['value'];
                        }
						if (array_search('physicalZip', array_column($arr, 'name'))) {
                            $physicalZip = $arr[array_search('physicalZip', array_column($arr, 'name'))]['value'];
                        }
						if (array_search('mailingAddressLine1', array_column($arr, 'name'))) {
                            $mailingAddressLine1 = $arr[array_search('mailingAddressLine1', array_column($arr, 'name'))]['value'];
                        }
						if (array_search('mailingAddressLine2', array_column($arr, 'name'))) {
                            $mailingAddressLine2 = $arr[array_search('mailingAddressLine2', array_column($arr, 'name'))]['value'];
                        }
						if (array_search('mailingCity', array_column($arr, 'name'))) {
                            $mailingCity = $arr[array_search('mailingCity', array_column($arr, 'name'))]['value'];
                        }
						if (array_search('mailingState', array_column($arr, 'name'))) {
                            $mailingState = $arr[array_search('mailingState', array_column($arr, 'name'))]['value'];
                        }
						if (array_search('mailingZip', array_column($arr, 'name'))) {
                            $mailingZip = $arr[array_search('mailingZip', array_column($arr, 'name'))]['value'];
                        }
					 ...
					 $valueCSV=array();	
                    if (!empty($archiveData)) {
                        $valueCSV['item_id'] = $archiveData['item_id'];
                        $valueCSV['item_title'] = $archiveData['item_title'];
                        $valueCSV['user_pro_type'] = $value['_properties']['type'];
                        $valueCSV['username'] = $value['user_login'];
                        $valueCSV['register_emailId'] = $register_emailId;
						$valueCSV['phoneNumber'] = $phoneNumber;
						$valueCSV['physicalAddressLine1'] = $physicalAddressLine1;
						$valueCSV['physicalAddressLine2'] = $physicalAddressLine2;
						$valueCSV['physicalCity'] = $physicalCity;
						$valueCSV['physicalState'] = $physicalState;
						$valueCSV['physicalZip'] = $physicalZip;
						$valueCSV['mailingAddressLine1'] = $mailingAddressLine1;
						$valueCSV['mailingAddressLine2'] = $mailingAddressLine2;
						$valueCSV['mailingCity'] = $mailingCity;
						$valueCSV['mailingState'] = $mailingState;
						 $valueCSV['mailingZip'] = $mailingZip;
                        $userListWithArchiveGroup[] = $value;
						 fputcsv($output, $valueCSV);
                    }
                }
            ...
            case 'U':
                ...
                foreach ($responseData as $key => $value) {
                   $csvData=array();
					$csvData[]=$value['user_id'];
					$csvData[]=$value['user_login'];
					//$csvData[]=$value['user_title'];
					$csvData[]=$value["_properties"]['firstName'];
					$csvData[]=$value["_properties"]['lastName'];
					$csvData[]=$value["_properties"]['email'];
					$csvData[]=$value["_properties"]['phoneNumber'];
					$csvData[]=$value["_properties"]['mallingAddress'];
					$csvData[]=$value["_properties"]['mailingCity'];
					$csvData[]=$value["_properties"]['mailingState'];
					$csvData[]=$value["_properties"]['mailingZip'];
					$csvData[]=$value["_properties"]['country'];
					//$userData=getUserProfileById($value['user_id']);
                   //$userListWithArchiveGroup1=array();
                    //$archiveData = getItemData($value['user_top_folder']);
					//$userListWithArchiveGroup1['user_id']=$value['user_id'];
                   //$userListWithArchiveGroup1['value']=$value;
					//$userListWithArchiveGroup1['userData']=$userData;
					//$userListWithArchiveGroup1['archiveData']=$archiveData;
					//$userListWithArchiveGroup[]=$userListWithArchiveGroup1;
					 fputcsv($output, $csvData);
                }
				fclose($output) or die("Can't close php://output");
                print 'suucess';json_encode($userListWithArchiveGroup);
                break;
        }
        break;
...
function cropAndUploadImage($imageObj, $x_coordinate, $y_coordinate, $type, $crop_height = '') {
    ...
    $dimensionArray = array('logo' => array('width' => 200, 'height' => 200), 'banner' => array('width' => 1600, 'height' => $crop_height), 'content' => array('width' => 600, 'height' => 400), 'archive_group_thumb' => array('width' => 400, 'height' => 400), 'archive_group_details_thumb' => array('width' => 400, 'height' => 400), 'historical_connection_logo' => array('width' => 200, 'height' => 200));
...
function cropAndUploadAdImage($imageObj, $x_coordinate, $y_coordinate, $crop_height) {
    // Set image height, width according to type.
    $dimensionArray = array('width' => 300, 'height' => 250);
    ...
    $fileName = str_replace(' ', '_', $fileName);
    $fileName = preg_replace('/[^A-Za-z0-9\-_.]/', '', $fileName);
    $fileNameWithPath = AIB_AD_STORAGE_PATH . '/' . $fileName;
    //$fileNameWithPath2 = '/home/stparch/virtual_sites/aib_historicals/ad_uploads/' . $fileName;
    // Create object of imegic class with original file.
    $image = new Imagick($inFile);
    // Crop the original image with given co-ordinates and mentioned height, width
    $image->cropImage($dimensionArray['width'], $dimensionArray['height'], $x_coordinate, $y_coordinate);
    // Saved the cropped image.
    $image->writeImage($fileNameWithPath);
    //$image->writeImage($fileNameWithPath2);
    return $fileName;
}
    ...
    if ($parent_id) {
        ...
        foreach ($uncompleteResponse['info']['records'] as $key => $uncompleteData) {
            if ($parent_id == $uncompleteData['item_parent_id']) {
                $recordItems = getAllItemRecords($uncompleteData['item_id']);
				$recordItems2 = [];
				foreach($recordItems as $key => $value){
					if($value['is_link'] == 'Y'){
						continue;
					}
					$recordItems2[] = $value;
				}
                $uncompleteData['item_records'] = $recordItems2;
                    ...
                    foreach ($apiResponse['info']['records'] as $key => $dataArray) {
                        ...
                        foreach ($dataArray["files"] as $FileRecord) {
                            ...
                            if ($FileRecord["file_type"] == 'tn') {
                                $ThumbID = $FileRecord["file_id"];
                                $itemDetails['files_records'][$key]['tn_file_id'] = $FileRecord["file_id"];
								$itemDetails['files_records'][$key]['item_id'] = $dataArray["item_id"];
								$itemDetails['files_records'][$key]['item_parent'] = $dataArray["item_parent"];
                                continue;
                            }
...
//Fix start for Issue ID 2140 on 23-Feb-2023
function getItemProp($item_id = null) {
    if ($item_id) {
        $sessionKey = $_SESSION['aib']['session_key'];
        // Create request for get item property
        $apiRequestData = array(
        ...
        // Service request for list item property
        return aibServiceRequest($apiRequestData, 'browse');
    } else {
        return false;
    }
}

function setItemProp($item_id = null, $propertyArr) {
    if ($item_id) {
        $sessionKey = $_SESSION['aib']['session_key'];
        // Create request for get item property
        $apiRequestData = array(
        ...
        $apiRequestData = array_merge($apiRequestData, $propertyArr);
        // Service request for list item property
        return aibServiceRequest($apiRequestData, 'browse');
    } else {
        return false;
...
function getPath($item_id){
    ...
    // Service request to get item tree data        
    return aibServiceRequest($postData, 'browse');
}
//Fix end for Issue ID 2140 on 23-Feb-2023

//Fix start for Issue ID 2140 on 27-Feb-2023
function getTotalNumberOfChildInTheItem($item_id){
    $sessionKey = $_SESSION['aib']['session_key'];
    $postData = array(
		"_key" => APIKEY,
		"_session" => $sessionKey,
	...
	if (!empty($apiResponse) && $apiResponse['status'] == 'OK') {
		foreach ($apiResponse['info']['records'] as $k1 => $v1) {
			...
			getTotalNumberOfChildInTheItem($item_id);
		}
	}
...
//Fix end for Issue ID 2140 on 27-Feb-2023
...
//        $dataArray['login_data'] = (strpos($dataArray['login_data'], $suffix.'_'.$archiveCode) !== false) ? $dataArray['login_data']  : $dataArray['login_data'].'_'.$suffix.'_'.$archiveCode;
        ...
        if(!empty($dataArray['user_password']))
        {
            $defaultPassword = $dataArray['user_password'];
        }else{
            $defaultPassword = 'AIB';
        }
        // Prepared mail data
		...
		/******* Fix Start Issue Id 0002414  04-Feb-2025 **********/
		 if ($userType == 'A') {
			 
			 $email_data['subject'] = 'ArchiveInABox -- New Admin Account Created';
		 }
		/******* Fix End Issue Id 0002414  04-Feb-2025 **********/
        if ($userType == 'S') {
            $title = 'assistant';
        ...
        if ($apiResponse['status'] == 'OK') {
            ...
            $userProperty[4]['name'] = 'term_service';
            $userProperty[4]['value'] = 'Y';
            // Create user profile property
            $postRequestData = array(
                "_key" => APIKEY,
                "_session" => $sessionKey,
        ...
        if ($apiResponse['status'] == 'OK') {
            foreach ($apiResponse['info']['records'] as $record) {
                if ($record['item_type'] == 'RE' || $record['item_type'] == 'IT') {
                    $itemId[] = $record['item_id'];
                    if ($record['item_type'] == 'RE') {
                        $only_item = getItemChildToOCR2($record['item_id']);
                        foreach ($only_item as $it) {
                            $itemId[] = $it;
                        }
                    }
                }
...
function getItemChildToOCR2($item_id = null) {
    if ($item_id) {
        $itemId = [];
        //$itemId[] = $item_id;
        $sessionKey = $_SESSION['aib']['session_key'];
