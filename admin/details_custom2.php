...
include_once COMMON_TEMPLATE_PATH . 'header_custom2.php';
...
<?php
include_once COMMON_TEMPLATE_PATH . 'footer_custom2.php';
$backURLSearch = '';
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (result.status == 'success') {
                        ...
                        showPopupMessage('error', result.message + ' (Error Code: 392)');
                    }
                },
                error: function () {
                    showPopupMessage('error', 'Previous request not completed. (Error Code: 393)');
                }
            });
        }
    });
...
//$('#details-back-button').click(function(){
    ...
    function ItemPageNumberSet(page, pagefolder) {
        ...
        $.ajax({
            ...
            error: function () {
                showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 394)');
            }
        });
    }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (result.status == 'success') {
                        ...
                        showPopupMessage('error', result.message + ' (Error Code: 395)');
                    }
                },
                error: function () {
                    showPopupMessage('error', 'Previous request not completed. (Error Code: 396)');
                }
            });
        }
    });
...
//            });
            $.ajax({
                ...
                error: function () {
                    showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 397)');
                }
            });
        }
        ...
        if (current_value == 1) {
            ...
            if (itemId != '' && $("#thumb-list-" + row_val).length == 0) {
                ...
                if (parseInt($('#start').val()) > 0) {
                    var start_val = parseInt($('#start').val()) - 5;
                    $('#start').val(start_val);
                }
...
//    var total_thumb_page = $('.sapple-slider-thumb').length;
        ...
        if (current_value == total_item_page && $("#thumb-list-" + row_val).length == 0) {
            ...
            var detailPagecontent = 'Y';
            var action = 'Next';
            if (total_items > total_item_page * 5) {
                getRecordTreedata(folder_id, itemId, share, flg, previousId, ref, detailPagecontent, pageNumber, pagefolder, start, total_item_page, action, '', '', '', scrapbook_item);
            }
        } else {
            current_value = parseInt(parseInt(current_value) + 1);
...
//        var thumb_url = '<?php echo THUMB_URL; ?>';
        ...
        if (current_active_thumb_count != maximum_image_count) {
            ...
            if ($("#sapple-slider-thumb-item-" + parseInt(current_active_thumb_count + 1)).length == 0) {
                ...
                if (parentCount < currentParentId) {
                    ...
                    var strat_set_val = (parseInt(currentParentId) - 1) * 5;
        ...
        if (current_active_thumb_count != 0) {
            ...
            if ($("#sapple-slider-thumb-item-" + parseInt(current_active_thumb_count - 1)).length == 0) {
                ...
                if (parentCount > currentParentId) {
                    ...
                    var strat_set_val = (parseInt(parentCount) - 1) * 5;
                    $('#start').val(strat_set_val);
                    if (parseInt($('#start').val()) > 0) {
                        var start_val = parseInt($('#start').val()) - 5;
                        $('#start').val(start_val);
    ...
    function getRecordTreedata(folder_id, itemId, share, flg, previousId, ref, detailPagecontent, pageNumber = '', pagefolder = '', start = '', total_item_page = '', action = '', imgFlg = '', searchString = '', recordIdsArr = '', scrapbook_item = '') {
        ...
        $.ajax({
            ...
            error: function () {
                $('.loading-div').hide();
                showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 398)');
            }
        });
    }
    ...
    function getRecordIds(id) {
        $.ajax({
            ...
            error: function () {
                showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 399)');
            }
        });
    }
    function imageLoad() {
    ...
    function fullImageWithHighLight(pr_file_id, item_id, height, searchString) {
        $.ajax({
            ...
            error: function () {
                showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 431)');
            }
        });
    }
</script>
