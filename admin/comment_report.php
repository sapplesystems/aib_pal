...
<div class="content-wrapper">
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active">Content Report</li>
        </ol>
        <h4 class="list_title">Manage Reported Comment </h4>
    </section>
    <section class="content bgTexture">
        ...
        <div class="row" id="dataTableDiv">
            <div class="col-md-12 tableStyle">
                ...
                <div class="archive-list custom-dropdown marginLeft50 archive_list_data" style="visibility: hidden;">
			    ...
			    <th width="15%" class="text-center">Reporting Reason</th>
...
<script type="text/javascript">
        ...
        $.ajax({
			...
			var delButton = '';
                    ...
                    {  
                        ...
                        var delButton = '<span class="delete_request_data" data-form-request-id="'+ record[i].item_link +'" data-form-comment-id="'+ record[i].comment_id +'" request-id='+ record[i].req_id +'><img src="'+image_path+'delete_icon.png" alt="" /></span>';
                    }
			...
			record[i].reporting_reason_comment,
                        record[i].created,
                        record[i].comment, 
    ...
    $(document).on('click','.delete_request_data', function(){
        if(confirm('Are you sure to delete ? This cannot be undone')){
            ...
            var comment_id = $(this).attr('data-form-comment-id');  
   ...
   $(document).on('change','#select_dropdown',function(){
    ...
    function deleteFolderItem(item_id){  
        $.ajax({
           type: 'POST',
           url: 'services_admin_api.php',
           data: {mode:'unlink_share_link',obj_id:item_id},
           success: function (response) {
               $('.admin-loading-image').hide();
               var record = JSON.parse(response);
               if(record.status == 'success'){ 
