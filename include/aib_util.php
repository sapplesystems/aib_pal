...
function aib_generate_tree_nav_child_edit($DBHandle,$AvoidSet,$UserRecord,$InitialParent,$ClickFunction,$ULClass = false,$LIClass = false,$ArchiveClass = false,$CollectionClass = false,$SubGroupClass = false,$ItemRefMap = false,$ExpandMap = false)
{
	...
	foreach($ChildList as $ItemRecord)
	{
		...
		$FolderType = ftree_get_property($DBHandle,$TempID,AIB_FOLDER_PROPERTY_FOLDER_TYPE);
		$SubSize = ftree_list_child_objects($DBHandle,$TempID,false,false,FTREE_OBJECT_TYPE_FOLDER,true);
		if ($SubSize < 1)
		{
			$ImageString = " style='list-style-image: url(\"/images/button-nochild.png\");' ";
			$SpanOn = "<span style='position:relative; left:-0.5em;'>";
...
function aib_generate_tree_nav_div($DBHandle,$UserID,$InitialParent,$ClickFunction,$ULClass = false,$LIClass = false,$ArchiveClass = false,$CollectionClass = false,$SubGroupClass = false, $ItemID = false)
{
	...
	$SpanOn = "<span style='position:relative; left:-1.0em;'>";
	$SpanOff = "</span>";
	while(true)
	{
		...
		$IconString = " style='list-style-image: url(\"/images/button-nochild.png\");'";
		if ($SubSize > 0)
		{
			if ($ArchiveClass != false)
			{
//				$OutLines[] = "<li id='$IDName' onclick=\"$ClickFunction(event,this,$TempID);\" class='$ArchiveClass'>".aib_urldecode($ItemRecord["item_title"]);
				$OutLines[] = "<li id='$IDName' class='$ArchiveClass'>".$SpanOn.aib_urldecode($ItemRecord["item_title"]).$SpanOff;
			}
			else
			{
//				$OutLines[] = "<li id='$IDName' onclick=\"$ClickFunction(event,this,$TempID);\">".aib_urldecode($ItemRecord["item_title"]);
				$OutLines[] = "<li id='$IDName'>".$SpanOn.aib_urldecode($ItemRecord["item_title"]).$SpanOff;
			}
		...
		else
		{
			if ($ArchiveClass != false)
			{
				$OutLines[] = "<li id='$IDName' class='$ArchiveClass' $IconString>".$SpanOn.aib_urldecode($ItemRecord["item_title"]).$SpanOff;
			}
			else
			{
				$OutLines[] = "<li id='$IDName' $IconString>".$SpanOn.aib_urldecode($ItemRecord["item_title"]).$SpanOff;
			}
...
function aib_store_uploaded_pdf($DBHandle,$BatchRecord,$SourceFile,$FileQueueID,$ItemID,$TargetStorageArea = false,$VerboseFlag = false,$GenerateThumbFlag = false,$UserID = -1, $GroupID = -1,$OptionData = false)
{
	...
	if ($PageCount == 0 || $PageCount === false)
	{
		$LocalMsg = "";
		if (file_exists($SourceFile) == false)
		{
			$LocalMsg = "; File doesn't exist";
		}
		else
		{
			$LocalMsg = "; File is ".sprintf("%d",filesize($SourceFile))." bytes";
		}

		$NewItemErrors[] = array("status" => "ERROR", "error" => "Cannot get page count in PDF for '$SourceFile' $LocalMsg", "info" => $FileInfo, "file" => $BatchRecord);
		return($NewItemErrors);
	}
	...
	while($RetryCount > 0)
	{
		...
		if (file_exists($OutputOriginalName) == false)
		{
			...
			sleep(1);
			continue;
//			$NewItemErrors[] = array("status" => "ERROR", "error" => "Cannot store original PDF file to $OutputOriginalName", "info" => $FileInfo, "file" => $BatchRecord);
//			return($NewItemErrors);
		}
...
// Store PDF file; accounts for multiple pages
// -------------------------------------------
function aib_store_uploaded_msft($DBHandle,$BatchRecord,$SourceFile,$FileQueueID,$ItemID,$TargetStorageArea = false,$VerboseFlag = false,$GenerateThumbFlag = false,$UserID = -1, $GroupID = -1,$OptionData = false)
{
	$UseAltTitle = false;
	...
	$SourceSeg = explode("/",$SourceFile);
	$OriginalName = array_pop($SourceSeg);
	$LocalMimeType = "application/binary";

	preg_match("/[\.][^\n]+$/",$OriginalName,$ExtMatchSet);
	switch(strtolower($ExtMatchSet[0]))
	{
		case ".xls":
			$LocalMimeType = "application/vnd.ms-excel";
			break;
		...
		case ".doc":
			$LocalMimeType = "application/msword";
			break;

		case ".docx":
			$LocalMimeType = "application/vnd.openxmlformats-officedocument.wordprocessingml.document";
			break;

		case ".ppt":
			$LocalMimeType = "application/vnd.ms-powerpoint";
			break;

		case ".pptx":
			$LocalMimeType = "application/vnd.openxmlformats-officedocument.presentationml.presentation";
			break;
...
//			return($NewItemErrors);
	...
	if ($RetryCount <= 0)
	{
		$NewItemErrors[] = array("status" => "ERROR", "error" => "Cannot store original file to $OutputOriginalName", "info" => array(), "file" => $BatchRecord);
		return($NewItemErrors);
	}

	// Associate original file as an AUX attached to the item.  Do not generate a thumbnail.

	$FileInfo = array("batch_id" => $BatchID, "original_name" => $OriginalName, "storage_name" => $OriginalNameInfo["stored_path"],
		"mime_type" => $LocalMimeType, "processing_info" => "src_bch=".$BatchRecord["file_batch"],"storage_size" => $SourceFileSize);
	$FileResult = aib_add_file_to_item($DBHandle,$ParentItemID,$StorageAreaID,AIB_FILE_CLASS_AUX,$FileInfo);
	if ($FileResult["id"] === false)
	{
		$NewItemErrors[] = array("status" => "ERROR", "error" => "Cannot associate original format file with item", "info" => $FileInfo, "file" => $BatchRecord);
		return($NewItemErrors);
	}

	// Set the MIME type based on the filename extension
	
	aib_store_info_file($OriginalNameInfo,$OriginalName,$LocalMimeType);
	preg_match("/[\.][^\n]+$/",$OriginalName,$ExtMatchSet);

	// Convert to a PDF in the same path as the original, then store the PDF

	if (substr($SourceFile,0,1) == "/")
	{
		$DestPath = "/".join("/",$SourceSeg);
	}
	else
	{
		$DestPath = join("/",$SourceSeg);
	...
	if (substr($DestPath,0,2) == "//")
	{
		$DestPath = substr($DestPath,1);
	}

	$BaseOutName = preg_replace("/[\.][^\n]+$/",".pdf",$OriginalName);
	$PDFFileName = $DestPath."/".$BaseOutName;

	// Convert using LibreOffice, which creates the converted file with the same name in the target path.
	
	system("soffice --headless --convert-to pdf --outdir \"".$DestPath."\" \"".$SourceFile."\" 2>> /home/stparch/aib_utilities/soffice_log.txt >> /home/stparch/aib_utilities/soffice_log.txt");

	if (file_exists($PDFFileName) == false)
	{
		$NewItemErrors[] = array("status" => "ERROR", "error" => "Cannot move generated PDF from $TempOutputName to $PDFFileName", "info" => $FileInfo, "file" => $BatchRecord);
		return($NewItemErrors);
	}

	// Store the PDF as if it was an uploaded PDF
	
	return(aib_store_uploaded_pdf($DBHandle,$BatchRecord,$PDFFileName,$FileQueueID,$ItemID,$TargetStorageArea,$VerboseFlag,$GenerateThumbFlag,$UserID,$GroupID,$OptionData));
}
...
function aib_store_uploaded_file($DBHandle,$FileQueueID,$ItemID,$TargetStorageArea = false,$VerboseFlag = false,$GenerateThumbFlag = false,$UserID = -1, $GroupID = -1, $DeleteFlag = true)
{
	...
	if (preg_match("/[\.][Dd][Oo][Cc]$/", $SourceFileName) != false ||
		preg_match("/[\.][Dd][Oo][Cc][Xx]$/",$SourceFileName) != false ||
		preg_match("/[\.][Xx][Ll][Ss]$/",$SourceFileName) != false ||
		preg_match("/[\.][Xx][Ll][Ss][Xx]$/",$SourceFileName) != false ||
		preg_match("/[\.][Pp][Pp][Tt]$/",$SourceFileName) != false ||
		preg_match("/[\.][Pp][Pp][Tt][Xx]$/",$SourceFileName) != false)
	{
		return(aib_store_uploaded_msft($DBHandle,$BatchRecord,$SourceFileName,$FileQueueID,$ItemID,$TargetStorageArea,$VerboseFlag,$GenerateThumbFlag,$UserID,$GroupID));
	}
