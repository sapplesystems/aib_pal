...
define('LLM_QUERY_TIMEOUT',18000);
...
// Request LLM search
// ------------------
function hsearch_query_llm($DBHandle,$Options)
{
	...
	$RequestText = base64_encode($Options["chat_request"]);

	$LocalRequestText = "'".$RequestText."'";
	$Query = "INSERT INTO llm_request (index_id,status,query_text,output_text) VALUES ('$ArchiveID','Q',$LocalRequestText,'');";
	...
	$QueryID = mysqli_insert_id($DBHandle);
	return($QueryID);
}

// Get updated LLM query response
// ------------------------------
function hsearch_get_llm_output($DBHandle,$RequestID)
{
	// Find the specific request

	$Query = "SELECT * FROM llm_request WHERE request_id='$RequestID';";
	$Result = mysqli_query($DBHandle,$Query);
	if ($Result == false)
	{
		return(false);
	...
	// If complete, set output status to "done", and delete the request from the table
	
	if ($Row["status"] == 'C')
	{
		$Query = "DELETE FROM llm_request WHERE request_id='$RequestID';";
	...
	else
	{
		if ($Row["status"] == "A")
		{
			$OutData["status"] = 'working';
		}
		else
		{
			$OutData["status"] = "waiting";
		}
	}

	// Return query and current output text
	
	$OutData["query_text"] = base64_decode($Row["query_text"]);
	$OutData["output_text"] = base64_decode($Row["output_text"]);
	return($OutData);
}

// Submit request to LLM
// ---------------------
function hsearch_query_llm_old($Options)
{
	if (isset($Options["chat_request"]) == false)
	{
		return("");
...
log_debug("TIMEOUT ON LLM QUERY");
	return("");
}

// Request currently available output from LLM for a given search request.
// -----------------------------------------------------------------------
...
{
	$OutArray = array(
		"<llm_request_id>$InBuffer</llm_request_id>",
		array(),
	);
...
// --------------------------
function hsearch_llm_response_to_xml_old($InBuffer,$Options)
{
	$LineSet = explode("\n",$InBuffer);
	$Mode = "prefix";
	$TempList = array();
...
log_debug("Ending doc; added vector doc to list");
			$DocData = array();
			continue;
		}
		...
		if (preg_match("/^[\[][\[]END CONTENT[\]][\]]/",$LocalLine) != false)
		{
			...
			$DocData["text"] = preg_replace("/[\<]/","&lt;",$DocData["text"]);
			$DocData["text"] = preg_replace("/[\>]/","&gt;",$DocData["text"]);
			$DocData["text"] = preg_replace("/[\&]/","&amp;",$DocData["text"]);
			$DocData["text"] = preg_replace("/[\']/","&apos;",$DocData["text"]);
			$DocData["text"] = preg_replace("/[\"]/","&quot;",$DocData["text"]);
...
log_debug("Set data is ".var_export($SetData,true));
	...
	if (isset($SetData["chat_result"]))
	{
		if (trim($SetData["chat_result"]) != "")
		{
			...
			$LocalResultsArray[] = "\t\t<matchwords>";
			$LocalResultsArray[] = "\t\t<word></word>";
			$LocalResultsArray[] = "\t\t</matchwords>";
	...
	foreach($SetData["doclist"] as $Doc)
	{
		...
		$LocalResultsArray[] = "\t\t<title>$LocalNode</title>";
...
// ----------------------
function hsearch_perform_search($DBHandle,$Options)
{
...
log_debug("Performing search using ".var_export($Options,true));
	...
	$StartLLMTime = time();
log_debug("Send query to LLM");
	$LLMBuffer = hsearch_query_llm($DBHandle,$Options);
	$EndLLMTime = time();
...
log_debug("estsearchutil result = \n$Result");
	if ($Result == false)
	{
		...
		if ($LLMBuffer != false)
		{
			...
			$FinalLines[] = "<resultset>\n\t<search_spec>\n\t\t<phrase>".$Options["chat_request"]."</phrase>";
			$FinalLines[] = "\t\t<attribute></attribute>";
			$FinalLines[] = "\t\t<title></title>";
			...
			$FinalLines[] = "\t<phrase>".$Options["chat_request"]."</phrase>";
			$FinalLines[] = "\t<proctime>$DeltaLLMTime</proctime>";
			$FinalLines[] = "\t<hints>\n\t</hints>";
			$FinalLines[] = "\t<related_terms>";
			$FinalLines[] = "\t</related_terms>";
			$FinalLines[] = $LLMXML;
...
log_debug("Final buffer =\n$FinalBuffer");
			$OutData = array("status" => "OK", "data" => $FinalBuffer);
		}
		else
		{
			if ($CurlObj != false)
			...
			{
				$OutData = array("status" => "ERROR", "info" => $Options["base_url"]."/cgi-bin/estsearchutil","data" => false);
			}
		}
	}
	else
