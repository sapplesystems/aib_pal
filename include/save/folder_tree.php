...
function ftree_count_child_object_property($DBHandle,$Parent,$UserID,$GroupID,$ItemType = false,$PropertyName)
{
	if ($ItemType != false)
	{
		$ResultSet = mysqli_query($DBHandle,"SELECT item_id FROM ftree WHERE item_parent=$Parent AND item_type='$ItemType' ORDER BY item_type,item_title;");
	}
	else
	{
		$ResultSet = mysqli_query($DBHandle,"SELECT item_id FROM ftree WHERE item_parent=$Parent ORDER BY item_type,item_title;");
	}
...
function ftree_replace_prop($DBHandle,$ItemID,$PropertyName,$PropertyValue,$LongFlag = "N", $DeleteAltFlag = "Y")
{
	if ($LongFlag == "Y")
	{
		$Query = "REPLACE INTO ftree_long_prop (item_id,property_name,property_value) VALUES ('$ItemID','$PropertyName','$PropertyValue');";
	}
	else
	{
		$Query = "REPLACE INTO ftree_prop (item_id,property_name,property_value) VALUES ('$ItemID','$PropertyName','$PropertyValue');";
	}
