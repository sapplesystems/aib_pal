...
// -------------------------
function ftree_field_urldecode($InString)
{
...
function ftree_field_get_item_fields($DBHandle,$ItemID)
{
	...
	if ($RecordList != false)
	{
		...
		{
			$OutData[$FieldRecord["field_id"]] = ftree_field_urldecode($FieldRecord["field_value"]);
		}
	}
...
{
	$OutData = array();
	$RecordList = ftree_field_query_ext($DBHandle,"SELECT * FROM field_data WHERE item_id=$ItemID ORDER BY record_id;");
	if ($RecordList != false)
	{
		foreach($RecordList as $FieldRecord)
		{
			$FieldDef = ftree_field_get_field($DBHandle,$FieldRecord["field_id"]);
			$OutData[$FieldRecord["field_id"]] = array("value" => ftree_field_urldecode($FieldRecord["field_value"]), "def" => $FieldDef);
		}
	}

	$RecordList = ftree_field_query_ext($DBHandle,"SELECT * FROM field_longdata WHERE item_id=$ItemID ORDER BY record_id;");
	if ($RecordList != false)
	{
		foreach($RecordList as $FieldRecord)
		{
...
function ftree_field_export_to_search($DBHandle,$BasePath,$BaseURI,$ItemList,$ArchiveInfo = false, $TypePropertyName = false, $AllowedTypes = false, $AllowNoType = false,$SkipLinks = true, $SearchAllowName = "searchindex")
{
	...
	foreach($ItemList as $ItemRecord)
	{
		...
		{
			$ArchiveInfo = ftree_get_archive_and_archive_group($DBHandle,$ItemID,true);
			if ($ArchiveInfo == false)
			{
				continue;
			}
		...
		if (isset($ItemProperties[AIB_FOLDER_PROPERTY_PRIVATE]) == true)
		{
			$PrivacySetting = $ItemProperties[AIB_FOLDER_PROPERTY_PRIVATE];
		}
		else
		{
			$PrivacySetting = "N";
...
/*
		if ($TypePropertyName !== false && $AllowedTypes !== false)
		{
			...
			{
				if (ftree_field_urldecode($LocalName) == $TypePropertyName)
				{
					$LocalType = ftree_field_urldecode($LocalValue);
					if (isset($AllowedTypes[ftree_field_urldecode($LocalValue)]) == true)
					{
						$AllowedItem = true;
						break;
					}
...
*/
		// If there is a set of allowed property values and a property name field set, check item to
		// make sure it can be used in the stats

		if ($TypePropertyName != false && $AllowedTypes != false)
		{
			$AllowedItem = false;

			// See if the property containing the type is available

			if (isset($ItemProperties[$TypePropertyName]) == true)
			{
				// Get value of the property

				$LocalValue = ftree_field_urldecode($ItemProperties[$TypePropertyName]);

				// If the property value is not allowed, skip item

				if (isset($AllowedTypes[ftree_field_urldecode($LocalValue)]) == false)
				{
					continue;
				}
			}
			...
			{
				// No type available.  If we aren't processing items
				// without a type, skip item.
		...
		// If this is an STPA edition, then set flag so we can do pages

		if (isset($ItemProperties["aib:sort_name"]) == true)
		{
			$ItemSourceInfo = json_decode(urldecode($ItemRecord["item_source_info"]));
		}
		else
		{
			$ItemSourceInfo = false;
		}
...
//		$URIValue = ftree_field_do_hard_subs($BaseURI,$ItemID,$Counter,$TimeSegs[0],$TimeSegs[1],$ItemProperties);
		...
		$TitlePathList = ftree_get_item_title_path($DBHandle,$ItemID,true);
		...
		{
			$TitleSegs[] = ftree_field_urldecode($TitlePathRecord["item_title"]);
			$IDSegs[] = "n".$TitlePathRecord["item_id"]."x";
		}

		array_shift($TitleSegs);
		$TitlePath = join("/",$TitleSegs);
		$IDSegPath = join(" ",$IDSegs);
		$ItemTitle = ftree_field_urldecode($ItemRecord["item_title"]);
		...
		fputs($Handle,"@archive_group_title=".ftree_field_urldecode($ArchiveInfo["archive_group"]["item_title"])."\n");
		if ($ArchiveInfo["archive"] != false)
		{
			fputs($Handle,"@archive_title=".ftree_field_urldecode($ArchiveInfo["archive"]["item_title"])."\n");
		}
		...
		fputs($Handle,"item_id=$ItemID\n");
		fputs($Handle,"item_path=$IDSegPath\n");
		...
		if ($TagRecordList != false)
		{
			...
			{
				$TagTextList[] = ftree_field_urldecode($TagRecord["tag_value"]);
			}
		...
		{
			$FieldTitle = ftree_field_urldecode($FieldDef["field_title"]);
			...
			{
				$FieldTitle = ftree_field_urldecode($AltTitleList[0]["field_alt_title"]);
			}
			else
			{
				...
				if ($LocalFormID !== false)
				{
					...
					if ($FormFieldList != false)
					{
						...
						if (ltrim(rtrim(ftree_field_urldecode($TempFieldDef["field_alt_title"]))) != "")
						{
							$FieldTitle = ltrim(rtrim(ftree_field_urldecode($TempFieldDef["field_alt_title"])));
						}
					}
				}
			}
			...
			{
				$FieldValue = ftree_field_urldecode($FieldDataCache[$FieldID]);
			}
			else
			{
				$FieldValue = "";
			...
			$FieldValue = preg_replace("/[\x01-\x1F\x7F-\xff]/"," ",$FieldValue);
//			$FieldValue = preg_replace("/[\x80-\xff]+/"," ",$FieldValue);
//			$FieldValue = preg_replace("/[\x09\x0c\x0a]+/"," ",$FieldValue);
			$OutFields[] = $FieldTitle.": ".preg_replace("/[\n\r]+/"," ",$FieldValue);

			// If the field isn't OCR text, output an attribute (separate search field)
			
			if ($FieldDef["field_symbolic_name"] != AIB_PREDEF_FIELD_OCRTEXT)
			{
...
//		fputs($Handle,"Archive Group: ".ftree_field_urldecode($ArchiveInfo["archive_group"]["item_title"])."\n");
//		fputs($Handle,"Archive: ".ftree_field_urldecode($ArchiveInfo["archive"]["item_title"])."\n");
...
function ftree_list_symbolic_fields($DBHandle,$UserID = false,$OwnerType = false,$Owner = false)
{
	...
	if ($ResultList != false)
	{
		...
		{
			$NewRow = $Row;
			$NewRow["field_title"] = ftree_field_urldecode($Row["field_title"]);
			$NewRow["field_format"] = ftree_field_urldecode($Row["field_format"]);
			$OutList[] = $NewRow;
		}
	}
...
function ftree_list_forms($DBHandle,$UserID,$OwnerType = false)
{
	...
	if ($ResultList != false)
	{
		...
		{
			$NewRow = $Row;
			$NewRow["form_title"] = ftree_field_urldecode($Row["form_title"]);
			$OutList[] = $NewRow;
		}
	}
