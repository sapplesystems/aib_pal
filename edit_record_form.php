...
// #########
	...
	// Get current call method and source.  SourcePage is the page from which the form was invoked,
	// SourceKey is the key string required to return to the source page with the correct information,
	// OpCode is what operation is being performed:
	//
	//	edit
	//	save
	//	next_queued_record
	//		Next record available for data entry
	//
	//	next_queued_item
	//		Next item available for data entry
	//
	//
	// EditID is the item ID of the tree entry being edited
	...
	$PassedParent = aib_get_with_default($FormData,"parent","");
	...
	switch($UserType)
	{
		case FTREE_USER_TYPE_ROOT:
			$DisplayData["page_title"] = "SYSTEM ADMINISTRATOR: EDIT ARCHIVE RECORD/ITEM";
			break;
		...
		default:
			$DisplayData["page_title"] = $UserRecord["user_title"].": EDIT ARCHIVE RECORD/ITEM";
			break;
	}
...
include('template/common_header_admin.php');
	...
	switch($OpCode)
	{
		...
		case "edit":
			...
			// If the source was "records", this is an edit request.  Determine the type of entry, and then
			...
			if ($SourceType == "records")
			{
				...
				switch($ItemType)
				{
					case AIB_ITEM_TYPE_RECORD:
						$FormData["record_id"] = $TempID;
						$FormData["item_id"] = "-1";
						$FormData["primary"] = $ItemRecord["item_parent"];
						$PassedParent = $FormData["primary"];
						break;

					case AIB_ITEM_TYPE_ITEM:
						$FormData["record_id"] = $ItemRecord["item_parent"];
						$FormData["item_id"] = $TempID;
						$ParentRecord = ftree_get_item($GLOBALS["aib_db"],$ItemRecord["item_parent"]);
						$FormData["primary"] = $ParentRecord["item_id"];
						$PassedParent = $FormData["record_id"];
						$EditItemID = $TempID;
						break;
		...
		case "save_record":
			// Get the id of the item to save and the record item ID

			$CurrentSubgroupID = aib_get_with_default($FormData,"primary","-1");
			$CurrentRecordID = aib_get_with_default($FormData,"record_id","-1");
			$CurrentRecordItemID = aib_get_with_default($FormData,"item_id","-1");
			$SavedRecordID = $CurrentRecordID;
			$SavedItemID = $CurrentRecordItemID;
			...
			// Get a list of all fields (including title)

			$ItemRecord = ftree_get_item($GLOBALS["aib_db"],$CurrentRecordID);
			$NewTitle = aib_get_with_default($FormData,"recordfield_title",false);
			$UpdatedFields = array();
			foreach($FormData as $FormFieldName => $FormFieldValue)
			{
				if (preg_match("/^formfield[\_]/",$FormFieldName) == false)
				{
					continue;
				}

				$FieldID = preg_replace("/^formfield[\_]/","",$FormFieldName);
				$UpdatedFields[$FieldID] = $FormFieldValue;
			}

			// Update data

			ftree_field_update_item_fields($GLOBALS["aib_db"],$CurrentRecordID,$UpdatedFields);
			$OriginalTitle = urldecode($ItemRecord["item_title"]);
			if ($OriginalTitle != $NewTitle)
			{
				$RenameResult = ftree_rename($GLOBALS["aib_db"],$CurrentRecordID,urlencode($NewTitle));
				if ($RenameResult[0] != "OK")
				{
					$ErrorMessage = $RenameResult[1];
			...
			// Save indexing document

			ftree_field_export_to_search($GLOBALS["aib_db"],AIB_BASE_INDEX_DOC_PATH,"/browse.php?parent=[[ITEMID]]",array($ItemRecord));

			// Get the list of child items.  If there are child items, make the first item the next thing to edit.
			...
			if ($ChildItemList != false)
			{
				...
				{
					$NextItemFlag = true;
					$CurrentRecordItemID = $ChildItemList[0]["item_id"];
				}
			}

			$NextRecordFlag = true;

			// If there are no child items, then mark the current record as complete and go to next record

			if ($NextItemFlag == false)
			{
				$MarkResult = ftree_mark_data_entry_complete($GLOBALS["aib_db"],$CurrentRecordID,$UserID);
				...
				// If there aren't any more, then break here
				...
				// Otherwise, get the ID of the first record
		...
		case "save_item":
			...
			// Update the item; first, get all fields and title.

			$ItemRecord = ftree_get_item($GLOBALS["aib_db"],$CurrentRecordItemID);
			$ItemForm = ftree_field_get_item_form($GLOBALS["aib_db"],$CurrentRecordItemID);
			$NewTitle = aib_get_with_default($FormData,"recordfield_title",false);
			$UpdatedFields = array();
			foreach($FormData as $FormFieldName => $FormFieldValue)
			{
			...
			ftree_field_update_item_fields($GLOBALS["aib_db"],$CurrentRecordItemID,$UpdatedFields);

			// Get original title and new; if mismatch, update title.

			$OriginalTitle = urldecode($ItemRecord["item_title"]);
			if ($OriginalTitle != $NewTitle)
			{
				$RenameResult = ftree_rename($GLOBALS["aib_db"],$CurrentRecordItemID,urlencode($NewTitle));
				if ($RenameResult[0] != "OK")
				{
					$ErrorMessage = $RenameResult[1];
					$NextItemFlag = false;
			...
			// If there are items, go to the next item (if available).  If none available, drop through to next record selection

			$NextItemID = -1;
			$ChildCount = count($ChildItemList);
			if ($ChildCount > 0)
			{
				$LastChildID = $ChildItemList[$ChildCount - 1]["item_id"];
				if ($CurrentRecordItemID != $LastChildID)
				{
					for ($LocalCounter = 0; $LocalCounter < $ChildCount; $LocalCounter++)
					{
						$TempRecord = $ChildItemList[$LocalCounter];
						if ($CurrentRecordItemID == $TempRecord["item_id"])
						{
							$NextItemID = $ChildItemList[$LocalCounter + 1]["item_id"];
							break;
						}
					}
				}
				...
				if ($NextItemID > 0)
				{
					$CurrentRecordItemID = $NextItemID;
					$NextRecordFlag = true;
					$NextItemFlag = true;
					break;
				}
			...
			// If no more child items, then mark the current record as completed
	...
	switch($OpCode)
	{
		// Record was saved.  If there's a next item ID, show chain options for that.  If no next item
		// ID but there is a record ID, show chain options for that.  Otherwise, links for home page
		// or wherever source was.
		...
		case "save_record":
			...
			$SavedRecord = ftree_get_item($GLOBALS["aib_db"],$SavedRecordID);
			$OutBuffer[] = "<table width='100%' cellpadding='0' cellspacing='0'>";
			$OutBuffer[] = "<tr>";
			$OutBuffer[] = "<td align='right' width='30%' class='saved-changes-title'>Saved Changes To: &nbsp;</td>";
			$OutBuffer[] = "<td class='edit-record-record-title'> &nbsp; </td>";
			$OutBuffer[] = "</tr>";
			$OutBuffer[] = "<tr>";
			$OutBuffer[] = "<td align='right' width='30%'>Sub-Group: &nbsp; </td>";
			$OutBuffer[] = "<td class='edit-record-record-title'>".urldecode($SubgroupRecord["item_title"])."</td>";
			$OutBuffer[] = "</tr>";
			$OutBuffer[] = "<tr>";
			$OutBuffer[] = "<td align='right' width='30%'>Record: &nbsp; </td>";
			$OutBuffer[] = "<td class='edit-record-record-title'>".urldecode($SavedRecord["item_title"])."</td>";
			$OutBuffer[] = "<tr>";
			$OutBuffer[] = "<td align='right' width='30%'> </td>";
			$OutBuffer[] = "<td class='edit-record-record-title'>";
			if ($NextItemFlag == true)
			{
				$Target = "/edit_record_form.php?opcode=_next_item&$RecordFields&parent=$PassedParent&srcmode=$SrcMode&srckey=$SourceKey&searchval=$SearchVal&srcpn=$SrcPn";
				$OutBuffer[] = "<br><br><br><a href=\"$Target\">Click Here To Go To The Next Item In The Record</a>";
				$TargetSpec[] = array();
			}
			else
			{
				if ($NextRecordFlag == true)
				{
					$Target = "/edit_record_form.php?opcode=_next&$RecordFields&parent=$PassedParent&srcmode=$SrcMode&srckey=$SourceKey&searchval=$SearchVal&srcpn=$SrcPn";
					$OutBuffer[] = "<br><br><br><a href=\"$Target\">Click Here To Go To The Next Record</a>";
					$TargetSpec[] = array();
				}
				else
				{
					$Target = "/admin_main.php?parent=$PassedParent&srcmode=$SrcMode&srckey=$SourceKey&searchval=$SearchVal&srcpn=$SrcPn";
					$OutBuffer[] = "<br><br><br><a href=\"$Target\">Click Here To Return To Home Page</a>";
					$TargetSpec[] = array();
				}
		...
		case "save_item":
			...
			$SavedRecord = ftree_get_item($GLOBALS["aib_db"],$SavedRecordID);
			$SavedItem = ftree_get_item($GLOBALS["aib_db"],$SavedItemID);
			$SubgroupRecord = ftree_get_item($GLOBALS["aib_db"],$CurrentSubgroupID);
			$SavedRecord = ftree_get_item($GLOBALS["aib_db"],$SavedRecordID);
			$OutBuffer[] = "<table width='100%' cellpadding='0' cellspacing='0'>";
			$OutBuffer[] = "<tr>";
			...
			$OutBuffer[] = "</tr>";
			$OutBuffer[] = "<tr>";
			$OutBuffer[] = "<td align='right' width='30%'>Item: &nbsp; </td>";
			$OutBuffer[] = "<td class='edit-record-record-title'>".urldecode($SavedItem["item_title"])."</td>";
			$OutBuffer[] = "</tr>";
			$OutBuffer[] = "<tr>";
			$OutBuffer[] = "<td align='right' width='30%'> </td>";
			$OutBuffer[] = "<td class='edit-record-record-title'>";
			...
			if ($SourceType == "records")
			{
				...
				$OutBuffer[] = "<td align='right' width='30%'> </td>";
				$OutBuffer[] = "<td class='edit-record-record-title'>";
				$Target = "/records.php?parent=$SavedRecordID&srcmode=$SrcMode&srckey=$SourceKey&searchval=$SearchVal&srcpn=$SrcPn";
				$OutBuffer[] = "<a href=\"$Target\">Click Here To Return To Record Management</a>";
				$OutBuffer[] = "</td></tr>";
				$OutBuffer[] = "<tr>";
				$OutBuffer[] = "<td align='right' width='30%'> </td>";
...
/*
			...
			{
				$Target = "/edit_record_form.php?opcode=_next_item&$RecordFields&parent=$PassedParent&srcmode=$SrcMode&srckey=$SourceKey&searchval=$SearchVal&srcpn=$SrcPn";
				$TargetSpec[] = array("url" => $Target, "title" => "Next Item In Record", "fields" => array());
			}
			else
			{
				if ($NextRecordFlag == true)
				{
					$Target = "/edit_record_form.php?opcode=_next&$RecordFields&parent=$PassedParent&srcmode=$SrcMode&srckey=$SourceKey&searchval=$SearchVal&srcpn=$SrcPn";
					$TargetSpec[] = array("url" => $Target, "title" => "Next Record", "fields" => array());
				}
			}
*/
		...
		case "do_del":
			...
			// If the Item ID is GE zero, then we're deleting an item.  Otherwise, we're deleting a record

			if ($CurrentRecordItemID > -1)
			{
				ftree_delete($GLOBALS["aib_db"],$CurrentRecordItemID);
			}
			else
			{
				ftree_delete($GLOBALS["aib_db"],$CurrentRecordID);
			}
...
//	else
	if (count($TargetSpec) < 1)
	{
		...
		switch($OpCode)
		{
			...
			case "save_record":
			case "start_sub":
				$TargetOpCode = "save_record";
				break;

			case "_next_item":
			case "save_item":
				$TargetOpCode = "save_item";
				break;
		...
		if ($TargetOpCode != false)
		{
			...
			if ($PassedParent == "" || $PassedParent === false)
			{
				$PassedParent = $CurrentSubgroupID;
			}
			...
			$OutBuffer[] = aib_draw_textarea_field($TempDef);
			...
			foreach($FieldDataList as $FieldID => $FieldInfo)
			{
				...
				switch($FieldDef["field_data_type"])
				{
					...
					default:
						...
						$TempDef["input_under"] = "Y";
						$TempDef["class_aib_input_field_title_cell"] = "aib-data-entry-field-title-cell";
						$OutBuffer[] = aib_draw_input_field($TempDef);
						break;
				}
			}
			...
			{
				case "save_record":
					$OutBuffer[] = aib_draw_form_submit("Save Record","Undo Changes","aib-data-entry-submit-button","aib-data-entry-reset-button");
					break;

				case "save_item":
					$OutBuffer[] = aib_draw_form_submit("Save Item","Undo Changes","aib-data-entry-submit-button","aib-data-entry-reset-button");
					break;
			...
			$IDPathList = ftree_get_item_id_path($GLOBALS["aib_db"],$CurrentRecordID);
