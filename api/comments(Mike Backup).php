...
// #########
	...
	switch($OpCode)
	{
		...
		case "cmnt_newthrd":
			...
			{
				$CommentSetInfo = ftree_create_object_by_path($GLOBALS["aib_db"],$UserID,$GroupID,$CommentSetParent,$PathDef,"\t");
				...
				$CommentSetID = $CommentSetInfo[1];
				ftree_set_property($GLOBALS["aib_db"],$CommentSetInfo[1],AIB_FOLDER_PROPERTY_FOLDER_TYPE,AIB_ITEM_TYPE_COMMENT_SET,true);
			}
			else
			{
				$CommentSetID = $CommentSetParent["item_id"];
			}
			...
			$CommentInfo = ftree_create_object_by_path($GLOBALS["aib_db"],$UserID,$GroupID,$CommentSetID,$PathDef,"\t");
			if ($CommentInfo[0] != "OK")
			{
				$OutData["status"] = "ERROR";
			...
			ftree_set_property($GLOBALS["aib_db"],$CommentInfo[1],AIB_FOLDER_PROPERTY_FOLDER_TYPE,AIB_ITEM_TYPE_COMMENT_THREAD,true);
		...
		// Update comment thread properties (title only)
		...
		case "cmnt_lthread":
			...
			$CommentSetParentInfo = ftree_get_object_by_parent_path($GLOBALS["aib_db"],$PathSpec,$ParentItem,"\t");
			if ($CommentSetParentInfo == false)
			{
				$OutData["status"] = "OK";
				$OutData["info"] = array("records" => array());
			}

			$CommentSetParent = $CommentSetParentInfo["item_id"];
			...
			$OutData["info"] = array("records" => array());
			$ResultList = ftree_list_child_objects($GLOBALS["aib_db"],$CommentSetParent,$UserID,false,FTREE_OBJECT_TYPE_FOLDER,false,$SortTitleFlag,$SortIDFlag);
...
//						$FieldList = ftree_field_get_item_fields_ext($GLOBALS["aib_db"],$ResultRecord["item_id"]);
		...
		case "cmnt_addcmnt":
			...
			$FieldDef = ftree_get_field_by_symbolic_name($DBHandle,AIB_PREDEF_FIELD_COMMENT_TEXT);
			if ($FieldDef == false)
			{
				$OutData["status"] = "ERROR";
		...
		case "cmnt_lstcmnt":
			...
			$OutData["info"] = array("records" => array());
			$ResultList = ftree_list_child_objects($GLOBALS["aib_db"],$ItemID,$UserID,false,FTREE_OBJECT_TYPE_FOLDER,false,$SortTitleFlag,$SortIDFlag);
			if ($CountOnlyFlag == true)
			{
				if ($ResultList[0] == "ERROR")
				{
