	...
	switch($OpCode)
	{
		...
		case "send":
			...
			try {
				...
				if ($ReplyName != false)
				{
					$Mailer->addReplyTo($ReplyAddress,$ReplyName);
				}
				else
				{
					$Mailer->addReplyTo($ReplyAddress);
				}
				...
				foreach($CCList as $ToAddress => $ToName)
				{
					if ($ToName != false)
					{
						$Mailer->addCC($ToAddress,$ToName);
					}
					else
					{
						$Mailer->addCC($ToAddress);
					}
				}
...
*/
				...
				$Mailer->isHTML($IsHTML);	// Set email format
