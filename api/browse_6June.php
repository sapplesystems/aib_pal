...
aib_browse_log_debug("Request at ".date("D/m/Y H:i:s"));
aib_browse_log_debug(var_export($FormData,true));
	switch($OpCode)
	{
				...
				foreach($ResultList as $TempResultRecord)
				{
					...
					if ($OptionDerefLinks == false)
					{
						...
						if ($TempResultRecord["item_type"] == FTREE_OBJECT_TYPE_LINK)
						{
							...
							$LinkData["link_title"] = urldecode($TempResultRecord["item_title"]);
						}
					}
					else
					{
						...
						if ($TempResultRecord["item_type"] == FTREE_OBJECT_TYPE_LINK)
						{
							...
							if ($DerefRecord != false)
							{
								...
								$DerefPropertyList = ftree_list_properties($GLOBALS["aib_db"],$DerefRecord["item_id"]);
								...
								$LinkData["final_deref_item_title"] = urldecode($DerefRecord["item_title"]);
...
//						$FieldList = ftree_field_get_item_fields_ext($GLOBALS["aib_db"],$ResultRecord["item_id"]);
					...
					{
						$PropertyCountSet = ftree_count_child_object_property_set($GLOBALS["aib_db"],$ResultRecord["item_id"],false,false,false,$PropertyCountDef);
