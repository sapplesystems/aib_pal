...
// #########
	...
	switch($OpCode)
	{
		...
		case "list":
			...
			$LongPropFlag = get_assoc_default($FormData,"long_properties","Y");
			if (preg_match("/[Yy]/",$LongPropFlag) != false)
			{
				$LongPropFlag = true;
			}
			else
			...
			$OptionLinkOwner = ftree_option_truefalse(get_assoc_default($FormData,"opt_link_owner","N"));
			$OptionHide = ftree_option_truefalse(get_assoc_default($FormData,"opt_hide_enable","N"));
			$OptionFollowLinks = ftree_option_truefalse(get_assoc_default($FormData,"opt_follow_links","N"));
			$OptionRootFolder = ftree_option_truefalse(get_assoc_default($FormData,"opt_get_root_folder","N"));
			$OptionRootFolderProperties = ftree_option_truefalse(get_assoc_default($FormData,"opt_get_root_prop","N"));
			$OptionOCRText = strtoupper(get_assoc_default($FormData,"opt_get_ocr_text","N"));
			$RequestStartResult = get_assoc_default($FormData,"start_result",false);
			$RequestResultCount = get_assoc_default($FormData,"result_count",false);
			if ($OptionOCRText == "N")
			{
				$OptionOCRText = false;
			}
			...
			if ($OptionLinkOwner == "N")
			{
				$OptionLinkOwner = false;
			}
			else
			{
				$OptionLinkOwner = true;
			}
			...
			$OptionLinkSourceProperties = get_assoc_default($FormData,"opt_get_link_source_properties","N");
			if ($OptionLinkSourceProperties == "Y")
			{
				$OptionLinkSourceProperties = true;
			}
			else
			{
				$OptionLinkSourceProperties = false;
			}
			...
			$ChildObjectListString = get_assoc_default($FormData,"item_list",false);
			if ($ChildObjectListString !== false)
			{
				$TempObjectList = explode(",",$ChildObjectListString);
				$ResultList = ftree_get_child_object_set($GLOBALS["aib_db"],$TempObjectList);
			}
			else
			{
			...
			if ($ResultList != false)
			{
				...
				if ($OptionSortOrder == "STPA")
				{
					...
					foreach($ResultList as $TempResultRecord)
					{
						...
						$STPSort = ftree_get_property($GLOBALS["aib_db"],$TempResultRecord["item_id"],"stp:sort_name");
						if ($STPSort == false)
						{
							$STPSort = browse_cleanup_text(browse_urldecode($TempResultRecord["item_title"])).",".$TempResultRecord["item_id"];
						}
						else
						{
				...
				// Process each child.  TempResultRecord contains the original child record (link or otherwise),
				// while ResultRecord is the dereferenced record.

				$RootInfoCache = false;
				$OutputResultCounter = 0;
				foreach($ResultList as $TempResultRecord)
				{
					// If hide option is set, make sure this record is visible.

					if ($OptionHide == true)
					{
						$EntryTypeProperty = ftree_get_property($GLOBALS["aib_db"],$TempResultRecord["item_id"],AIB_FOLDER_PROPERTY_VISIBLE);
						if ($EntryTypeProperty == "N")
						{
							continue;
						}
					}

					if ($RequestStartResult !== false && $RequestResultCount !== false)
					{
						if ($OutputResultCounter < $RequestStartResult)
						{
							$OutputResultCounter++;
							continue;
						}

						if ($OutputResultCounter >= $RequestStartResult + $RequestResultCount)
						{
							break;
						}
					}

					$TempResultRecord["item_title"] = urlencode(browse_cleanup_text(browse_urldecode($TempResultRecord["item_title"])));
					...
					if ($OptionDerefLinks == false)
					{
						...
						if ($TempResultRecord["item_type"] == FTREE_OBJECT_TYPE_LINK)
						{
							...
							$LinkData["link_title"] = browse_cleanup_text(browse_urldecode($TempResultRecord["item_title"]));
						}
					}
					else
					{
						...
						if ($TempResultRecord["item_type"] == FTREE_OBJECT_TYPE_LINK)
						{
							...
							// Get ultimate dereference

							$DerefRecord = aib_deref_link($GLOBALS["aib_db"],$TempResultRecord["item_id"]);
							if ($DerefRecord != false)
							{
								$DerefPropertyList = ftree_list_properties($GLOBALS["aib_db"],$DerefRecord["item_id"],true);

								// If the deref record is supposed to be hidden and we're filtering hidden records, skip this dereferenced record.

								if ($OptionHide == true)
								{
									if (get_assoc_default($DerefRecord,AIB_FOLDER_PROPERTY_VISIBLE,"Y") == "N")
									{
										continue;
									}
								}

								$TempLinkData = aib_util_generate_link_target_info($GLOBALS["aib_db"],
									"final_deref_","",$DerefRecord,$OptShowLinkHome);
								foreach($TempLinkData as $TempLinkDataName => $TempLinkDataValue)
								{
									$LinkData[$TempLinkDataName] = $TempLinkDataValue;
								}

								$LinkData["final_deref_id"] = $DerefRecord["item_id"];
								$LinkData["final_deref_tree_type"] = $DerefRecord["item_type"];
								$LinkData["final_deref_properties"] = array();
								$LinkData["final_deref_item_source_type"] = $DerefRecord["item_source_type"];
								$LinkData["final_deref_item_source_info"] = $DerefRecord["item_source_info"];
								$LinkData["final_deref_item_create_stamp"] = $DerefRecord["item_create_stamp"];
								$LinkData["final_deref_item_create_string"] = date("Y.m.d.H.i.s",$DerefRecord["item_create_stamp"]);
								$LinkData["final_deref_item_title"] = browse_cleanup_text(browse_urldecode($DerefRecord["item_title"]));
								foreach($DerefPropertyList as $LocalPropName => $LocalPropValue)
								{
									$LinkData["final_deref_properties"][urldecode($LocalPropName)] = urldecode($LocalPropValue);
								}
	
								// Get field data for derefed item

								$DerefFormID = ftree_field_get_item_form($GLOBALS["aib_db"],$DerefRecord["item_id"]);
								$DerefFieldList = get_item_fields_in_form_order($GLOBALS["aib_db"],$DerefRecord["item_id"],$DerefFormID);
								$DerefItemFields = array();
								if ($DerefFieldList != false)
								{
									foreach($DerefFieldList as $FieldInfo)
									{
										$LocalDef = $FieldInfo["def"];
										if ($LocalDef["field_symbolic_name"] == AIB_PREDEF_FIELD_OCR_TEXT)
										{
											if ($OptionOCRText == false)
											{
												continue;
											}
										}
										...
										if ($LocalDef["field_symbolic_name"] == AIB_PREDEF_FIELD_OCR_TEXT)
										{
											$TempRecord["field_value"] = urlencode($TempRecord["field_value"]);
										}

										$DerefItemFields[] = $TempRecord;
									}
								}

								$LinkData["final_deref_form"] = $DerefFormID;
								$LinkData["final_deref_fields"] = $DerefItemFields;
	
								// Get tags, if any
	
								$DerefTagSet = aib_get_item_tags($GLOBALS["aib_db"],$DerefRecord["item_id"]);
								if ($DerefTagSet != false)
								{
									$LinkData["final_deref_tags"] = $DerefTagSet;
								}

								// Get associated files
	
								$DerefFileList = aib_get_files_for_item($GLOBALS["aib_db"],$DerefRecord["item_id"]);
								if ($DerefFileList == false)
								{
									$DerefFileList = array();
								}
	
								$DerefItemFileList = array();
								foreach($DerefFileList as $FileRecord)
								{
								...
								$LinkData["final_deref_files"] = $DerefItemFileList;
									...
									switch($LinkInfo["type"])
									{
										...
										case FTREE_STP_LINK_EDITION:
											...
											$LinkData["stp_url"] = $LinkInfo["paper"].".".STP_ARCHIVE_DOMAIN."/".
												...
												$LinkInfo["day"]."/";
											$LinkData["stp_thumb"] = "https://www.".AIB_STP_THUMBNAIL_HOST."/aib_thumb.php?edition=".
					...
					// Get root folder if requested

					if ($OptionRootFolder == true)
					{
						// Grab root info.  If a link, then get fresh copy of root info.  Otherwise, copy from
						// cache for the current parent.

						if ($IsLink == true)
						{
							$TempRootInfo = ftree_get_archive_and_archive_group($GLOBALS["aib_db"],$ResultRecord["item_id"]);
						}
						else
						{
							if ($RootInfoCache == false)
							{
								$RootInfoCache = ftree_get_archive_and_archive_group($GLOBALS["aib_db"],$ResultRecord["item_id"]);
							}

							$TempRootInfo = $RootInfoCache;
						}

						$RootInfo["archive"] = $TempRootInfo["archive"];
						$RootInfo["archive_group"] = $TempRootInfo["archive_group"];
						$RootInfo["archive_properties"] = array();
						$RootInfo["archive_group_properties"] = array();
						if ($OptionRootFolderProperties == true)
						{
							if ($TempRootInfo["archive_group"] !== false)
							{
								$RootPropertyList = ftree_list_properties($GLOBALS["aib_db"],$TempRootInfo["archive_group"],$GetLongFlag);
								if ($RootPropertyList != false)
								{
									foreach($RootPropertyList as $RootPropertyName => $RootPropertyValue)
									{
										$RootLocalName = urldecode($RootPropertyName);
										$RootLocalValue = urldecode($RootPropertyValue);
										$RootInfo["archive_group_properties"][$RootLocalName] = browse_cleanup_text($RootLocalValue);
									}
								}
							}

							if ($TempRootInfo["archive"] !== false)
							{
								$RootPropertyList = ftree_list_properties($GLOBALS["aib_db"],$TempRootInfo["archive"],$GetLongFlag);
								if ($RootPropertyList != false)
								{
									foreach($RootPropertyList as $RootPropertyName => $RootPropertyValue)
									{
										$RootLocalName = urldecode($RootPropertyName);
										$RootLocalValue = urldecode($RootPropertyValue);
										$RootInfo["archive_properties"][$RootLocalName] = browse_cleanup_text($RootLocalValue);
									}
								}
							}
						}
					...
					if ($GetPropertyFlag == true)
					{
						...
						if ($PropertyList != false)
						{
							foreach($PropertyList as $PropertyName => $PropertyValue)
							{
								...
								$ItemPropertySet[$LocalName] = browse_cleanup_text($LocalValue);
							}
						}
					}
...
//						$FieldList = ftree_field_get_item_fields_ext($GLOBALS["aib_db"],$ResultRecord["item_id"]);
					...
					$OutRecord = array(
						...
						"item_title" => browse_cleanup_text(browse_urldecode($ResultRecord["item_title"])),
					...
					if ($IsLink == true)
					{
						...
						if ($OptionLinkOwner == true)
						{
							...
							$OutRecord["link_owner_title"] = browse_urldecode($TempUser["user_title"]);
							$OutRecord["link_owner_properties"] = array();
							$UserPropertyList = ftree_list_user_prop($GLOBALS["aib_db"],$OutRecord["link_owner"]);
							foreach($UserPropertyList as $LocalName => $LocalValue)
							{
								$TempName = urldecode($LocalName);
								$OutRecord["link_owner_properties"][$TempName] = browse_cleanup_text(urldecode($LocalValue));
							}
						...
						if ($OptionLinkProperties == true)
						{
							...
							if ($LinkProperties != false)
							{
								...
								{
									$TempName = urldecode($LocalName);
									$OutRecord["link_ref_properties"][$TempName] = browse_cleanup_text(urldecode($LocalValue));
								}
							}

							$OutRecord["link_ref_properties"]["_item_id"] = $TempResultRecord["item_ref"];
						}

						$OutRecord["link_properties"] = array();
						if ($OptionLinkSourceProperties == true)
						{
							$LinkProperties = ftree_list_properties($GLOBALS["aib_db"],$TempResultRecord["item_id"],true);
							if ($LinkProperties != false)
							{
								foreach($LinkProperties as $LocalName => $LocalValue)
								{
									$TempName = urldecode($LocalName);
									$OutRecord["link_properties"][$TempName] = browse_cleanup_text(urldecode($LocalValue));
								}

								$OutRecord["link_properties"]["_item_id"] = $TempResultRecord["item_id"];
							}
						}
					...
					// Count properties if needed. We always count the dereferenced folder tree item.

					$OutRecord["property_counts"] = array();
					if ($OptGetPropCount == true)
					{
//						$OutRecord["_debug"] = $ResultRecord;
						$PropertyCountSet = ftree_count_child_object_property_set_batch($GLOBALS["aib_db"],$ResultRecord["item_id"],false,false,false,$PropertyCountDef,$OptionFollowLinks,$LongPropFlag);
		...
		case "get":
			...
			if ($ResultRecord != false);
			{
				...
				if ($GetPropertyFlag == true)
				{
					$PropertyList = ftree_list_properties($GLOBALS["aib_db"],$ResultRecord["item_id"],true);
					if ($PropertyList != false)
					{
						foreach($PropertyList as $PropertyName => $PropertyValue)
						{
...
//			$FieldList = ftree_field_get_item_fields_ext($GLOBALS["aib_db"],$ItemID);
		...
		case "store_item_fields":
			...
			$Result = ftree_field_update_item_fields($GLOBALS["aib_db"],$ItemID,$StorageData,false);
			if ($Result == false)
			{
				$OutData["status"] = "ERROR";
...
*/
			...
			foreach($SourceFieldList as $FormName => $InfoName)
			{
				...
				{
					// If item title, URL encode. Else remain the same.

					if ($FormName == 'item_title')
					{
						$ItemInfo[$InfoName] = urlencode($FormData[$FormName]);
					}
					else
					{
		...
		case "modify_item":
			...
			foreach($ModifyList as $ModifyName)
			{
				...
				{
					// If updating item_title, URL encode.  Else copy.

					if ($ModifyName == "item_title")
					{
						$NewData[$ModifyName] = urlencode($FormData[$ModifyName]);
					}
					else
					{
		...
		case "delete_item":
			...
			// Get item record.  Don't delete anything with a parent of -1 unless safety parameter is set.

			$ItemInfo = ftree_get_item($GLOBALS["aib_db"],$ItemID);
			$InvalidFlag = false;
			while(true)
			{
				if ($ItemInfo["item_parent"] < 0 || $ItemID < 0 || $ItemID == 1)
				{
					$SafetyCheck = get_assoc_default($FormData,"opt_param_1",false);
					if ($SafetyCheck != "force")
					{
						$OutData["status"] = "ERROR";
						$OutData["info"] = "MISSING SAFETY";
						$InvalidFlag = true;
						break;
					}
...
*/
		...
		case "get_archive_and_group":
			...
			$OutData["status"] = "OK";
			if (isset($ArchiveInfo["archive"]["item_title"]) == true)
			{
				$ArchiveInfo["archive"]["item_title"] = browse_cleanup_text(browse_urldecode($ArchiveInfo["archive"]["item_title"]));
			}

			if (isset($ArchiveInfo["archive_group"]["item_title"]) == true)
			{
				$ArchiveInfo["archive_group"]["item_title"] = browse_cleanup_text(browse_urldecode($ArchiveInfo["archive_group"]["item_title"]));
			}
							...
							if ($GetPropertyFlag == true)
							{
								...
								if ($PropertyList != false)
								{
									foreach($PropertyList as $PropertyName => $PropertyValue)
									{
										...
										$TempPropertySet[$LocalName] = browse_cleanup_text(browse_urldecode($LocalValue));
									}
								}
							}
...
//								$FieldList = ftree_field_get_item_fields_ext($GLOBALS["aib_db"],$TempChildRecord["item_id"]);
							...
							$TempChildRecord["item_title"] = browse_cleanup_text(browse_urldecode($TempChildRecord["item_title"]));
				...
				if ($GetPropertyFlag == true)
				{
					$PropertyList = ftree_list_properties($GLOBALS["aib_db"],$ItemInfo["item_id"],true);
					if ($PropertyList != false)
					{
						foreach($PropertyList as $PropertyName => $PropertyValue)
						{
							...
							$ItemPropertySet[$LocalName] = browse_cleanup_text(browse_urldecode($LocalValue));
						}
					}
				}
...
//					$FieldList = ftree_field_get_item_fields_ext($GLOBALS["aib_db"],$ItemInfo["item_id"]);
				...
				$OutData["info"]["records"][] = array(
					...
					"item_type" => $LocalEntryType,
					"item_title" => browse_cleanup_text(browse_urldecode($ItemInfo["item_title"])),
		...
		// Recursively set property value. Options are:
		// "depth" -- Number of levels to set under the parent
		// "compare_prop_name" -- Name of a property to compare against (optional)
		// "compare_prop_value" -- Value of a property to compare against.  If value doesn't match, don't set property.
		// "valid_types" -- Comma-separated valid folder tree object types (folder, item, link).  Optional.
		// "valid_titles" -- Pipe-separated valid titles
		// "obj_type_list" -- Comma-separated valid AIB object types (subgroup, item, collection, etc.)
		// "opt_long" -- If set to "Y", use long properties storage
		// "prop_name" -- Name of property to set
		// "prop_value" -- Value of property to set
		// "delete_flag" -- If set to "Y", delete property instead of setting

		case "set_item_prop_recurse":

			// Get item

			$ParentItem = get_assoc_default($FormData,"parent_item",false);
			$RecursionLevels = get_assoc_default($FormData,"depth","0");
			$ComparePropName = get_assoc_default($FormData,"compare_prop_name",false);
			$ComparePropValue = get_assoc_default($FormData,"compare_prop_value",false);
			$ValidTypeListString = get_assoc_default($FormData,"valid_types",false);
			$ValidTitleListString = get_assoc_default($FormData,"valid_titles",false);
			$AIBObjTypeListString = get_assoc_default($FormData,"obj_type_list",false);
			$LongProperty = get_assoc_default($FormData,"opt_long","N");
			$DeleteFlag = get_assoc_default($FormData,"delete_flag","N");
			$PropertyName = get_assoc_default($FormData,"prop_name",false);
			$PropertyValue = get_assoc_default($FormData,"prop_value",false);
			$CrawlNonMatching = get_assoc_default($FormData,"crawl_non_matching","N");
			$IncludeParent = get_assoc_default($FormData,"include_parent","N");
			if ($ParentItem == false)
			{
				$OutData["status"] = "ERROR";
				$OutData["info"] = "MISSINGITEMORPATH";
			...
			{
				$OutData["status"] = "ERROR";
				$OutData["info"] = "MISSINGPROPNAMEORVALUE";
				break;
			}

			$PropertyValue = urlencode($PropertyValue);
			$ItemRecord = ftree_get_object_by_id($GLOBALS["aib_db"],$ParentItem);
			if ($ItemRecord == false)
			{
				$OutData["status"] = "ERROR";
				$OutData["info"] = "INVALIDID";
				$OutData["msg"] = "$ParentItem";
				break;
			}
			...
			// Initialize item set array
			
			$ItemSet = array();
			if (strtoupper($IncludeParent) == "Y")
			{
				$ItemSet[] = $ItemID;
			}

			// If we're filtering by tree entry type, build associative array for checks

			if ($ValidTypeListString != false)
			{
				$LocalList = explode(",",$ValidTypeListString);
				if (count($LocalList) > 0)
				{
					$ValidTypeSet = array();
					foreach($LocalList as $TypeEntry)
					{
						$ValidTypeSet[$TypeEntry] = true;
					}
				}
				else
				{
			...
			// If we're filtering by tree entry titles, build associative array for checks

			if ($ValidTitleListString != false)
			{
				$LocalList = explode("||",$ValidTitleListString);
				if (count($LocalList) > 0)
				{
					$ValidTitleSet = array();
					foreach($LocalList as $TypeEntry)
					{
						$ValidTitleSet[] = $TypeEntry;
					}
				}
				else
				{
			...
			// If we're filtering by AIB object types, build associative array for checks

			if ($AIBObjTypeListString != false)
			{
				$LocalList = explode(",",strtolower($AIBObjTypeListString));
				if (count($LocalList) > 0)
				{
					$ValidObjSet = array();
					foreach($LocalList as $TypeEntry)
					{
						$ValidObjSet[$TypeEntry] = true;
					}
				}
				else
				{
			...
			// Get all entries to potentially be modified

			ftree_list_recursive($GLOBALS["aib_db"],$ItemID,$ItemSet,$ValidTypeSet,$ValidTitleSet,$CrawlNonMatching,$RecursionLevels,0);
//			ftree_list_recursive_brief($GLOBALS["aib_db"],$ItemID,$ItemSet,$ValidTypeSet,$CrawlNonMatching,$RecursionLevels,0);
			$RecordStack = array();

			// Set properties for child items based on filters

			foreach($ItemSet as $ChildItemID)
			{
				// If there is an AIB object type set, compare
				
				if ($ValidObjSet != false)
				{
					$LocalObjType = ftree_get_property($GLOBALS["aib_db"],$ChildItemID,AIB_FOLDER_PROPERTY_FOLDER_TYPE);
					if ($LocalObjType == false)
					{
						continue;
					}

					if (isset($ValidObjSet[$LocalObjType]) == false)
					{
						continue;
					}
				}

				// If there's a property to be compared, do so here.

				if ($ComparePropName != false)
				{
					$TestValue = ftree_get_property($GLOBALS["aib_db"],$ChildItemID,$ComparePropName);
					if ($TestValue != $ComparePropValue)
					{
						continue;
					}
				}

				$RecordStack[] = $ChildItemID;
				if (count($RecordStack) >= 40)
				{
					if ($DeleteFlag == "Y")
					{
						ftree_delete_property_batch($GLOBALS["aib_db"],$RecordStack,$PropertyName,"B");
					}
					else
					{
						ftree_replace_prop_batch($GLOBALS["aib_db"],$RecordStack,$PropertyName,$PropertyValue,$LongProperty,"Y");
					}
...
/*
				// If deleting property, do so here
				
				if ($DeleteFlag == "Y")
				{
					ftree_delete_property($GLOBALS["aib_db"],$ChildItemID,$PropertyName);
					ftree_delete_long_property($GLOBALS["aib_db"],$ChildItemID,$PropertyName);
					continue;
				}
*/
//				ftree_replace_prop($GLOBALS["aib_db"],$ChildItemID,$PropertyName,$PropertyValue,$LongProperty,"Y");

				// Store property, erasing any property value in alternate table (long/short).
/*
				if ($LongProperty == "N")
				{
					ftree_delete_long_property($GLOBALS["aib_db"],$ChildItemID,$PropertyName);
					ftree_set_property($GLOBALS["aib_db"],$ChildItemID,$PropertyName,$PropertyValue);
				}
				else
				{
					ftree_delete_property($GLOBALS["aib_db"],$ChildItemID,$PropertyName);
					ftree_set_long_property($GLOBALS["aib_db"],$ChildItemID,$PropertyName,$PropertyValue);
				}
*/
//			}
		...
		case "get_item_prop":
			...
			{
				$OutData["info"]["records"][$PropName] = browse_cleanup_text(browse_urldecode($PropValue));
			}
		...
		case "list_item_ref":
			...
			foreach($Result as $Record)
			{
				$OutData["records"][] = array(
					...
					"item_title" => browse_cleanup_text(browse_urldecode($Record["item_title"])),
