...
// Log debug
// ---------
function aib_comment_log_debug($Msg)
{
	$Handle = fopen("/tmp/aib_comment_log_debug.txt","a+");
	if ($Handle != false)
	{
		fputs($Handle,$Msg."\n");
		fclose($Handle);
...
// #########
	...
	switch($OpCode)
	{
		// Create a comment thread to which comments may be added.  First checks for comments folder in parent,
		...
		case "cmnt_newthrd":
			...
			// See if comments folder exists.  If not, create

			$PathSpec = join("\t",array(
				"F:".AIB_PREDEF_FOLDER_NAME_COMMENT_SET,
				));
			$CommentSetParent = ftree_get_object_by_parent_path($GLOBALS["aib_db"],$PathSpec,$ParentItem,"\t");
			if ($CommentSetParent === false)
			{
				$CommentSetInfo = ftree_create_object_by_path($GLOBALS["aib_db"],$UserID,$GroupID,$ParentItem,$PathSpec,"\t");
				if ($CommentSetInfo[0] != "OK")
				{
					$OutData["status"] = "ERROR";
					$OutData["info"] = join(";",$CommentSetInfo);
					break;
				}

				$CommentSetParent = $CommentSetInfo[1];
				ftree_set_property($GLOBALS["aib_db"],$CommentSetParent,AIB_FOLDER_PROPERTY_FOLDER_TYPE,AIB_ITEM_TYPE_COMMENT_SET,true);
			}

			// Create comment thread by path

			$PathDef = join("\t",array(
				"F:".urlencode($Title)
				));
			$CommentInfo = ftree_create_object_by_path($GLOBALS["aib_db"],$UserID,$GroupID,$CommentSetParent,$PathDef,"\t");
			if ($CommentInfo[0] != "OK")
			{
				$OutData["status"] = "ERROR";
				$OutData["info"] = join(";",$CommentInfo);
				break;
			}

			$CommentThreadID = $CommentInfo[1];
			ftree_set_property($GLOBALS["aib_db"],$CommentThreadID,AIB_FOLDER_PROPERTY_FOLDER_TYPE,AIB_ITEM_TYPE_COMMENT_THREAD,true);

			// Return new comment thread folder ID

			$OutData["info"] = $CommentInfo[1];
			break;
			

		// Remove comment thread (or any other tree item for that matter)
		...
		case "cmnt_delthrd":
			...
			ftree_delete($GLOBALS["aib_db"],$ItemID,true);
			$OutData["status"] = "OK";
			$OutData["info"] = "";
			break;
		...
		case "cmnt_lthread":
			...
			// See if comments folder exists.  If not, no data
			...
			$OutData["info"] = array("records" => array());
			$ResultList = ftree_list_child_objects($GLOBALS["aib_db"],$CommentSetParent,$UserID,false,FTREE_OBJECT_TYPE_FOLDER,$CountOnlyFlag,$SortTitleFlag,$SortIDFlag);
...
//						$FieldList = ftree_field_get_item_fields_ext($GLOBALS["aib_db"],$ResultRecord["item_id"]);
		...
		case "cmnt_addcmnt":
			...
			$ThreadParent = get_assoc_default($FormData,"parent_thread",false);
			if ($ThreadParent === false)
			{
				$OutData["status"] = "ERROR";
				$OutData["info"] = "MISSINGPARENT";
				break;
			...
			$Title = get_assoc_default($FormData,"comment_title",false);
			$UserID = get_assoc_default($FormData,"user_id",-1);
			$CommentText = get_assoc_default($FormData,"comment_text",false);
			if ($CommentText == false)
			{
				$OutData["status"] = "ERROR";
				$OutData["info"] = "MISSINGTEXT";
				break;
			}

			// Get field definition for comment text field

			$FieldDef = ftree_get_field_by_symbolic_name($GLOBALS["aib_db"],AIB_PREDEF_FIELD_COMMENT_TEXT);
			if ($FieldDef == false)
			{
				$OutData["status"] = "ERROR";
				$OutData["info"] = "SYSTEMFIELDMISSING";
				break;
			}

			// Create a child item using the title
			...
			ftree_set_property($GLOBALS["aib_db"],$NewObject[1],AIB_FOLDER_PROPERTY_FOLDER_TYPE,AIB_ITEM_TYPE_COMMENT,true);

			// Store field associated with comment

			$CommentID = $NewObject[1];
			ftree_field_store_item_fields($GLOBALS["aib_db"],$CommentID,array($FieldDef["field_id"] => $CommentText));

			$OutData["info"] = $CommentID;
			break;

		// Remove comment (or any other tree item)
		...
		case "cmnt_lstcmnt":
			...
			$ItemID = get_assoc_default($FormData,"parent_thread",false);
			if ($ItemID === false)
			{
				$OutData["status"] = "ERROR";
				$OutData["info"] = "MISSINGID";
			...
			// Note that field retrieval is defaulted to "Y" since comments are stored in a text field

			$OptionGetFields = strtoupper(get_assoc_default($FormData,"opt_get_field","Y"));
			$OptionGetThumb = strtoupper(get_assoc_default($FormData,"opt_get_thumb","N"));
			$OptionGetPrimary = strtoupper(get_assoc_default($FormData,"opt_get_primary","N"));
			$OptionGetFirstThumb = strtoupper(get_assoc_default($FormData,"opt_get_first_thumb","N"));
			$OptionFilterType = strtoupper(get_assoc_default($FormData,"opt_filter_type",false));
			...
			if ($UserID === false)
			{
				$ResultList = ftree_list_child_objects($GLOBALS["aib_db"],$ItemID,$UserID,false,FTREE_OBJECT_TYPE_FOLDER,$CountOnlyFlag,$SortTitleFlag,$SortIDFlag);
			}
			else
			{
				$TempList = ftree_query_ext($GLOBALS["aib_db"],"SELECT * FROM ftree WHERE item_type='".FTREE_OBJECT_TYPE_FOLDER."' AND item_user_id='$UserID' ORDER BY item_parent,item_id;");
				if ($TempList == false)
				{
					$TempList = array();
				}
				...
				{
					$TypeProperty = ftree_get_property($GLOBALS["aib_db"],$TempRecord["item_id"],AIB_FOLDER_PROPERTY_FOLDER_TYPE);
					if ($TypeProperty != AIB_ITEM_TYPE_COMMENT && $TypeProperty != AIB_ITEM_TYPE_COMMENT_THREAD)
					{
						continue;
					}

					if ($CountOnlyFlag === false)
					{
...
//						$FieldList = ftree_field_get_item_fields_ext($GLOBALS["aib_db"],$ResultRecord["item_id"]);
					...
					$ItemParentType = ftree_get_property($GLOBALS["aib_db"],$ResultRecord["item_parent"],AIB_FOLDER_PROPERTY_FOLDER_TYPE);
					if ($ItemParentType == false)
					{
						$ItemParentType = "";
					}
					...
					$OutRecord = array(
						...
						"item_parent_type" => $ItemParentType,
						"item_tree_type" => $ResultRecord["item_type"],
						"item_user_id" => $ResultRecord["item_user_id"],
						"item_title" => urldecode($ResultRecord["item_title"]),
						"item_ref" => $ResultRecord["item_ref"],
						"item_source_type" => $ResultRecord["item_source_type"],
						"item_source_info" => $ResultRecord["item_source_info"],
		...
		// Update comment thread or comment properties
		...
		case "cmnt_upd":
			...
			$NewCommentText = get_assoc_default($FormData,"new_comment_text",false);
			if ($ItemID === false)
			{
				$OutData["status"] = "ERROR";
				$OutData["info"] = "MISSINGID";
			...
			if ($NewCommentText != false)
			{
				...
				if ($FolderType != AIB_ITEM_TYPE_COMMENT)
				{
					$OutData["status"] = "ERROR";
					$OutData["info"] = "NOTACOMMENT";
					break;
				}
				...
				ftree_field_store_item_fields($GLOBALS["aib_db"],$ItemID,array($FieldDef["field_id"] => $NewCommentText));
			}
