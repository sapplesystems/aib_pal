...
<div class="content-wrapper">
    <section class="content-header"> 
        ...
        <h4 class="list_title text-center"><span class="pull-left">Add New Assistant</span> <span class="headingNameDesign"><?php echo $_SESSION['aib']['user_data']['item_title'];?></span> </h4>
    </section>
    <section class="content">
        <div class="admin-loading-image"><img src="<?php echo IMAGE_PATH . 'loading.gif'; ?>" alt="Loading...." /></div>
        <div class="row">
            <div class="col-md-offset-3 col-md-6 col-md-offset-3">
                <form class="marginBottom30 formStyle form-group" action="" method="POST" id="assistantForm" name="assistantForm">
                   <input type="hidden" name="user_exist" id="user_exist" value="false">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-7"><input type="text" class="form-control"  id="login_data"  name="login_data" placeholder="Login username"></div>
                    </div>
                    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-7"><button type="button" class="btn btn-info borderRadiusNone" id="addAssistantButton">Add Assistant</button> &nbsp;
...
$(document).ready(function(){
    ...
    $('#addAssistantButton').click(function(){
        ...
        if($("#assistantForm").valid() && user_exist=='false'){
//            if($("#archive_name option:selected").attr('data-type') == 'ag'){
//                alert("Assistant can't be created in archive group");
//                return false;
//            }
            ...
            $.ajax({
                url: "services_admin_api.php",
                type: "post",
                data: {mode: 'add_assistant', formData: assistantFormData},
                success: function (data){
                     ...
                     if(result.status == 'success'){
						...
						showPopupMessage('error', result.message + ' (Error Code: 1157)');
                     }
                     $('.admin-loading-image').hide();
                },
                error: function () {
                    showPopupMessage('error','Something went wrong, Please try again. (Error Code: 103)');
                }
            });
        }
    });
    ...
    $("#assistantForm").validate({
        ...
        messages: { 
            archive_name:{
                required: "Please select an archive"
            },
            login_data:{
	...
	$('#user_email').keypress(function (e) {
	 ...
	 if(key == 13)   
	  { 
		$('#addAssistantButton').click();
	  }
	}); 
	
});  

$(document).on('focusout', '#login_data', function(){
    ...
    if(user_login ==''){
       ...
       geck_user_exist()
//    if(user_login != ''){
//        if(user_login.indexOf('_assist_'+suffix) == -1){
//            $('#login_data').val(user_login+'_assist_'+suffix);
//        }
//    }
});

$(function(){
    $('#asst_name').keypress(function (e) {
        var key = e.which;
        if(key == 13){  
           $('#addAssistantButton').click();
        }
...
function geck_user_exist(){
      ...
      $.ajax({
            ...
            error: function () {
                showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 104)');
                $('.admin-loading-image').hide();
            }
        });
  }   
