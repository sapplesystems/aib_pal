...
include('template/common_header.php');
	...
	{
//		$ChildItemList = ftree_list_child_objects($GLOBALS["aib_db"],$CurrentParent,false,false,FTREE_OBJECT_TYPE_FOLDER,false,true);
...
//				}
		...
		case AIB_ITEM_TYPE_COLLECTION:
			...
			foreach($ChildItemList as $ChildRecord)
			{
				...
				if ($ChildRecord["item_source_type"] != FTREE_SOURCE_TYPE_STPARCHIVE)
				{
					$ChildItemIcon = ftree_get_property($GLOBALS["aib_db"],$ChildID,AIB_FOLDER_PROPERTY_FOLDER_ICON);
					if ($ChildItemIcon != false)
					{
				...
				{
					$LinkInfo = json_decode(urldecode($ChildRecord["item_source_info"]),true);
					...
					// Output list element.  IMPORTANT:  We don't use the link to the year set on STP Archive; we instead link
					// to subfolders.

					$OutBuffer[] = "<a href='/browse.php?parent=$ChildID'><img class='browse-subgroup-logo-image' src='$ThumbURL'></a>";
				}
			...
			switch($DisplayMode)
			{
				...
				default:
					...
					foreach($NewChildItemList as $ChildRecord)
					{
						...
						$ChildTitle = urldecode($ChildRecord["item_title"]);
						$ChildItemType = $ChildRecord["_item_type"];
						$ChildSourceType = $ChildRecord["item_source_type"];
						$TreeType = $ChildRecord["item_type"];
						...
						$TitleLink = "<a href='/browse.php?parent=$ChildID' class='aib-browse-record-link'>$ChildTitle</a>";
						if ($TreeType != FTREE_OBJECT_TYPE_LINK)
						{
						...
						{
							switch($ChildRecord["item_source_type"])
							{
								case FTREE_SOURCE_TYPE_STPARCHIVE:
									$LinkInfo = json_decode(urldecode($ChildRecord["item_source_info"]),true);
									if (isset($LinkInfo["type"]) == false)
									{
										$URL = "/records.php?opcode=list&parent=".$ChildRecord["item_id"];
										$ThumbURL = "/images/no_image.png";
										break;
									}
									...
									$URL = "http://".$URL;
									$ThumbURL = "http://".$ThumbURL;
									$OutBuffer[] = "<a class='aib-browse-record-thumb-link' target='_blank' href='$URL'><img class='browse-record-thumb-image' src=\"$ThumbURL\"></a>";
									$TitleLink = "<a href='$URL' target='_blank' class='aib-browse-record-link'>$ChildTitle</a>";
									break;
