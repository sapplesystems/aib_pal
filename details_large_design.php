...
?>
<style>
/* Accordian css start here for item details page 14-July-2023 */

.accordianAIB .btn.btn-link.collapsed{background-color:#15345a;color:#ffffff;text-align:left;}
.accordianAIB .btn.btn-link{background-color:#fcc854;color:#000;text-align:left;}

.accordion-contianer{margin-bottom:10px;}
.accordion {
...
.record_home{color: var(--first-color); cursor: pointer;border: 1px solid var(--first-color);padding: 5px;border-radius: 50%;font-size: 16px;position: absolute;top: 63px;left: 129px;}
.record_info{color: var(--first-color); cursor: pointer;border-radius: 50%;font-size: 22px;position: absolute;top: 66px;left: 165px;}
.pagination-counter {
    text-align:center;
    position:absolute;
    right: -55px;
...
/* Accordian css end  here for item details page 14-July-2023 */
...
if (isset ($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'], 'search.html') !== false) {
...
<script type="text/javascript">
	...
	var pagefolder = '<?php echo isset($_REQUEST['previous'])?$_REQUEST['previous']:"";?>';
		...
		flg = parseInt(flg);
		 /************* Fix Start Issue Id 2143  18-jan-2023 ***************/
        var ref = '<?php echo isset($_REQUEST['item_refrence_id'])?$_REQUEST['item_refrence_id']:""; ?>';
		/************* Fix end Issue Id 2143  18-jan-2023 ***************/
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (result.status == 'success') {
                    ...
                    showPopupMessage('error', result.message + ' (Error Code: 151)');
                }
            },
            error: function () {
                showPopupMessage('error', 'Previous request not completed. (Error Code: 152)');
            }
        });
    }
});
...
function ItemPageNumberSet(page,pagefolder){  
		var folder_id = pagefolder.split(",");  
		  $.ajax({
			...
			error: function () {
				showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 153)');
			}
		});   
	}
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (result.status == 'success') {
                        ...
                        showPopupMessage('error', result.message + ' (Error Code: 154)');
                    }
                },
                error: function () {
                    showPopupMessage('error', 'Previous request not completed. (Error Code: 155)');
                }
            });
	}
});
...
//            });
            $.ajax({
                ...
                error: function () {
                    showPopupMessage('error','Something went wrong, Please try again. (Error Code: 156)');
                }
	}); 
}
 ...
 function getRecordTreedata(folder_id,itemId,share,flg,previousId,ref,detailPagecontent,pageNumber='',pagefolder='',start='',total_item_page='',action='',imgFlg='',searchString = '',recordIdsArr = '',scrapbook_item = ''){
    if(recordIdsArr !='' && typeof(recordIdsArr) != 'undefined'){
		...
		ItemPageNumberSet(pageNumber,pagefolder);
            ...
            error: function () {
                $('.loading-div').hide();
                showPopupMessage('error','Something went wrong, Please try again. (Error Code: 157)');
            }
        });
}
...
/* Fix start for Bug Id 2268   08-feb-2024  */////////	
 function pageMoveToItemDetailOpenNew(id) {
        var page = $('#total_item_page').val(); 
        var current_page = 1;
        if(page != ''){
            current_page = page;
        ...
        var previous_id_obj = '<?php echo $previousId; ?>';
        var queryString = 'folder_id='+id+'&previous=' + previous_id_obj + '&page='+current_page;
        getEncryptedString(queryString, 'item-details.html',1);
    }
/* Fix End for Bug Id 2268   08-feb-2024  */////////		
 function getRecordIds(id){
     $.ajax({
		...
		/************* Fix end Issue Id 2143  18-jan-2023 ***************/
            ...
            error: function () {
                showPopupMessage('error','Something went wrong, Please try again. (Error Code: 158)');
            }
	}); 
 }  
 function imageLoad(){
...
//    if(pr_file_id !=''){
        ...
        error: function () {
            showPopupMessage('error','Something went wrong, Please try again. (Error Code: 159)');
        }
   });
//    }
...
var GlobalPrevAvailFlag = false;
var GlobalNextAvailFlag = true;
