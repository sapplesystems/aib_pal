...
$RawFolderID = $_REQUEST["folder-id"];

$MatchSet = array();

preg_match("/^[0-9]+/",$RawFolderID,$MatchSet);

$CleanFolderID = $MatchSet[0];


$folder_id = $CleanFolderID;//$_REQUEST['folder-id'];
...
switch ($societyTemp) {
   /*  case "custom1":
    ...
    case "custom2":
        ...
        break;
    case "custom3":
        include_once TEMPLATE_PATH . 'custom3_society.php';
        break;   */
    case "custom4":
	include_once TEMPLATE_PATH . 'custom4_society.php';
	break;
    default:
...
<script type="text/javascript">
    ...
    function getItemPropDetails(archive_id) {
        ...
        if (archive_id != '') {
			...
			$('#archive_title_sub').html(record.item_title);
                    ...
                    if (record.prop_details.society_archive_header_image && record.prop_details.society_archive_header_image !== 'undefined') {
                        $('.society_banner_image').css('background-image', 'url(' + archive_image_url + record.prop_details.society_archive_header_image + ')');
                        $('.SocietyclientLanding').css('background-image', 'url(' + archive_image_url + record.prop_details.society_archive_header_image + ')');
                    } else {
                        //$('.header_baner').css('background-image', 'url(public/images/systemAdmin-header-img.jpg)');
                        //$('.clientLanding').css('background-image', 'url(public/images/systemAdmin-header-img.jpg)');
                        $('.SocietyclientLanding').css('background-image', 'url(public/images/default-header-img.jpg)');
                    }
                ...
                error: function () {
                    showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 297)');
                }
            });
        }
    }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (result.status == 'success') {
                        ...
                        showPopupMessage('error', result.message + ' (Error Code: 298)');
                    }
                },
                error: function () {
                    showPopupMessage('error', 'Previous request not completed. (Error Code: 299)');
                }
            });
        }
    });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (result.status == 'success') {
                    ...
                    showPopupMessage('error', 'error', 'Something went wrong, Please try again. (Error Code: 311)');
                }
                $('.loading-div').hide();
            },
            error: function () {
                showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 312)');
            }
        });
    }
</script> 
