...
function rateLimit($max_requests, $time_window) {
    ...
    if (!isset($_SESSION['rate_limit'][$client_ip])) {
        $_SESSION['rate_limit'][$client_ip] = [];
    }

    // Filter out requests outside of the time window
    $_SESSION['rate_limit'][$client_ip] = array_filter($_SESSION['rate_limit'][$client_ip], function($timestamp) use ($current_time, $time_window) {
        return ($current_time - $timestamp) < $time_window;
    });

    // Check the number of requests within the time window
    if (count($_SESSION['rate_limit'][$client_ip]) >= $max_requests) {
    ...
    $_SESSION['rate_limit'][$client_ip][] = $current_time;
}

// Example usage: limit to 50 requests per 10 seconds
//rateLimit(50, 10);
function isValidCsrfToken($csrf_token) {
    list($token, $timestamp, $hmac) = explode(':', $csrf_token, 3);
	
	 if ((time() - $timestamp) > 120000) {
       return false; // Token has expired
    }

    // Recreate the HMAC based on the token and the secret key
    $calculated_hmac = hash_hmac('sha256', $token.$timestamp, SECRET_KEY);

    // Compare the provided HMAC with the calculated one
    return hash_equals($hmac, $calculated_hmac);
}
$headers = getallheaders(); // Gets all headers
$csrf_token = isset($headers['X-CSRF-Token']) ? $headers['X-CSRF-Token'] : null;
if (($csrf_token && isValidCsrfToken($csrf_token)) or $_REQUEST['mode']=='get_state_country') {
...
//Fix End for Issue ID 2390 on 27-Aug-2024
function log_debug_message($Msg) {
    $Handle = fopen("./services_debug.txt", "a+");
    if ($Handle !== false) {
...
function getItemParentProperties($folder_id) {
    ...
    if (isset($apiResponse['info']['records'][1]['item_id'])) {
        ...
        $response = aibServiceRequest($postData2, 'browse');
    }
    return $response;
}
...
switch ($_REQUEST['mode']) {
	...
	case 'regenerate_token':
		...
		echo $token . ':' . $timestamp . ':' . $hmac;
		break;
	//Fix End for Issue ID 2390 on 27-Aug-2024	
    case 'send_test_mail':
        $claim_email_template = file_get_contents(EMAIL_PATH . "/claim_registration.html");
        $claim_email_data['to'] = BUSINESS_EMAIL;
        $claim_email_data['from'] = 'sappletestid@gmail.com';
        $claim_email_data['reply'] = 'sappletestid@gmail.com';
		/******************** Fix Start SS 2324 07-Sep-2023 ************/
        $claim_email_data['subject'] = 'ArchiveInABox -- ';
		/******************** Fix End SS 2324 07-Sep-2023 ************/
        $claim_email_template = str_replace('#username#', 'test user', $claim_email_template);
        $claim_email_template = str_replace('#society_name#', 'test society', $claim_email_template);
        $email = sendMail($claim_email_data, $claim_email_template);
        echo '<pre>';
        print_r($email);
        echo '</pre>';
        break;
    ...
    case 'check_user_exist':
        ...
        if ($_POST['register_username1']) {
            $postDataLogin = array(
                "_key" => APIKEY,
                "_session" => $sessionKey,
                "_op" => 'get_profile',
                "_user" => 1,
                "user_login" => $_POST['register_username1']
            );
            // Service request to get user profile
    ...
    case 'get_google_ad_script':
        ...
        if ($item_id) {
            ...
            $PostData = array(
                ...
                "_session" => $sessionKey,
                "_user" => "1",
                "_op" => "advert_visible_ads",
                "item_id" => $item_id,
            );
            $advertisement = aibServiceRequest($PostData, "advertisement");
            $ad_count = count($advertisement['info']['records']);
            $treeDataArray = getTreeData($item_id);
            $root_parent_id = $treeDataArray[1]['item_id'];
            $response = getItemDetailsWithProp($root_parent_id);
			
            $response = array(
                'status' => 'OK', 
                'google_ad_script' => $response['properties']['google_ad_script'],
				'switch_off_all_adds' => $response['properties']['switch_off_all_adds'],
                'top_left_position' => $response['properties']['top_left_position'],
                'top_right_position' => $response['properties']['top_right_position'],
                'bottom_left_position' => $response['properties']['bottom_left_position'],
                'bottom_right_position' => $response['properties']['bottom_right_position'],
                'custom_ad_count' => $ad_count,
                'default_ad_script' => GOOGLE_AD_SCRIPT,
                'ad_display_type' => ($apiResponse['info']['records']['ad_display_type']) ? $apiResponse['info']['records']['ad_display_type'] : '1',
                    ...
                    if ($apiUserProp['status'] == 'a' || $userStatus == 'a') {
                        if ($archiveStatus) {
                            ...
                            if ($apiResponse['status'] == 'OK') {
								...
								// term and condition versioning start here
								$tc_data = array(
									'_key' => APIKEY,
									'_user' => 1,
									'_op' => 'get_item_prop',
								...
								$tc_resp = aibServiceRequest($tc_data, 'browse');
								if($tc_resp['status'] == 'OK'){
									$tnc_version = $tc_resp['info']['records']['tnc_version'];
									if(empty($tnc_version)){
										$tnc_version = 1;
									}
									$user_info_resp = $apiResponse;
                                    // echo "<pre>";print_r($user_info_resp);
                                   
									if($user_info_resp['status'] == 'OK'){
										$accept_term_service = $user_info_resp['info']['properties']['accept_term_service'];
										if(!empty($accept_term_service)){
											$decoded_accept_term_service = urldecode($accept_term_service);
											$decoded_accept_term_service_arr = json_decode($decoded_accept_term_service, true);
										}
									}
									if(array_key_exists('terms_and_conditions_'.$tnc_version,$decoded_accept_term_service_arr)){
										$terms_and_conditions_up_to_date = 0;
										// do nothing
									}else{
										$terms_and_conditions_up_to_date = 1;
									}

                                    if(array_key_exists('security_question_answers',$user_info_resp['info']['properties'])){
										$accept_security_questions = 0;
										// do nothing
									}else{
										$accept_security_questions = 1;
									}
								}
								// term and condition versioning end here
								...
								$_SESSION['aib']['user_data']['terms_and_conditions_up_to_date'] = $terms_and_conditions_up_to_date;
                                $_SESSION['aib']['user_data']['accept_security_questions'] = $accept_security_questions;

                    
                                
                                if(!array_key_exists('security_question_answers', $apiResponse['info']['properties']))
                                {
                                    ...
                                    $apiResponseQuestions = aibServiceRequest($postData, 'browse');

                                    $apiResponse['info']['question_list'] = $apiResponseQuestions['info']['records'];

                                }
                               
                            }
                            $responseData = array('status' => 'success', 'message' => 'Login successfully' ,'data'=>$apiResponse['info']);
                        } else {
    ...
    case 'list_tree_items':
		...
		$item_in_folderid=isset($_POST['itemId']) ? $_POST['itemId'] : "";
        ...
        if (isset($treeDataArray[1])) {
            ...
            $_SESSION['archive_details_image'] = (!empty($treeDataArray[1]['properties']['archive_details_image'])) ? ARCHIVE_IMAGE . $treeDataArray[1]['properties']['archive_details_image'] : '';
		...
		if(isset($itemData['item_title']) and $itemData['item_title']!=''){
			$currentItemTitle=$itemData['item_title'];
		}
		else{
			$currentItemTitle='';
		}
		...
		$shortBy='TITLE';
		$_SESSION['shortByASCII']='';
		if(isset($itemData['properties']['sort_by']) && $itemData['properties']['sort_by'] != '' && $itemData['properties']['sort_by']=='ID'){
			$shortBy='ID';
		}
		elseif(isset($itemData['properties']['sort_by']) && $itemData['properties']['sort_by'] != '' && $itemData['properties']['sort_by']=='ID'){
			$shortBy='TITLE';
		}
		elseif(isset($itemData['properties']['sort_by']) && $itemData['properties']['sort_by'] != '' && $itemData['properties']['sort_by']=='ASCII'){
			$shortBy='TITLE';
			$_SESSION['shortByASCII']='ASCII';
			
		}
		
		if(isset($_SESSION['sort_by']) and $_SESSION['sort_by']!=''){
				$sort_by_order=$_SESSION['sort_by'];
			}
       // $shortBy = (isset($itemData['properties']['sort_by']) && $itemData['properties']['sort_by'] != '') ? $itemData['properties']['sort_by'] : 'TITLE';
        ...
        switch ($itemData['item_type']) {
            ...
            case 'RE':
				//Fix start for Issue ID 2147 on 28-Apr-2023
					//Fix start for Issue ID 2369 on 11-Apr-2024
						if(isset($treeDataArray[0]['item_title']) and trim($treeDataArray[0]['item_title'])!='_STDUSER'){

							updateRecordViewCount($folderId,$itemData,$sessionKey,$item_in_folderid);
						}
					//Fix End for Issue ID 2369 on 11-Apr-2024
        ...
        if (isset($apiResponse['info']['records'])) {
            ...
            foreach ($apiResponse['info']['records'] as $key => $dataArray) {
                ...
                //Fix start for Issue ID 2002 on 22-Feb-2023
                if ((!empty($_SESSION['aib']['user_data']) && $_SESSION['aib']['user_data']['user_type'] == 'R') || (!empty($_SESSION['aib']['user_data']) && $_SESSION['aib']['user_data']['user_type'] == 'A' && $_SESSION['aib']['user_data']['user_top_folder'] == $root_parent_id)) {
                ...
                if (isset($dataArray['item_type']) && $dataArray['item_type'] == 'RE') {
                    ...
                    $sheare_user = (!empty($itemProperty['share_user'])) ? json_decode($itemProperty['share_user']) : [];
                ...
                if (isset($dataArray['item_type']) && $dataArray['item_type'] == 'IT') {
                    ...
                    //Fix start for Issue ID 2002 on 22-Feb-2023
                    if (!empty($_SESSION['aib']['user_data']) && $_SESSION['aib']['user_data']['user_type'] == 'R') { //Pass
                    } else if (!empty($_SESSION['aib']['user_data']) && ($_SESSION['aib']['user_data']['user_type'] == 'A' || $_SESSION['aib']['user_data']['user_type'] == 'U') && $treeDataArray[1]['item_id'] == $_SESSION['aib']['user_data']['user_top_folder']) {
        ...
        if (isset($apiResponse['info']['records'])) {
            ...
            foreach ($apiResponse['info']['records'] as $itemKey => $itemDataArray) {
                ...
                //Fix start for Issue ID 2002 on 22-Feb-2023
                $logintopFolder = (!empty($_SESSION['aib']['user_data']) && isset($_SESSION['aib']['user_data']['user_top_folder']) && $_SESSION['aib']['user_data']['user_top_folder'] != '') ? $_SESSION['aib']['user_data']['user_top_folder'] : 0;
        ...
        if ($apiResponse['info']['records'][0]['item_type'] != 'AR') {
            //Fix start for Issue ID 2002 on 22-Feb-2023
            $ebayCheckCondition = (!empty($ebayApiStatus['properties']['ebay_status'])) ? $ebayApiStatus['properties']['ebay_status'] : '';
            //Fix end for Issue ID 2002 on 22-Feb-2023
        }
        // Apply Filter Rules
        // Get item details with all property of ftree item
        //Fix start for Issue ID 2002 on 22-Feb-2023
        $userProfileImage = (!empty($_POST['previousId'])) ? getItemDetailsWithProp($_POST['previousId']) : '';
        //Fix end for Issue ID 2002 on 22-Feb-2023
        ...
        if ($itemData['item_type'] == 'RE') {
            ...
            if ($themeName == 'custom') {
                $item_count_per_page = ITEM_COUNT_PER_PAGE_CUSTOM2;
            } elseif ($themeName == 'custom1') {
                $item_count_per_page = ITEM_COUNT_PER_PAGE_CUSTOM2;
            } else {
                $item_count_per_page = ITEM_COUNT_PER_PAGE_DEFAULT;
            }
            ...
            $totalRow = ($itemArrayKey + 1) / $item_count_per_page;
            if ($totalRow === intval($totalRow)) {
                $start = (intval($totalRow) - 1) * $item_count_per_page;
                $totalPageOfItem = intval($totalRow);
            } else {
                $start = intval($totalRow) * $item_count_per_page;
			...
			$arrRecordsId = array_unique($arrRecordsId ?? []);
        ...
        if ($apiResponse['status'] == 'OK' && $itemData['item_type'] != 'RE') {
            ...
            $themeName = isset($treeDataArray[1]['properties']['details_page_design']) ? $treeDataArray[1]['properties']['details_page_design'] : '';
			/*<!----------- Issue Id 2379 Fix Start date 22-may-2024--------------->*/
	/*if(isset($_SESSION['shortByASCII']) and $_SESSION['shortByASCII']=='ASCII'){
    array_multisort( array_column($apiResponse['info']['records'], "item_title"), SORT_ASC,SORT_NATURAL, $apiResponse['info']['records']);
	}*/
	/*<!----------- Issue Id 2379 Fix End date 22-may-2024--------------->*/
			if(isset($_SESSION['sort_by']) and $_SESSION['sort_by']!=''){
				$sort_by_order=$_SESSION['sort_by'];
		//echo strtok($my_string, "1935 test brijesh"); 
		if($_SESSION['sort_by']=='a-z'){
			array_multisort( array_column($apiResponse['info']['records'], "item_title"), SORT_ASC,SORT_STRING, $apiResponse['info']['records']);
		}
		elseif($_SESSION['sort_by']=='z-a'){
			array_multisort( array_column($apiResponse['info']['records'], "item_title"), SORT_DESC,SORT_STRING, $apiResponse['info']['records']);
		}
		elseif($_SESSION['sort_by']=='1-9'){
			array_multisort( array_column($apiResponse['info']['records'], "item_tiele_first_word"), SORT_ASC,SORT_NATURAL, $apiResponse['info']['records']);
		}
		elseif($_SESSION['sort_by']=='9-1'){
			array_multisort( array_column($apiResponse['info']['records'], "item_tiele_first_word"), SORT_DESC,SORT_NATURAL, $apiResponse['info']['records']);
		}
	}
	elseif(isset($_SESSION['shortByASCII']) and $_SESSION['shortByASCII']=='ASCII'){
	  array_multisort( array_column($apiResponse['info']['records'], "item_title"), SORT_ASC,SORT_NATURAL, $apiResponse['info']['records']);
	}
            ...
            if (isset($_POST['start']) && $_POST['start'] != '') {
                ...
                $themeName = isset($treeDataArray[1]['properties']['details_page_design']) ? $treeDataArray[1]['properties']['details_page_design'] : '';
                $display_record_name = isset($treeDataArray[1]['properties']['display_name']) ? $treeDataArray[1]['properties']['display_name'] : '';
                $is_unclaimed_society = '';
                if ($treeDataArray[1]['properties']['society_for_claim'] && $treeDataArray[1]['properties']['society_for_claim'] == '1') {
                ...
                if ($themeName == 'custom') {
                    ...
                    include_once TEMPLATE_PATH . 'details-custom2.php'; //TEMPLATE_PATH . 'details-custom-content.php';
                }
				elseif ($themeName == 'large_design') {
                    include_once TEMPLATE_PATH . 'details-description_large.php'; //TEMPLATE_PATH . 'details-custom-content.php';
                }
				else {
    ...
    case 'list_tree_items_detail_pagination' :
        ...
        if (isset($_POST['action']) && $_POST['action'] == 'Back') {
            $start = $start - ITEM_COUNT_PER_PAGE_DEFAULT;
        }
		if (isset($_POST['start']) && $_POST['start'] != '' && $_POST['start']>0 && isset($_POST['action']) && $_POST['action'] == 'Next') {
            $start = $start + ITEM_COUNT_PER_PAGE_DEFAULT;
        }
		//echo $start;
    ...
    case 'list_tree_public_items':
        ...
        // Request array to get item tree details of a ftree item   
		 $itemData = getItemData($folderId);
		/*<!----------- Issue Id 2379 Fix Start date 21-May-2024--------------->*/
		$shortBy='TITLE';
		$_SESSION['shortByASCII']='';
		if(isset($itemData['properties']['sort_by']) && $itemData['properties']['sort_by'] != '' && $itemData['properties']['sort_by']=='ID'){
			$shortBy='ID';
       ...
       // $shortBy = (isset($itemData['properties']['sort_by']) && $itemData['properties']['sort_by'] != '') ? $itemData['properties']['sort_by'] : 'TITLE';
		/*<!----------- Issue Id 2379 Fix End date 21-May-2024--------------->*/
        ...
        if (end($treeDataArray)['item_type'] == 'AR') {
			...
			$totalRecords=count($firstTimeDataArray['records']);
            ...
            if (!empty($postDataArray)) {
                if ($postDataArray['user_type'] == 'A') {//society
                    ...
                    if ($apiResponseAG['status'] == 'OK') {
                        ...
                        $userProperty[3]['name'] = 'email_verify';
                        $userProperty[3]['value'] = 'yes';
                        ...
                        if (isset($postDataArray['term_service']) && !empty($postDataArray['term_service'])) {
							...
							if($tc_resp['status'] == 'OK'){
								...
								$terms_and_conditions_accept_array = ['terms_and_conditions_'.$tnc_version => date('Y-m-d H:i:s')];
								array_push($userProperty, ['name' => 'accept_term_service', 'value' => json_encode($terms_and_conditions_accept_array)]);
							}
							// term and condition versioning end here
                        ...
                        $status = 'a';
                        $flg = 1;
                        setUserProfileStatus($apiResponse['info'], $status, $flg);                                                
                        ...
                        if ($apiResponse['status'] == 'OK') {
                            if ($postDataArray['request_type'] == 'CLAIM') {
                                $encrypted_params = encryptQueryString('folder-id=' . $newArchiveId);
                                $link_aib_site = 'http://' . HOST_ROOT_PATH . '/society.html?q=' . $encrypted_params;

                                $claim_email_template = file_get_contents(EMAIL_PATH . "/claim_registration.html");
                                $claim_email_data['to'] = BUSINESS_EMAIL;
                                $claim_email_data['from'] = $postDataArray['register_emailId'];
                                $claim_email_data['reply'] = $postDataArray['register_emailId'];
								/******************** Fix Start SS 2324 07-Sep-2023 ************/
                                $claim_email_data['subject'] = 'ArchiveInABox --  ' . $postDataArray['society_name'] . ' has been Claimed';
								/******************** Fix End SS 2324 07-Sep-2023 ************/
                                $claim_email_template = str_replace('#username#', $postDataArray['register_username'], $claim_email_template);
                                $claim_email_template = str_replace('#society_name#', $postDataArray['society_name'], $claim_email_template);
                                $claim_email_template = str_replace('#historical_society_name#', $postDataArray['society_name'], $claim_email_template);
                                $claim_email_template = str_replace('#contact_name#', $postDataArray['name'], $claim_email_template);
                                $claim_email_template = str_replace('#phone_number#', $postDataArray['phoneNumber'], $claim_email_template);
                                $claim_email_template = str_replace('#confirm_email#', $postDataArray['register_emailId'], $claim_email_template);
                                $claim_email_template = str_replace('#city#', $postDataArray['physicalCity'], $claim_email_template);
                                $claim_email_template = str_replace('#state#', $postDataArray['physicalState'], $claim_email_template);
                                $claim_email_template = str_replace('#website_url#', $postDataArray['websiteURL'], $claim_email_template);
                                $claim_email_template = str_replace('#link_aib_site#', $link_aib_site, $claim_email_template);
                                /* if($Registred_Society_Opt_Out == '1'){
                                  $email = sendMail($claim_email_data, $claim_email_template);
                                  } */
                                $email = sendMail($claim_email_data, $claim_email_template);
                            }
    ...
    case 'logout_user':
        ...
        session_destroy();
        break;
    case 'security_questions_answers':
        ...
        $total_answer = 0;
        foreach ($postDataArray['question_id'] as $index => $questionId) {
            $answer = isset($postDataArray['answer'][$index]) ? $postDataArray['answer'][$index] : '';
            if(!empty($answer)){
                $total_answer = $total_answer+1;
                array_push($userProperty, ['name' => 'question_id_' . ($questionId), 'value' => $questionId]);
                array_push($userProperty, ['name' => 'answer_' . ($questionId), 'value' => $answer]);
            }
            
        }
            array_push($userProperty, ['name' => 'security_question_answers', 'value' => 1]);

           
        if($total_answer < 2){
            $responseData = array('status' => 'error', 'message' => 'Please give atleast 2 answers.');
        }else{
             // Request array to get user profile        
            $postData = array(
                "_key" => APIKEY,
            ...
            $apiResponse = aibServiceRequest($postData, 'users');
            if ($apiResponse['status'] == 'OK') {
                $_SESSION['aib']['user_data']['accept_security_questions'] = 0;
                $responseData = array('status' => 'success', 'message' => 'Security questions and answers set');
            } else {
                $responseData = array('status' => 'error', 'message' => 'Not set security questions and answers');
            }
        }
        print json_encode($responseData);
        break;
       ...
       $postData = array(
        ...
        if ($apiResponseQuestions['status'] == 'OK') {
                $responseData = array('status' => 'success', 'data' => $apiResponseQuestions['info']['records']);
        } else {
                $responseData = array('status' => 'error', 'message' => 'Somthing Error.');
        }
        print json_encode($responseData);
        break;
    ...
    case 'forget_password_email':
        ...
        if ($apiResponse['status'] == 'OK') {
            $answer_match = 'false';
            if($apiResponse['info']['properties']['security_question_answers'] == 1)
            {
                foreach ($postData['question_id'] as $index => $questionId) {
                    $questionKey = 'question_id_' . $questionId;
                    $answerKey   = 'answer_' . $questionId;

                    // Check if keys exist and answer is not empty
                    if (
                        array_key_exists($questionKey, $apiResponse['info']['properties']) &&
                        array_key_exists($answerKey, $apiResponse['info']['properties']) &&
                        trim($apiResponse['info']['properties'][$answerKey]) !== ''
                    ) {
                        // Values from API
                        $questionValueFromApi = $apiResponse['info']['properties'][$questionKey];
                        $answerValueFromApi   = $apiResponse['info']['properties'][$answerKey];

                        // Values submitted from form
                        $submittedAnswer = $postData['answer'][$index];

                        // ✅ Optional: Compare submitted answer with API answer
                        if (urlencode($submittedAnswer) === trim($answerValueFromApi)) {
                            $answer_match = 'true';
                        } else {
                            $answer_match = 'false';
                        }
                        ...
                        $responseData = array('status' => 'error', 'message' => 'You cannot change your passsword.Please set security question and answers first.');
                    }
                ...
                {
                    $url = 'id=' . $apiResponse['info']['user_id'] . '&type=forget';
                    $link =  HOST_PATH . 'thank-you.html?' . urlencode($url);
                    $responseData = array('status' => 'success', 'message' => $link);

                }else{
                    $responseData = array('status' => 'error', 'message' => 'You cannot change your passsword.Your answers not match.');
                }
            }else{
    ...
    case 'fetch_children_tree':
        ...
        $apiResponse = aibServiceRequest($postData, 'browse');
        if ($apiResponse['status'] == 'OK') {
            $html .= '<ul class="aib-nav-tree-ul" id="ul_of_'.$folderId.'">';
            foreach ($apiResponse['info']['records'] as $key => $value) {
                if($value['item_tree_type'] == 'F'){
                    $html .= '<li id="li_' . $value['item_id'] . '" onclick="fetchTreeChildren(event, this, ' . $value['item_id'] . ');"><input type="checkbox" data_type="'.strtolower($value['item_type']).'" value="'.$value['item_id'].'" class="FetchTreeChildrenChk" onclick="fetchTreeChildrenChk(event, this, '.$value['item_id'].');" /> ' . strtoupper($value['item_title']) . '</li>';
                }
            }
            $html .= '</ul>';
        }
    ...
    case 'get_settable_pagenumber':
        $folderid = $_POST['folder_id'];
        $set = 1;
		if(isset($_SESSION['pagination']["page_number_id_" . $folderid]) and $_SESSION['pagination']["page_number_id_" . $folderid]>0)
		{
    ...
    case 'get_society_details':
        ...
        print json_encode($responseArray);
        break;
    case '_get_advertisement':
        $item_id = $_POST['folder_id'];
        $position_for = $_POST['position_for'];
        $home_page = $_POST['home_page'];
        $item_detail_page = $_POST['item_detail_page'];
        $array_rand_val = 2;
        if($item_detail_page && $item_detail_page > 0){
            $array_rand_val = 3;
        }
        $response = array('status' => 'error', 'msg' => '', 'record_id' => '');
        if ($item_id) {
            $apiRequestData = array(
                '_key' => APIKEY,
                '_user' => 1,
            ...
            $apiResponse = aibServiceRequest($apiRequestData, 'browse');
            $get_path_count = count($apiResponse['info']['records']);

            if (isset($apiResponse['info']['records'][1]['item_id'])) {
                $folder_id_parent = $apiResponse['info']['records'][1]['item_id'];
            }
            
            $check_apiRequestData = array(
                '_key' => APIKEY,
                '_user' => 1,
                '_op' => 'get',
                '_session' => $sessionKey,
            ...
            $check_apiResponse = aibServiceRequest($check_apiRequestData, 'browse');
            
            if($check_apiResponse['status'] == 'OK' && $check_apiResponse['info']['records'][0]['item_type'] == 'IT'){
                if (isset($apiResponse['info']['records'][1]['item_id']) && $apiResponse['info']['records'][1]['item_id'] > 1) {
                    $item_id = $apiResponse['info']['records'][$get_path_count - 1]['item_id'];
                }
            }
            ...
            $parent_property = aibServiceRequest($apiRequestData, 'browse');
            ...
            $ad_sorting_order = 0;
            if ($apiResponse['status'] == 'OK') {
                if ($apiResponse['info']['records']['ad_sorting_order']) {
                    $ad_sorting_order = $apiResponse['info']['records']['ad_sorting_order'];
                }
            }
            ...
            $response = aibServiceRequest($PostData, "advertisement");
            if($response['status'] == 'OK'){
                $random_3 = array();
                $array_rand = array_rand($response['info']['records'],$array_rand_val);
                $random_3[] = $response['info']['records'][$array_rand[0]];
                $random_3[] = $response['info']['records'][$array_rand[1]];
                $random_3[] = $response['info']['records'][$array_rand[2]];
                $response['info']['records'] = $random_3;
            }
        }
    ...
    case 'get_advertisement':
        ...
        if ($item_id) {
            ...
            if ($ad_sorting_order == '1') {
                usort($response['info']['records'], function($a, $b) {
                    return $a['ad_sort_order'] - $b['ad_sort_order'];
                });
                if($position_for == 'top_left'){
                    include_once TEMPLATE_PATH . 'advertisement1.php';
                }else if($position_for == 'top_right'){
                    include_once TEMPLATE_PATH . 'advertisement2.php';
                }else if($position_for == 'bottom_left'){
                    include_once TEMPLATE_PATH . 'advertisement3.php';
                }else if($position_for == 'bottom_right'){
                    include_once TEMPLATE_PATH . 'advertisement4.php';
                }else if($position_for == 'bottom_right2'){
                    include_once TEMPLATE_PATH . 'advertisement5.php';
                }
            } else {
                if($position_for == 'top_left'){
                    shuffle($response['info']['records']);
                    include_once TEMPLATE_PATH . 'advertisement1.php';
                }else if($position_for == 'top_right'){
                    shuffle($response['info']['records']);
                    include_once TEMPLATE_PATH . 'advertisement2.php';
        ...
        break;
    case 'get_advertisement_flip':
        $item_id = $_POST['folder_id'];
        $position_for = $_POST['position_for'];
        $ad_record = $_POST['ad_record'];
        $flag = $_POST['flag'];
        $flag++;
        $response = array('status' => 'error', 'msg' => '', 'record_id' => '');
        if ($item_id) {
    ...
    case 'get_item_complete_details':
        ...
        if ($item_id) {
            ...
            $apiResponse = aibServiceRequest($postData, 'browse');
			// Fix Start SS Bug Id 2318  13-Sep-2023/////////////////
			if($apiResponse['info']['records'][0]['item_type'] == 'RE'){
				 $postData = array(
                    "_key" => APIKEY,
                    "_session" => $sessionKey,
                    "_user" => 1,
                    "_op" => "list",
                    "parent" => $apiResponse['info']['records'][0]['item_id'] 
                );
                // Service request to get item details of ftree item    
                $apiResponseItems = aibServiceRequest($postData, 'browse');
			}
			// Fix End SS Bug Id 2318  13-Sep-2023/////////////////
    ...
    case 'submit_request':
			/*** Fix start for issue Id 2392 22-07-2024 fixed by SS **********/
		if(isset($_SESSION['aib']['user_data']['user_id']) and $_SESSION['aib']['user_data']['user_id']>0){
        // Parse $_POST['formData'] in to $postData variable          
        parse_str($_POST['formData'], $postData);
        ...
        print json_encode($responseArray);
			  ...
			  $responseArray = ['status' => 'error', 'message' => 'Please login first to submit trouble ticket.'];
			 print json_encode($responseArray);
		}
		/*** Fix End for issue Id 2392 22-07-2024 fixed by SS **********/	
        break;
...
//            $userArchive = getItemData($folder_id);
    ...
    case 'search_data':
        ...
        $search_text = base64_decode(!empty($_POST['search_text']) ? $_POST['search_text'] : "");
        /******** Fix start  0002356  3-jan-2024 ************/
		$search_text = str_replace(array("\/"), '', $search_text);
		/******** Fix end  0002356  3-jan-2024 ************/
        ...
        $search_folder_id = !empty($_POST['search_folder_id']) ? $_POST['search_folder_id'] : '';
        $define_search_ids = !empty($_POST['define_search_ids']) ? $_POST['define_search_ids'] : '';
		$chat_request = base64_decode(!empty($_POST['chat_request']) ? $_POST['chat_request'] : '');
        $previousYouWere = getTreeData($current_item_id);
        unset($previousYouWere[0]);
        $apiResponse = "";
        log_debug(date("Y-m-d H:i:s")." ".__LINE__." ".var_export($_POST,true));
        $normChatKey = normalizeChatQuery($chat_request);
        if (($search_text || $chat_request) && $archive_id) {
            // Request array to get search  data  
            //Fix start for Issue ID 2149 on 14-Feb-2023  
			//unset($_SESSION['chat_request']);
			//unset($_SESSION['llmChat']);
		    //	unset($_SESSION['archive_id']);
			/*echo '<pre>';
			print_r($_SESSION);
			echo '</pre>';
			echo '=='.$chat_request;*/
			if(ENABLE_LLM_CHAT_CACHE =="Y" && !empty($chat_request) && isset($_SESSION['llm_chat_cache'][$archive_id.'_'.$normChatKey]['html'])){
                ...
                "define_search_ids" => $define_search_ids,
				"chat_request"=>''
            ); 
			}
			else{
            ...
            log_debug(date("Y-m-d H:i:s")." ".__LINE__." API RESPONSE ".var_export($apiResponse,true));
        }
		//file_put_contents('/mnt/data/stparch/virtual_sites/www.archiveinabox.com/serach_result'.time().'.xml',$apiResponse);
	    //	$apiResponse=file_get_contents('/mnt/data/stparch/virtual_sites/www.archiveinabox.com/serach_result1750744532.xml');
		/******** Bug Id 0002147 fix start 18-May-2023  ********************/
        $dom = new DomDocument();
        $DomSuccess = $dom->loadXML($apiResponse);
        // DEBUGGING
        if ($DomSuccess == false)
        {
            log_debug("ERROR processing API response for search");
            log_debug($apiResponse);
        }
        // echo "<pre>";print_r($dom);die;
        $searchResultResponse = array();
		$totalResultCount=0; $llmRequestId = null;
        foreach ($dom->getElementsByTagName('hitcount') as $hitcountVal) {
        // print_R($hitcountVal);
            $totalResultCount=$hitcountVal->nodeValue;
        }
        // Extract <llm_request_id>
        foreach ($dom->getElementsByTagName('llm_request_id') as $llmNode) {
            $llmRequestId = $llmNode->nodeValue;
        }
        
        // Check for cached final chat result before rendering template
        $cachedChatHtml = '';
        // unset($_SESSION['llm_chat_cache']);
        if (ENABLE_LLM_CHAT_CACHE =="Y" && !empty($chat_request) && isset($_SESSION['llm_chat_cache'][$archive_id.'_'.$normChatKey]['html'])) {
            $cachedChatHtml = $_SESSION['llm_chat_cache'][$archive_id.'_'.$normChatKey]['html'];
        }
	    ...
	    foreach ($dom->getElementsByTagName('doc') as $item) {
            ...
            for ($i = 0; $i < $item->childNodes->length; ++$i) {
                ...
                if ($child->nodeType == XML_ELEMENT_NODE) {
					...
					if($child->tagName=='snippet'){
						 for ($j = 0; $j < $child->childNodes->length; ++$j) {
							$childNext = $child->childNodes->item($j);
							
							 if( $preTagName==$childNext->tagName ){
							
							
								 if($childNext->tagName=='sniptext')
								 	{
									
									  $nodeValuedata .=' . . . '.$childNext->nodeValue;}
								 else
									{ $nodeValuedata .=$childNext->nodeValue;}
							 }
							 else{
							 ...
							 if($childNext->tagName!='')
								$preTagName=$childNext->tagName;
							
						 }
					}
					
					if($nodeValuedata!=''){
                        $child->nodeValue = htmlspecialchars($nodeValuedata, ENT_XML1 | ENT_QUOTES, 'UTF-8');
						// $child->nodeValue = $nodeValuedata;//$child->nodeValue;
					}
			...
			//echo '<pre>';print_R($searchResultData);
            $searchResultResponse[] = $searchResultData;
        }
	    /******** Bug Id 0002147 fix end 18-May-2023  ********************/
		
	/*	if(isset($_SESSION['chat_request']) and $_SESSION['chat_request']==$chat_request and isset($_SESSION['archive_id']) and $_SESSION['archive_id']==$archive_id){
			//echo 'session data';
			$apiResponseResultllmSearch=$_SESSION['apiResponseResultllmSearch'];
			$resultllmScoreArray=$_SESSION['resultllmScoreArray'];
			$snipTextllmDataArray=$_SESSION['snipTextllmDataArray'];
			$resultIDllmArray=$_SESSION['resultIDllmArray'];
			$llmChat=$_SESSION['llmChat'];
            ...
            //echo 'api data';
            $llmChat=array();
            // echo '<pre> Pre-filter response';print_R($searchResultResponse);echo '</pre>';
            if (!empty($searchResultResponse)) {
                // Used foreach loop over api response to check conditions  
                foreach ($searchResultResponse as $key => $searchDataArray) {
                    
                    if(isset($searchDataArray['llmchat']) and $searchDataArray['llmchat']=='Y'){
                        $llmChat[]=$searchDataArray;
                        unset($searchResultResponse[$key]);
                    }
                    elseif(isset($searchDataArray['llmhit']) and $searchDataArray['llmhit']=='Y'){
                        $llmChat[]=$searchDataArray;
                        unset($searchResultResponse[$key]);
                    }
                }
            ...
            if(count($llmChat))
            {
                $totalResultCount= $totalResultCount-count($llmChat);
                
                foreach ($llmChat as $key => $llmDataArray) {
                    
                    if(isset($llmDataArray['llmhit'])){ 
                        $resultIDllmArray[] = $llmDataArray['uri'];
                        $resultllmScoreArray[$llmDataArray['uri']] = $llmDataArray;
                        $snipTextllmDataArray[$llmDataArray['uri']] = $llmDataArray['snippet'];
                    }	
                }
                    $resultIDllString = implode(',', $resultIDllmArray);
                    $postData = array(
                            "_key" => APIKEY,
                            "_session" => $sessionKey,
                            "_user" => 1,
                            ...
                            "opt_get_property" => 'Y',
                            "opt_get_long_prop" => 'Y',
                            "item_list" => $resultIDllString
                    );
            
                    $apiResponseResultllmSearch = aibServiceRequest($postData, 'browse'); 
                if ($apiResponseResultllmSearch['status'] == 'OK') {
                    ...
                    foreach ($apiResponseResultllmSearch['info']['records'] as $key => $result) {
                        //echo $result['item_id'];
                        
                        $itemParents = getTreeData($result['item_id']);
                        unset($itemParents[0]);
                        array_pop($itemParents);
                        $apiResponseResultllmSearch['info']['records'][$key]['item_location'] = $itemParents;
                        if (in_array($result['item_id'], $visitedItem)) {
                            $apiResponseResultllmSearch['info']['records'][$key]['link_visited'] = 'yes';
                        }
                        if(isset($resultllmScoreArray[$result['item_id']]['llmchat'])){
                            $apiResponseResultllmSearch['info']['records'][$key]['llmchat'] = 'Y';
                        }
                        if(isset($resultllmScoreArray[$result['item_id']]['snippet'])){
                            $apiResponseResultllmSearch['info']['records'][$key]['snippet'] =$resultllmScoreArray[$result['item_id']]['snippet'];
                        }
                        if(isset($resultllmScoreArray[$result['item_id']]['llmhit'])){
                            $apiResponseResultllmSearch['info']['records'][$key]['llmhit'] = 'Y';
                        }
                        if (isset($result['properties']['aib:private']) and $result['properties']['aib:private'] == 'Y') {
                            unset($apiResponseResultllmSearch['info']['records'][$key]);
                            continue;
                        ...
                        $path ='';
                        $pathTitle='';
                        foreach ($itemParents as $keyP => $resultP) {
                            if( $resultP['item_type']!='RE'){
                                $path .=$resultP['item_id'].'-';
                                $pathTitle .=$resultP['item_title'].'-';
                            }
                        }
                        $apiResponseResultllmSearch['info']['records'][$key]['path']=$path;
                        $apiResponseResultllmSearch['info']['records'][$key]['pathTitle']=$pathTitle;
                    }

                    //Fix start for Issue ID 2149 on 17-March-2023 sorting
                    array_multisort(array_column($apiResponseResultllmSearch['info']['records'], 'pathTitle'), SORT_ASC, $apiResponseResultllmSearch['info']['records']);
                    //Fix end for Issue ID 2149 on 17-March-2023
                
                //  echo '<pre>============';print_r($apiResponseResultllmSearch['info']['records']);echo '</pre>';die;
                    
                    if(count($apiResponseResultllmSearch)){
                        $_SESSION['apiResponseResultllmSearch']=$apiResponseResultllmSearch;
                        $_SESSION['resultllmScoreArray']=$resultllmScoreArray;
                        $_SESSION['snipTextllmDataArray']=$snipTextllmDataArray;
                        $_SESSION['resultIDllmArray']=$resultIDllmArray;
                        $_SESSION['chat_request']=$chat_request;
                        $_SESSION['archive_id']=$archive_id;
                        $_SESSION['llmChat']=$llmChat;
		...
		// echo '<pre> llmChat content ';print_R($llmChat);echo '</pre>';die;
        //	echo '<pre> filtered searchResultResponse ';print_R($searchResultResponse);echo '</pre>';//die;
        ...
        log_debug("postData for browse data is ".var_export($postData,true));
            // Service request to get item list of ftree parent item  
	    $apiResponseResultSearch = aibServiceRequest($postData, 'browse');
        log_debug("response for browse data is ".var_export($apiResponseResultSearch,true));
            if ($apiResponseResultSearch['status'] == 'OK') {
                ...
                foreach ($apiResponseResultSearch['info']['records'] as $key => $result) {
                    ...
                    $apiResponseResultSearch['info']['records'][$key]['path']=$path;
                    $apiResponseResultSearch['info']['records'][$key]['pathTitle']=$pathTitle;
                }

                //Fix start for Issue ID 2149 on 17-March-2023 sorting
                array_multisort(array_column($apiResponseResultSearch['info']['records'], 'pathTitle'), SORT_ASC, $apiResponseResultSearch['info']['records']);
                //Fix end for Issue ID 2149 on 17-March-2023
               
                //echo '<pre>';print_r($apiResponseResultSearch['info']['records']);echo '</pre>';die;

            }
        }
        
        $totalStatndardResultCount = !empty($apiResponseResultSearch['info']['records'])?count($apiResponseResultSearch['info']['records']):0;
	    	// echo '<pre>';print_r($apiResponseResultSearch['info']['records']);echo '</pre>';
        // search_result_data.php file has been included here to show search data
        include_once TEMPLATE_PATH . 'search_result_data.php';
        break;
        
    case 'get_llm_stream_poll':
        $llm_request_id = $_POST['llm_request_id'] ?? '';
        $archive_id = $_POST['archive_id'] ?? '';
        $chat_request = $_POST['chat_request'] ?? '';
        $demo = $_POST['demo'] ?? '';

        if (!empty($llm_request_id)) {
            $searchLlmData = [
                "_session" => time() - 100,
            ...
            if($demo == 1)
            {
                $apiResponse = llmStreamDemo($searchLlmData);
            }else{
                $apiResponse = aibSearchLlmStream($searchLlmData);
            }
    ...
    case 'get_item_details':
        $item_id = $_POST['item_id'];
		updateItemViewCount( $item_id,$sessionKey);
        if ($item_id) {
            // Get item details of ftree item            
    ...
    case 'get_archive_search_items':
        ...
        foreach ($records as $record) {
            ...
            if ($record['fullProp']['status'] == 1) {
                $state = (!empty($record['fullProp']['archive_display_state'])) ? ucwords(strtolower($record['fullProp']['archive_display_state'])) : '';
                $county = (!empty($record['fullProp']['archive_display_county'])) ? ucwords(strtolower($record['fullProp']['archive_display_county'])) : '';
                $city = (!empty($record['fullProp']['archive_display_city'])) ? ucwords(strtolower($record['fullProp']['archive_display_city'])) : '';
                $zip = (!empty($record['fullProp']['archive_display_zip'])) ? ucwords(strtolower($record['fullProp']['archive_display_zip'])) : '';
        ...
        if ($id) {
            ...
            if ($username) {
                $apiRequestData = array(
                    ...
                    "_user" => 1,
                    "obj_id" => $item_id,
                    "propname_1" => 'temp_claim_data_' . $username
                );
                $apiResponse = aibServiceRequest($apiRequestData, 'browse');
            }
            if ($item_id) {
                ...
                $apiResponse = aibServiceRequest($apiRequestData, 'browse');
                $search = 'temp_claim_data_';
                $itme_prop = $apiResponse['info']['records'];
                $count = 0;
                foreach ($itme_prop as $k => $v) {
                ...
                if ($count == 0) {
                    $apiRequestDataItem = array(
                        ...
                        'obj_id' => $item_id,
                        'opt_long' => 'Y',
                        'propname_1' => 'society_for_claim',
                        'propval_1' => '1',
                    );
                    aibServiceRequest($apiRequestDataItem, 'browse');
                }
            }
        } else {
            $responseArray = ['status' => 'error', 'message' => 'error'];
        ...
        if ($archive_user_id != $temp_user_created_by_admin && $flg == 'Y') {
            ...
            if ($temp_data_count && $flg == 'Y') {
                ...
                if (is_array($temp_claim_admin_data_arr) && $temp_claim_admin_data_arr_count > 0) {
                    ...
                    $postRegArray['propname_' . $count] = 'approved_date';
                    $postRegArray['propval_' . $count] = '';
                    $count++;
            ...
            if ($user_property) {
                if ($status == 'Y') {
                    ...
                    $apiRequestDataItem = array(
                        ...
                        'propname_3' => 'approved_date',
                        'propval_3' => date('Y-m-d'),
                    );
                    $apiResponse = aibServiceRequest($apiRequestDataItem, 'browse');
					...
					/******************** Fix End SS 2324 07-Sep-2023 ************/
                    /* $mail_opt_out = getMailOptOut();
                      if($mail_opt_out['status'] == 'OK'){
                      $User_Approved_Opt_Out = $mail_opt_out['info']['records']['User_Approved'];
                      }
                      if($User_Approved_Opt_Out == '1'){
                      $email = sendMail($email_data, $email_template);
                      }else{
                      $email = 1;
                      } */
    ...
    case 'share_item':
        ...
        print json_encode($responseArray);
			  ...
			  $responseArray = ['status' => 'error', 'message' => 'Please login first to share item.'];
			 print json_encode($responseArray);
		}
		/*** Fix End for issue Id 2392 22-07-2024 fixed by SS **********/	
        break;
...
//                        $dataArray=getItemData($relatedDataArray['item_parent']);
    ...
    case 'get_historical_connection_item':
        ...
        $relatedItemsData = getHistoricalConnectionLists($record_id);
        $parentRelatedItems = getAllParentsRelatedItems($record_id);
        $parent_properties = getItemParentProperties($record_id);
        ...
        $relatedItemsData = $newFillteredArray;
        // related_items.php file has been included here to show related items
        if ($parent_properties['status'] == 'OK') {
            $custom_template = $parent_properties['info']['records'][0]['properties']['custom_template'];
        }
        if ($custom_template == 'custom2') {
            include_once TEMPLATE_PATH . 'historical_connection_item_custom2.php';
        } else if ($custom_template == 'custom1') {
            include_once TEMPLATE_PATH . 'historical_connection_item_custom1.php';
        } else {
            include_once TEMPLATE_PATH . 'historical_connection_item_default.php';
        }
        break;
    case 'get_historical_connection_item_details':
        ...
        if ($themeName == 'custom') {
            ...
            include_once TEMPLATE_PATH . 'custom2_historical_connection_item_details.php';
        } else {
            include_once TEMPLATE_PATH . 'default_historical_connection_item_details.php';
        }
        break;
    ...
    case'update_user_term_condition':
		...
		if($tc_resp['status'] == 'OK'){
			...
			if(array_key_exists('terms_and_conditions_'.$tnc_version,$decoded_accept_term_service_arr)){
				...
				$terms_and_conditions_accept_array = [];
				foreach($decoded_accept_term_service_arr as $key => $val){
					$terms_and_conditions_accept_array[$key] = $val;
				}
				$terms_and_conditions_accept_array['terms_and_conditions_'.$tnc_version] = date('Y-m-d H:i:s');
				array_push($userProperty, ['name' => 'accept_term_service', 'value' => json_encode($terms_and_conditions_accept_array)]);
			}
        ...
        if ($apiResponseProfile['status'] == 'OK') {
			...
			$_SESSION['aib']['user_data']['terms_and_conditions_up_to_date'] = 0;
            ...
            $apiResponse = aibServiceRequest($postData, 'browse');
            $responseArray = ['status' => 'success', 'message' => $apiResponse['info']['records']];
        }
        print json_encode($responseArray);
        break;
        ...
        if ($item_id != '') {
            ...
            if($apiResponse['status'] == 'OK'){
                ...
                for($x = 0; $x < count($records); $x++){
                    if($records[$x]['item_type'] == 'RE'){
                        $record_item_id = $records[$x]['item_id'];
                        break;
                    }
                }
            }
            ...
            if (!empty($apiResponse['info']['records'])) {
                foreach ($apiResponse['info']['records'] as $dataArray) {
                    if ($loggedInUser != $dataArray['user_id']) {
                        //Fix start for Issue ID 2002 on 22-Feb-2023
                        $_email = (!empty($dataArray['_properties']['email'])) ? $dataArray['_properties']['email'] : '';
                        $societyAdminListArray[$dataArray['user_id']] = $dataArray['user_login'] . '(' . $_email . ')';
                        //Fix end for Issue ID 2002 on 22-Feb-2023
                    }
                }
            }
...
");
	...
	case 'image_highlight_with_dimenstion':
        $item_id = $_POST['item_id'];
        $imageHeight = $_POST['imageHeight'];
        $imageWidth = $_POST['imageWidth'];
		$searchString = $_POST['searchString'];
		
		echo $res=getImageHighlightWithDimenstion($item_id,$imageHeight,$imageWidth,$searchString);
        break;	
	/**********************Fix end for Issue ID 0002031  13-Oct-2022*************************************************************/
		
	/**********************Fix start for Issue ID 0002150  27-Jan-2023***********************************************************/	
	case 'item_get_download_enable':
		$res=getItemData($_REQUEST['item_id']);
		if(isset($res['properties']['enable_disable_download']) and $res['properties']['enable_disable_download']==1)
		{
	...
	/**********************Fix Start for Issue ID 0002319  27-Sep-2023**********************************************************/		
	case 'updateRecStreetView':
		$item_id= $_REQUEST['item_id'];
		$streetView= $_REQUEST['streetView'];
		$sessionKey = $_SESSION['aib']['session_key'];
        $propertyVisible = [
            '_key' => APIKEY,
            '_user' => 1,
            ...
            '_session' => $sessionKey,
            'obj_id' => $item_id,
            'propname_1' => 'streetView',
            'propval_1' => $streetView
        ];
        $apiVisibleResponse = aibServiceRequest($propertyVisible, 'browse');
	break;	
	/**********************Fix end for Issue ID 0002319  27-Sep-2023**********************************************************/	
		
	case 'getMapDataByLocation':
		
		
		/************ Fix start 2371  22-feb-2024 **********************/
		/******************** Get request Itme Society *********************/
		 $req_society_id='';
		...
		$postData = array(
					...
					"_user" => 1,
					"_op" => "get_path",
					"obj_id" => $_REQUEST['parent_id'],
					
				);
				// Service request to get item tree data        
				$apiResponse = aibServiceRequest($postData, 'browse');
				
				if ($apiResponse['status'] == 'OK' and isset($apiResponse['info']['records'][1])) {
				   $req_society_id=$apiResponse['info']['records'][1]['item_id'];
					//$parentData['item_title']=urldecode($parentData['item_title']);
				}
		}
		/************ Fix End 2371  22-feb-2024 **********************/
		$itemrecord_lat= $_REQUEST['itemrecord_lat'];
		$itemrecord_lng= $_REQUEST['itemrecord_lng'];
		$sessionKey = $_SESSION['aib']['session_key'];
        $propertyVisible = [
            '_key' => APIKEY,
            '_user' => 1,
            ...
            '_session' => $sessionKey,
            'radius' =>100,
            'center_y' => $itemrecord_lat,
            'center_x' => $itemrecord_lng,
			'get_properties' =>'Y',
			'get_item_data'=>'Y',
			'context'=>$req_society_id
        ];
		
		
        $apiVisibleResponse = aibServiceRequest($propertyVisible, 'locationsearch');
		//echo '<pre>';print_R( $apiVisibleResponse);die;
		
		if($apiVisibleResponse['status']=='OK'){
			$parentData=array();
			foreach($apiVisibleResponse['records'] as $key=>$record){
				...
				if(empty($parentData)){
						...
						if ($apiResponse['status'] == 'OK' and isset($apiResponse['info']['records'][1])) {
						   $parentData=$apiResponse['info']['records'][1];
							$parentData['item_title']=urldecode($parentData['item_title']);
							$society_id=$apiResponse['info']['records'][1]['item_id'];
						}
					}
				/************ Fix start 2371  22-feb-2024 **********************/
				if($req_society_id>0 and $req_society_id!=$society_id){
					unset($apiVisibleResponse['records'][$key]);
				}
				elseif($society_id==$apiVisibleResponse['records'][$key]['item_id']){
					unset($apiVisibleResponse['records'][$key]);
				}
				else{
				$apiVisibleResponse['records'][$key]['item_title']=urldecode($apiVisibleResponse['records'][$key]['item_title']);
				$apiVisibleResponse['records'][$key]['parentData']=$parentData;
				}	
				/************ Fix End 2371  22-feb-2024 **********************/
		
			}
			//echo '<pre>>';print_r($apiVisibleResponse);
			//echo '<pre>>';print_r($apiVisibleResponse);
			$finalRes=array();
			foreach($apiVisibleResponse['records'] as $record){
				$data=array();
				$data['lat']=$record['location']['lat'];
				$data['lon']=$record['location']['lon'];
				$data['item_title']=$record['item_title'];
				$data['item_id']=$record['item_id'];
				$data['item_parent']=$record['item_parent'];
				$data['item_title_p']=$record['parentData']['item_title'];
				$data['item_title_p_id']=$record['parentData']['item_id'];
				$data['itemrecord_address_city']=$record['properties']['itemrecord_address_city'];
				$data['itemrecord_address_pin_code']=$record['properties']['itemrecord_address_pin_code'];
				$data['itemrecord_address_state']=$record['properties']['itemrecord_address_state'];
				$data['itemrecord_address_line']=$record['properties']['itemrecord_address_line'];
				$data['parent_id']=$record['parentData']['item_id'];
				//$finalRes[$record['location']['lat'].$record['location']['lon']][0]['lat']=$record['location']['lat'];
				//$finalRes[$record['location']['lat'].$record['location']['lon']][0]['lon']=$record['location']['lon'];
				$finalRes[$record['location']['lat'].$record['location']['lon']][]=$data;
			}
			//echo '<pre>>';print_r($finalRes);
			foreach($finalRes as $res){
				$data=[];
				$data['infohtml']='<div onwheel="event.preventDefault();">';
				$parentArray=[];
				foreach($res as $r){
					$data['lat']=$r['lat'];
					$data['lon']=$r['lon'];
					...
					$html='';
					if(!isset($data['item_id_c'])){
						$data['item_id_c']=0;
					}
					if(!in_array($r['item_title_p_id'],$parentArray)){
						$parentArray[]=$r['item_title_p_id'];
						$html='<p><strong style="font-size: 16px;">'.$r['item_title_p'].'</strong></p>';
						$add=$r['itemrecord_address_line'].'<br>'.$r['itemrecord_address_city'].',&nbsp;'.$r['itemrecord_address_state'].',&nbsp;'.$r['itemrecord_address_pin_code'];
							$html .='<p  style="font-size: 14px;">'.$add.'</p><ul>';
							
					}
					if($_REQUEST['item_id_c']==$r['item_id']){
						$data['item_id_c']=$r['item_id'];
					}
					
					$html.='<li style="padding: 5px;"><a  href="javascript:pageMoveToItemDetailOpenNew('.$r['item_id'].')">'.$r['item_title'].'</a></li>';
					$data['infohtml']=$data['infohtml'].$html;
				}
				$data['infohtml'].='</ul></div>';
				$dataRes[]=$data;
			}
			
			//$responseArray = ['status' => 'OK', 'message' => '','data'=>$apiVisibleResponse['records']];
			$responseArray = ['status' => 'OK', 'message' => '','data'=>$dataRes];
		}
		else{
			$responseArray = ['status' => 'error', 'message' => 'Some issue with map api.'];
		}
		 
		
		 print json_encode($responseArray);
	break;
    case 'getDefineTreeData':
		
		$parent_id= $_REQUEST['parent_id'];
		$sessionKey = $_SESSION['aib']['session_key'];
		$OutData=getItemListData( $parent_id,$sessionKey);
		    $tree = [];

    foreach ($OutData['info']['records']	 as $child) {
		
		if(!in_array($child['item_type'],array('IT','RE'))){
			$OutDataInner=getItemListData( $child['item_id'],$sessionKey);
			$childCount=0;
			if(count($OutDataInner['info']['records'])){
				   foreach ($OutDataInner['info']['records']	 as $childInner) {
						if(!in_array($childInner['item_type'],array('IT','RE'))){
							$childCount++;
						}
					}
			}
			if($childCount){
				$tree[] = [
					...
					'text' => $child['item_title'],
					'children' =>true// Recursive call
				];
			}
			else{
				$tree[] = [
...
function getRecordRelatedItems($record_id = null) {
    if ($record_id != null) {
        ...
        $itemDetails = getItemData($record_id);
        //Fix start for Issue ID 2002 on 22-Feb-2023
        $item_user_id = (!empty($itemDetails['item_user_id'])) ? $itemDetails['item_user_id'] : 0;
        // Get user profile by id        
        $userDetails = getUserProfileById($item_user_id);
        $relatedItemsData = [];
        // Request array to get all shared list         
...
function aibSearchRequest($postData) {
    log_debug(date("Y-m-d H:i:s")." ".__LINE__." ".var_export($postData,true));
    // Create a new curl resource
	
	// print_r($postData);die;
	 $postDataItem = array(
            "_key" => APIKEY,
            "_session" => $postData['_session'],
            "_user" => 1,
            "_op" => "search",
		 "equation" => $postData['phrase'],
	 	"api_key" =>APIKEY,
	 	 "session_key" =>$postData['_session'],
	 "skip_to"=>$postData['pagenum'],
	 "output_count"=>$postData['perpage'],
	 "index_name"=>$postData['_indexcfg'],
	 "index_path"=>SEARCH_INDEX_PATH,
	"indexpath" => SEARCH_INDEX_PATH,
	"indexcfg" =>$postData['_indexcfg'],
	 "base_url"=>SEARCH_BASE_URL,
		  "chat_request"=>$postData['chat_request']
    );
    //"index_path"=>'/home/stparch/virtual_sites/aib_indexes',
	//"indexpath" => "/home/stparch/virtual_sites/aib_indexes",
    if (!empty($postData['search_folder_id'])) {
        $postDataItem["attr_1"] = 'item_path STRINC n' . $postData['search_folder_id'] . 'x';
    }

    if (!empty($postData['define_search_ids'])) {
        // Explode the comma-separated values into an array
        $searchIds = explode(',', $postData['define_search_ids']);
    
        // Transform each ID into the required format
        $formattedIds = array_map(function ($id) {
            return "n" . trim($id) . "x";
        }, $searchIds);
    
        // Join them using space with "STROR"
        $postDataItem["attr_1"] = 'item_path STROR ' . implode(' ', $formattedIds);
    }
	...
	$Options = array(
		CURLOPT_POST => 1,
		CURLOPT_HEADER => 0,
		CURLOPT_URL => SEARCH_BASE_CURL_URL,
		CURLOPT_FRESH_CONNECT => 0,
		CURLOPT_RETURNTRANSFER => 1,
		CURLOPT_FORBID_REUSE => 0,
		CURLOPT_TIMEOUT => 1200,
		CURLOPT_POSTFIELDS => http_build_query($postDataItem)
	);
    // CURLOPT_URL => "http://develop.archiveinabox.com/api/hsearch.php",
	curl_setopt_array($CurlObj,$Options);
	$Result = curl_exec($CurlObj);
    log_debug("CURL search response is ".var_export($Result,true));
    ...
    curl_setopt_array($curlObj, $options);
    $resultc = curl_exec($curlObj);
    $resultFinal = str_replace('<status><status_value>OK</status_value></status>', '', $resultc);
    curl_close($curlObj);
 */

	 $Result = json_decode($Result);
	$Result=$Result->info->data;
	$Result = str_replace('<status><status_value>OK</status_value></status>', '', $Result);
	  $Result = str_replace('<document>', '', $Result);
	$Result = str_replace('</document>', '', $Result);
	//echo '<pre>';print_r($postData);print_r($resultFinal);print_r($Result);echo '</pre>';//die;
    return $Result;
}

/**
 * Poll the LLM stream API and format current HTML output
 */
function aibSearchLlmStream($postData)
{
    $archive_id = $postData['archive_id'] ?? null;
    $postDataItem = [
        "_key"           => APIKEY,
        "_session"       => $postData['_session'],
        "_user"          => 1,
        ...
        "api_key"        => APIKEY,
        "session_key"    => $postData['_session'],
        "llm_request_id" => $postData['llm_request_id']
    ];
   ...
   curl_close($curl);
    //    print_r($rawResult);die;
       // $rawResult=trim(file_get_contents('/mnt/data/stparch/virtual_sites/www.archiveinabox.com/get_llm_stream_poll_rawResult_711756984445.txt'));
   // file_put_contents('/mnt/data/stparch/virtual_sites/www.archiveinabox.com/get_llm_stream_poll_rawResult_request_'.rand(1,100).time().'.txt',print_R($postDataItem,true));
	//file_put_contents('/mnt/data/stparch/virtual_sites/www.archiveinabox.com/get_llm_stream_poll_rawResult_'.rand(1,100).time().'.txt',print_R($rawResult,true));
    // echo "<pre>";print_r($rawResult);echo "</pre>";die;
    $decoded = json_decode($rawResult, true);
    $finalHtml = '';
    $SearchText = base64_decode($postData['chat_request']);
    $SearchText = preg_replace("/[\"]/", " ", $SearchText);
    $SearchText = strtolower($SearchText);
    if (!empty($decoded) && isset($decoded['status'])) {
        $status = $decoded['status'];

        if (!empty($decoded['info']) || (!empty($decoded['api_response']['info']))) {
            $infoRaw = $decoded['api_response']['info'] ?? $decoded['info'];

            // Always parse partial text for WORKING and full for DONE
            if (preg_match('/<llm_text>([\s\S]*?)(?:<\/llm_text>|$)/', $infoRaw, $match)) {
                $partialText = trim($match[1]);
                if ($partialText !== '') {
                    $finalHtml .= '<div class="llm-main-text" id="cached-llm-text-data">' . nl2br(htmlspecialchars($partialText)) . '</div>';
                }
            }

            // Parse docs if present (will be partial or full)
            if (preg_match_all('/<llm_doc>([\s\S]*?)<\/llm_doc>/', $infoRaw, $docMatches)) {
                $finalHtml .= '<div class="llm-doc-section" id="cached-references-data"><h4>Chatbot Response References</h4>';
				$result=parsellmRefResponse($rawResult);
                ...
                /*foreach ($docMatches[1] as $doc) {
                    $score = '';
                    $content = '';
                    if (preg_match('/<score>(.*?)<\/score>/', $doc, $s)) {
                        $score = trim($s[1]);
                    }
                    if (preg_match('/<content>([\s\S]*?)<\/content>/', $doc, $c)) {
                        $content = nl2br(htmlspecialchars(trim($c[1])));
                    }
                    $finalHtml .= '<div class="llm-doc">';
                    if ($score !== '') {
                        $finalHtml .= '<div class="llm-doc-score">Score: ' . htmlspecialchars($score) . '</div>';
                    }
                    if ($content !== '') {
						

                        $finalHtml .= '<div class="llm-doc-content">' . $content . '</div>';
                    }
                    $finalHtml .= '</div>';
                }*/
				if(isset($rawResultParse->documents)){
					 ...
					 foreach ($rawResultParse->documents as $doc) {
						 ...
						 if($doc->structured->uri){
							$resultIDllmArray[]=$doc->structured->uri;
							 if(($doc->structured->item_path)){
								 $item_path= str_replace('n','',$doc->structured->item_path);
								 $item_path= str_replace('x','',$item_path);
								 $item_path_arr=explode(' ',$item_path);
								 $resultIDllmArray=array_merge($resultIDllmArray,$item_path_arr);
							 }
                    ...
                    $postData = array(
                            "_key" => APIKEY,
                            "_session" =>  $_SESSION['aib']['session_key'],
                            "_user" => 1,
                            "_op" => "list",
                            "opt_sort" => 'ID',
                            "parent" => -1,
                    ...
                    $apiResponseResultllmSearch = aibServiceRequest($postData, 'browse'); 
					// echo "<pre>";print_r($apiResponseResultllmSearch);die;
                    //file_put_contents('/mnt/data/stparch/virtual_sites/www.archiveinabox.com/apiResponseResultllmSearchpostData'.rand(1,100).time().'.txt',print_R($postData,true));
					//file_put_contents('/mnt/data/stparch/virtual_sites/www.archiveinabox.com/apiResponseResultllmSearch'.rand(1,100).time().'.txt',print_R($apiResponseResultllmSearch,true));
                if ($apiResponseResultllmSearch['status'] == 'OK') {
					 foreach($apiResponseResultllmSearch['info']['records'] as $recordsllm){
						 
						 $recordsllmArray[$recordsllm['item_id']]=$recordsllm;
					 }
					
				}
					//file_put_contents('/mnt/data/stparch/virtual_sites/www.archiveinabox.com/recordsllmArray'.rand(1,100).time().'.txt',print_R($recordsllmArray,true));
				//  echo "<pre>";print_r($recordsllmArray)."</pre>";  
					$finalHtmlNew .='';
				 foreach ($rawResultParse->documents as $doc) {
					//  echo "<pre>";print_r($doc);die;
					 if($doc->structured->ocr_text){
						 ...
						 $file_id='';
							 $item_parent='';
							  $item_id='';
						 if(isset($recordsllmArray[$doc->structured->uri]) and isset($recordsllmArray[$doc->structured->uri]['files'][0])){
							 
							 $file_id=$recordsllmArray[$doc->structured->uri]['files'][0]['file_id'];
							 $item_parent=$recordsllmArray[$doc->structured->uri]['item_parent'];
							  $item_id=$recordsllmArray[$doc->structured->uri]['item_id'];
						 }
						    $item_path= str_replace('n','',$doc->structured->item_path);
								 $item_path= str_replace('x','',$item_path);
								 $item_path_arr=explode(' ',$item_path);
						 $itemPathTitle=array();
                        //  var_dump($item_path_arr);die;
						 if(count($item_path_arr)){
							foreach($item_path_arr as $itemPath){
								
								if($recordsllmArray[$itemPath]['item_type']=='IT' and $recordsllmArray[$itemPath]['item_type']='RE'){
									$pathString=' <li><a class="organizations search_result_clicked" search_item_id="'.$recordsllmArray[$itemPath]['item_id'].'" href="item-details.html?q=' . encryptQueryString('folder_id=' . $recordsllmArray[$itemPath]['item_parent']. '&itemId=' . $recordsllmArray[$itemPath]['item_id']. '&search_text=' . $SearchText).'" target="_blank">'.$recordsllmArray[$itemPath]['item_title'].'</a></li>';
								}
								else{
									$pathString=' <li><a class="organizations search_result_clicked" search_item_id="'.$recordsllmArray[$itemPath]['item_id'].'" href="home.html?q=' . encryptQueryString('folder_id=' . $recordsllmArray[$itemPath]['item_parent']. '&itemId=' . $recordsllmArray[$itemPath]['item_id']. '&search_text=' . $SearchText).'" target="_blank">'.$recordsllmArray[$itemPath]['item_title'].'</a></li>';
								}
								
								if(!in_array($pathString,$itemPathTitle)){
									
									$itemPathTitle[]=$pathString;
								}
							} 
						 }
						 $itemPathTitle=implode('',$itemPathTitle);
                           // Truncate OCR text to 100 words
                            $truncatedOcrText = truncateToWords($doc->structured->ocr_text, LLM_SEARCH_WORD_LIMIT); 
						   
                            $finalHtml .= '<div class="llm-doc col-md-12">
						    <div class="col-md-12 search-result"><ul class="listing search_data_listing" style="margin-top:0px;">'.$itemPathTitle.'</ul></div>
						    <div class="col-md-1"></div>
						   <div class="col-md-2">
                                <a class="organizations search_result_clicked" search_item_id="'.$item_id.'" href="item-details.html?q=' . encryptQueryString('folder_id=' . $item_parent. '&itemId=' . $item_id. '&search_text=' . $SearchText).'" target="_blank">
                                    <img class="searchResultImg" src="' . THUMB_URL . '?id='.$file_id.'" alt="">
                                </a>
                            </div>
						   <div class="col-md-9">';
						 if ($doc->structured->title !== '') {
								$finalHtml .= '  <div class="marginTopBottom10">
                                    <a class="organizations search_result_clicked" search_item_id="'.$item_id.'" href="item-details.html?q=' . encryptQueryString('folder_id=' . $item_parent . '&itemId=' . $item_id. '&search_text=' . $SearchText).'" target="_blank"><span style="color: #2698de;font-size: 16px;">' . htmlspecialchars($doc->structured->title).' </span></a>
							...
							if ($doc->structured->score !== '') {
								...
								$finalHtml .= '<div class="llm-doc-content"><a class="organizations search_result_clicked" search_item_id="'.$item_id.'" href="item-details.html?q=' . encryptQueryString('folder_id=' . $item_parent . '&itemId=' . $item_id. '&search_text=' . $SearchText).'">' . htmlspecialchars($truncatedOcrText) . '</a></div>';
							}
							$finalHtml .= '</div></div>';
						 
						  $finalHtmlNew .= '<div class="llm-doc col-md-12">
						   <div class="col-md-12 search-result"><ul class="listing search_data_listing" style="margin-top:0px;">'.$itemPathTitle.'</ul></div>
						    <div class="col-md-1"></div>
						   <div class="col-md-2">
                                <a class="organizations search_result_clicked" search_item_id="'.$item_id.'" href="item-details.html?q=' . encryptQueryString('folder_id=' . $item_parent . '&itemId=' . $item_id. '&search_text=' . $SearchText).'" target="_blank">
						   ...
						   <div class="col-md-9">';
						 if ($doc->structured->title !== '') {
								$finalHtmlNew .=  '  <div class="marginTopBottom10">
							...
							if ($doc->score !== '') {
								$finalHtmlNew .= '<div class="llm-doc-score">Score: ' . htmlspecialchars($doc->score) . '</div>';
							}
							if ($doc->structured->score !== '') {
                                
								$finalHtmlNew .= '<div class="llm-doc-content"><a class="organizations search_result_clicked" search_item_id="'.$item_id.'" href="item-details.html?q=' . encryptQueryString('folder_id=' . $item_parent . '&itemId=' . $item_id. '&search_text=' . $SearchText).'" target="_blank">' . htmlspecialchars($truncatedOcrText) . '</a></div>';
							}
							$finalHtmlNew .= '</div></div>';
                ...
                $finalHtml .= '</div>';
				//file_put_contents('/mnt/data/stparch/virtual_sites/www.archiveinabox.com/get_llm_stream_poll_finalHtml_'.rand(1,100).time().'.txt',print_R($finalHtml,true));
				//file_put_contents('/mnt/data/stparch/virtual_sites/www.archiveinabox.com/get_llm_stream_poll_finalHtmlNew_'.rand(1,100).time().'.txt',print_R($finalHtmlNew,true));
            }
        }

        // Store in session if DONE
        if ($status === 'DONE') {
            $queryText = trim($decoded['query_text'] ?? '');
            $normKey = normalizeChatQuery($queryText);

            if (ENABLE_LLM_CHAT_CACHE =="Y" && $normKey !== '' && $archive_id !== null) {
                $_SESSION['llm_chat_cache'][$archive_id.'_'.$normKey] = [
                    'html' => $finalHtml,
                    'raw'  => $decoded
                ];
                $_SESSION['archive_id'] = $archive_id;
          ...
          /*  try {
                ...
                $logData = [
                    ...
                    'raw_info'   => $decoded['info'] ?? '',   // contains <llm_text> & refs
                    'status'     => $status
                ];
                ...
                $logFile = __DIR__ . '/llm_done_test_log.txt';
                //file_put_contents($logFile, $logLine, FILE_APPEND);

            } catch (Throwable $e) {
                error_log("Failed to write LLM DONE log: " . $e->getMessage());
            }*/
        }
    ...
    return [
        ...
        'api_response' => $decoded ?? null
    ];
}

function parsellmRefResponse($raw) {
    ...
    if (preg_match_all('/<llm_doc>(.*?)<\/llm_doc>/s', $info, $matches)) {
        foreach ($matches[1] as $block) {
            ...
            if (preg_match('/<content>(.*?)<\/content>/s', $block, $m)) {
                ...
                // Extract @key=value pairs (original format)
                if (preg_match_all('/@(\w+)=([^\n]+)/', $content, $pairs, PREG_SET_ORDER)) {
                    foreach ($pairs as $p) {
                        $key = strtolower($p[1]);
                        $val = trim($p[2], " \t\n\r;");
                        
                        // Special handling for title - extract only the File part
                        if ($key === 'title' && strpos($val, 'File:') !== false) {
                            if (preg_match('/File:\s*([^;]+)/', $val, $title_match)) {
                                $structured[$key] = 'File: ' . trim($title_match[1]);
                            } else {
                                $structured[$key] = $val;
                            }
                        } else {
                ...
                // Extract plain key=value pairs (handles both old and new formats)
                ...
                // IMPROVED: Extract item_path separately if inline (new format)
                if ($structured['item_path'] === null) {
                    if (preg_match('/item_path=([^\s]+(?:\s+[^\s]+)*)/', $content, $ip_match)) {
                        // Stop at OCR Text or other major sections
                        $item_path_raw = $ip_match[1];
                        if (preg_match('/^([^O]*?)(?=OCR Text:)/', $item_path_raw, $ip_clean)) {
                            $structured['item_path'] = trim($ip_clean[1]);
                        } else {
                            $structured['item_path'] = trim($item_path_raw);
                        }
                    }
                }

                // NEW LOGIC: Extract URI from item_path (get last number from nXXXx pattern)
                if ($structured['item_path'] !== null) {
                    // Extract all numbers from pattern like "n877938x"
                    if (preg_match_all('/n(\d+)x/', $structured['item_path'], $path_matches)) {
                        // Get the last number as URI
                        $uri_numbers = $path_matches[1];
                        if (!empty($uri_numbers)) {
                            $structured['uri'] = end($uri_numbers); // Gets 877938
                            $structured['item_id'] = $structured['uri']; // Same as URI
                        }
                    }
                }

                // IMPROVED: Extract title more accurately 
                if ($structured['title'] === null) {
                    // Try to extract from @title= first
                    if (preg_match('/@title=([^;]+);/', $content, $title_match)) {
                        $structured['title'] = trim($title_match[1]);
                    }
                    // Fallback: extract File: line
                    else if (preg_match('/File:([^;]+);?/', $content, $file_title_match)) {
                        $structured['title'] = trim($file_title_match[1]);
                    }
                }

                // Extract File line (original logic) - but will be overridden below if title exists
                if (preg_match('/^File:(.*)/m', $content, $fm)) {
                    $structured["File"] = trim($fm[0]);
                }

                // NEW LOGIC: Set File same as title if title exists
                if ($structured['title'] !== null) {
                    $structured['File'] = $structured['title'];
                }

                // IMPROVED: Extract OCR text more precisely
                if (preg_match('/OCR Text:\s*(.*?)(?=\n\n|\Z)/s', $content, $om)) {
                    $structured["ocr_text"] = trim($om[1]);
                } else if (preg_match('/OCR Text:(.*)/s', $content, $om)) {
                    $structured["ocr_text"] = trim($om[1]);
                }
...
function truncateToWords($text, $wordLimit = 100) {
    ...
    // Take first 100 words and add ellipsis
...
function normalizeChatQuery($q) {
	/*
    $q = strtolower(trim($q));                 // lowercase and trim
    $q = preg_replace('/\s+/', ' ', $q);       // collapse whitespace
    $q = preg_replace('/[^\w\s]/', '', $q);    // remove punctuation
	*/
	// base64_encode
    return base64_encode($q);
}

function llmStreamDemo($postData)
{
    $archive_id = $postData['archive_id'] ?? null;
    $postDataItem = [
        "_key"           => APIKEY,
   ...
   if($postData['demo'] == 1)
   {
      $rawResult=trim(file_get_contents('/mnt/data/stparch/virtual_sites/www.archiveinabox.com/get_llm_stream_poll_rawResult_141755708753.txt'));
   }
    // $rawResult=trim(file_get_contents('/mnt/data/stparch/virtual_sites/www.archiveinabox.com/get_llm_stream_poll_rawResult_141755708753.txt'));
//file_put_contents('/mnt/data/stparch/virtual_sites/www.archiveinabox.com/get_llm_stream_poll_rawResult_'.rand(1,100).time().'.txt',$rawResult);
    $decoded = json_decode($rawResult, true);
    $finalHtml = '';
    $SearchText = base64_decode($postData['chat_request']);
    $SearchText = preg_replace("/[\"]/", " ", $SearchText);
    ...
    if (!empty($decoded) && isset($decoded['status'])) {
        $status = $decoded['status'];
        if($postData['demo'] == 1){$status = 'DONE';}
        if (!empty($decoded['info']) || (!empty($decoded['api_response']['info']))) {
            $infoRaw = $decoded['api_response']['info'] ?? $decoded['info'];
            ...
            if (preg_match('/<llm_text>([\s\S]*?)(?:<\/llm_text>|$)/', $infoRaw, $match)) {
                $partialText = trim($match[1]);
                if ($partialText !== '') {
                    $finalHtml .= '<div class="llm-main-text">' . nl2br(htmlspecialchars($partialText)) . '</div>';
                }
            }

            // Parse docs if present (will be partial or full)
            if (preg_match_all('/<llm_doc>([\s\S]*?)<\/llm_doc>/', $infoRaw, $docMatches)) {
                $finalHtml .= '<div class="llm-doc-section"><h4>References</h4>';
				$result=parsellmRefResponse($rawResult);
				$rawResultParse=json_decode($result);
				//file_put_contents('/mnt/data/stparch/virtual_sites/www.archiveinabox.com/get_llm_stream_poll_rawResult_'.rand(1,100).time().'.txt',print_R($rawResult,true));
				//file_put_contents('/mnt/data/stparch/virtual_sites/www.archiveinabox.com/get_llm_stream_poll_rawResultParse_'.rand(1,100).time().'.txt',print_R($rawResultParse,true));
					 ...
					 if($doc->structured->ocr_text){
						 ...
						 if ($doc->structured->title !== '') {
							...
							if ($doc->structured->score !== '') {
								...
								$finalHtml .= '<div class="llm-doc-content"><a class="organizations search_result_clicked" search_item_id="'.$item_id.'" href="item-details.html?q=' . encryptQueryString('folder_id=' . $item_parent . '&itemId=' . $item_id. '&search_text=' . $SearchText).'">' . $doc->structured->ocr_text . '</a></div>';
							}
							$finalHtml .= '</div></div>';
						 ...
						 if ($doc->structured->title !== '') {
							...
							if ($doc->structured->score !== '') {
								...
								$finalHtmlNew .= '<div class="llm-doc-content"><a class="organizations search_result_clicked" search_item_id="'.$item_id.'" href="item-details.html?q=' . encryptQueryString('folder_id=' . $item_parent . '&itemId=' . $item_id. '&search_text=' . $SearchText).'" target="_blank">' . $doc->structured->ocr_text . '</a></div>';
							}
							$finalHtmlNew .= '</div></div>';
...
function aibSearchRequest_old($postData) {
    // Create a new curl resource
    $curlObj = curl_init();
    $options = array(
        CURLOPT_POST => 1,
...
function apiFilterPaginationData($apiResponse, $start, $ITEM_COUNT_PER_PAGE) {
	...
	foreach($apiResponse['info']['records'] as $recKey=>$recVal){
		$apiResponse['info']['records'][$recKey]['item_tiele_first_word']=strtok($recVal['item_title'],' ');
	}
	/*<!----------- Issue Id 2379 Fix Start date 19-apr-2024--------------->*/
	if(isset($_SESSION['sort_by']) and $_SESSION['sort_by']!=''){
				$sort_by_order=$_SESSION['sort_by'];
		//echo strtok($my_string, "1935 test brijesh"); 
		if($_SESSION['sort_by']=='a-z'){
	...
	/*<!----------- Issue Id 2379 Fix End date 19-apr-2024--------------->*/
...
function apiFilterArchivePaginationData($apiResponse, $start, $ITEM_COUNT_PER_PAGE) {
	...
	//echo '<pre>';print_R($apiResponse['info']['records']);die;
...
/*	echo '<pre>';
	print_R( $returnArray['info']['records']);echo '</pre>';*/
    return $returnArray;
}
...
function apiFilterRecordPaginationData($apiResponse, $start, $ITEM_COUNT_PER_PAGE) {
	...
	/*<!----------- Issue Id 2379 Fix Start date 19-apr-2024--------------->*/
	if(isset($_SESSION['shortByASCII']) and $_SESSION['shortByASCII']=='ASCII'){
	  array_multisort( array_column($apiResponse['records'], "item_title"), SORT_ASC,SORT_NATURAL, $apiResponse['records']);
	}
	/*<!----------- Issue Id 2379 Fix End date 19-apr-2024--------------->*/
    for ($c = $start; $c < ($start + $ITEM_COUNT_PER_PAGE); $c++) {
...
function getImageHighlightWithDimenstion($item_id,$imageHeight,$imageWidth,$searchString)
{
	...
	if ($item_id != '' && $searchString != '') {
       ...
       $apiResponse = aibServiceRequest($postData, 'generate_highlight_overlay');
        ...
        if ($apiResponse['status'] == 'OK') {
            $data= json_decode($apiResponse['info'], true);
			$newData=array();
			foreach($data['rect'] as $d)
			{
				unset($d['p']);
				$newData[]=$d;
...
//Fix start for Issue ID 2147 on 28-Apr-2023
function updateRecordViewCount($folderId,$itemData,$sessionKey,$item_in_folderid=null){
	
	if (!isset($_SESSION['view_record_history']) || !is_array($_SESSION['view_record_history'])) {
        $_SESSION['view_record_history'] = [];
    }
	
	if(!in_array($folderId,$_SESSION['view_record_history'])){
		
	$apiRequestDataGetPath = array(
									'_key' => APIKEY,
									'_user' => 1,
									'_op' => 'get_path',
									'_session' => $sessionKey, 
	...
	$apiResponseGetPath = aibServiceRequest($apiRequestDataGetPath, 'browse');	
	...
	/*echo '<pre>';
	print_R($itemData);
	print_R($apiResponseTrendingData);echo '</pre>';die;*/
	
	$view_count_record=1;
	$last_viewed_date=date('Y-m-d H:m:i');
	if(isset($itemData['properties']['view_count_record']) and $itemData['properties']['view_count_record']>0){
		$view_count_record=$itemData['properties']['view_count_record']+1;
	}
	 ...
	 $apiRequestDataItemStatus = array(
                            ...
                            '_session' => $sessionKey,
                            'obj_id' => $folderId,
                            'propname_1' => 'view_count_record',
                            'propval_1' => $view_count_record,
		  					'propname_2' => 'last_viewed_date',
                            'propval_2' =>$last_viewed_date,
		 					'propname_3' => 's_item_id',
							'propval_3' => $apiResponseGetPath['info']['records'][1]['item_id'],
							'propname_4' => 's_item_title',
							'propval_4' => $apiResponseGetPath['info']['records'][1]['item_title'],
                        );
                        $apiResponseStatus = aibServiceRequest($apiRequestDataItemStatus, 'browse');
	

		$found=0;
		$view_count_record_min=$view_count_record;
		$min_view_count_item_id=0;
		
		foreach($apiResponseTrendingData['info']['records'] as $trendingData){
			if($trendingData['properties']['record_id']==$folderId){
				
				
				$found=1;
				if($trendingData['properties']['view_count_record']<101){
					 $apiRequestDataItemStatus = array(
								'_key' => APIKEY,
								'_user' => 1,
								'_op' => 'set_item_prop',
								...
								'propname_1' => 'view_count_record',
								'propval_1' => ($trendingData['properties']['view_count_record']+1),
								'propname_2' => 'last_viewed_date',
								'propval_2' =>$last_viewed_date,
						 		'propname_3' => 's_item_id',
							'propval_3' => $apiResponseGetPath['info']['records'][1]['item_id'],
					...
					$apiRequestDataItemStatus = array(
								...
								'obj_id' => $trendingData['item_id'],
								'propname_1' => 'view_count_record',
								'propval_1' => ($trendingData['properties']['view_count_record']),
								'propname_2' => 'last_viewed_date',
								'propval_2' =>$last_viewed_date,
								'propname_3' => 's_item_id',
							'propval_3' => $apiResponseGetPath['info']['records'][1]['item_id'],
		...
		if($found==0 ){
            ...
            '_session' => $sessionKey,
            'parent' => TRENDING_ARTIFACTS_ID,
            'item_title' => 'trending_artifacts_'.count($apiResponseTrendingData['info']['records']),
			...
			if(isset($apiResponse['info']))
			{
				$apiRequestDataItemStatus = array(
                            ...
                            'propval_1' => 1,
		  					'propname_2' => 'last_viewed_date',
                            'propval_2' =>$last_viewed_date,
							'propname_3' => 'record_id',
                            'propval_3' =>$folderId,
							'propname_4' => 's_item_id',
							'propval_4' => $apiResponseGetPath['info']['records'][1]['item_id'],
							'propname_5' => 's_item_title',
							'propval_5' => $apiResponseGetPath['info']['records'][1]['item_title'],
	 ...
	 $_SESSION['view_record_history'][]=$folderId;
 }		
...
//Fix start for Issue ID 2147 on 28-Apr-2023
function updateItemViewCount($itemId,$sessionKey){
	...
	$apiResponseGetPathItem = aibServiceRequest($apiRequestDataGetPath, 'browse');	
	
	
	$folderId=$apiResponseGetPathItem['info']['records'][count($apiResponseGetPathItem['info']['records'])-2]['item_id'];
	 
	$itemData = getItemData($folderId);
	if(!in_array($itemId,$_SESSION['view_record_history'])){
	...
	//echo '<pre>';
	//print_R($itemData);
	//print_R($apiResponseGetPath);echo '</pre>';die;
	...
	if(isset($itemData['properties']['view_count_record']) and $itemData['properties']['view_count_record']>0){
	 ...
	 $apiRequestDataItemStatus = array(
		...
		foreach($apiResponseTrendingData['info']['records'] as $trendingData){
			if($trendingData['properties']['record_id']==$folderId and $trendingData['properties']['viewed_item_id']==$itemId  ){
	 ...
	 $_SESSION['view_record_history'][]=$itemId;
 }		
	
}
//Fix End for Issue ID 2147 on 28-Apr-2023
function getItemListData( $parent_id,$sessionKey){
	$postDataItem = array(
            "_key" => APIKEY,
            "_session" => $sessionKey,
            "_user" => 1,
            "_op" => "list",
		 "parent" => $parent_id,
		//"opt_prop_count_set"=>json_encode(array( "name" => "stphideftype","value" => "rec"))
        );
