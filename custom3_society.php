        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (result.status == 'success') {
                    ...
                    showPopupMessage('error', result.message + ' (Error Code: 1165)');
                }
            },
            error: function () {
                showPopupMessage('error', 'Previous request not completed. (Error Code: 123)');
            }
        });
    }
});
...
function ItemPageNumberSet(page,pagefolder){  
		var folder_id = pagefolder.split(",");  
		  $.ajax({
			...
			error: function () {
				showPopupMessage('error', 'Something went wrong, Please try again. (Error Code: 118)');
			}
		});   
	}
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (result.status == 'success') {
                        ...
                        showPopupMessage('error', result.message + ' (Error Code: 1166)');
                    }
                },
                error: function () {
...
//            });
            $.ajax({
                ...
                error: function () {
                    showPopupMessage('error','Something went wrong, Please try again. (Error Code: 119)');
                }
	}); 
}
...
//        var thumb_url = '<?php echo THUMB_URL; ?>';
 ...
 function getRecordTreedata(folder_id,itemId,share,flg,previousId,ref,detailPagecontent,pageNumber='',pagefolder='',start='',total_item_page='',action='',imgFlg='',searchString = '',recordIdsArr = '',scrapbook_item = ''){
    if(recordIdsArr !='' && typeof(recordIdsArr) != 'undefined'){
		...
		ItemPageNumberSet(pageNumber,pagefolder);
            ...
            error: function () {
                $('.loading-div').hide();
                showPopupMessage('error','Something went wrong, Please try again. (Error Code: 120)');
            }
        });
}
 ...
 function getRecordIds(id){
     $.ajax({
            ...
            error: function () {
                showPopupMessage('error','Something went wrong, Please try again. (Error Code: 121)');
            }
	}); 
 }  
 function imageLoad(){
...
//    if(pr_file_id !=''){
        ...
        error: function () {
            showPopupMessage('error','Something went wrong, Please try again. (Error Code: 122)');
        }
   });
//    }
