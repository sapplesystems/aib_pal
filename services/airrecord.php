			...
			foreach($ItemFields as $ItemFieldRecord)
			{
				...
				switch($FieldRecord["field_data_type"])
				{
					...
					case FTREE_FIELD_TYPE_DROPDOWN:
						...
						/********* fix start Bug id 2387 13-May-2204 ****************/
						//$FolderType = ftree_get_property($GLOBALS["aib_db"],$FieldID,"AIB_FOLDER_PROPERTY_FOLDER_TYPE");
						aib_open_db();
						$ddValues = ftree_get_property($GLOBALS["aib_db"],CUSTOM_FIELD_DD,$FieldID); 
						
						aib_close_db();
						$OptionList = explode("\n",urldecode($ddValues));
						foreach($OptionList as $Value => $Desc)
						{
							$OutLines[] = "<option value=\"$Desc\">$Desc</option>";
						}
						/********* fix end Bug id 2387 13-May-2204 ****************/
