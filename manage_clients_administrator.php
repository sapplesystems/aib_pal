...
<div class="content-wrapper">
    <section class="content-header">
        ...
        <h4 class="list_title text-center"> <span class="pull-left">Manage Client Administrator</span> <span class="headingNameDesign"><?php echo $_SESSION['aib']['user_data']['item_title'];?></span></h4>
    </section>
...
<script type="text/javascript">
    ...
    $(document).on('click', '#update_administrator_form', function (){
        if ($("#administrator_form").valid()) {
            ...
            $.ajax({
                ...
                error: function () {
                    showPopupMessage('error','Something went wrong, Please try again. (Error Code: 199)');
                }
            });
        }
    });
    ...
    function get_administrator_data(){
        ...
        $.ajax({
            url: "services_admin_api.php",
            type: "post",
            data: {mode: 'assistant_list',type:'A'},
            success: function (response) {
                var record = JSON.parse(response);
                for (i = 0; i < record.length; i++) {
                    table.row.add([
                        ...
                        record[i].user_title,
                        record[i].user_login,
			'<span class="edit_administrator" data-title="Edit" data-form-user-id='+ record[i].user_id +'><img src="'+image_path+'edit_icon.png" alt="" /></span><span class="delete_administrator" data-form-user-id="'+ record[i].user_id +'"><img src="'+image_path+'delete_icon.png" alt="" /></span><span class="login_as_administrator" data-title="Login as administrator" data-form-user-id="'+ record[i].user_id +'"><img title="Login as administrator" src="'+image_path+'login-as.png" alt="" /></span>'
                    ]).draw(false);
                    select: true
                }
                $('.admin-loading-image').hide();
		$("#dataTableDiv").show();
            ...
            error: function () {
                showPopupMessage('error','Something went wrong, Please try again. (Error Code: 201)');
            }
        });
    }
    ...
    $(document).on('click','.delete_administrator',function(){
        if(confirm("Are you sure to delete the assistant? This cannot be undone")){
            ...
            $.ajax({
                ...
                error: function () {
                    showPopupMessage('error','Something went wrong, Please try again. (Error Code: 202)');
                    //$('.admin-loading-image').hide();
                }
            });
        }
