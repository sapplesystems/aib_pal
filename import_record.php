...
if(isset($_REQUEST['folder_id']) && $_REQUEST['folder_id']!= ''){
    ...
    $folder_id = AIB_DEFAULT_FOLDER_ID;
}
$previous = '';
...
<div class="content-wrapper">
        <section class="content-header">
            ...
            <ol class="breadcrumb">
              ...
              <li class="active">Import Records -- Upload ZIP Archive</li>
            </ol>
           <h4 class="list_title">Import Records -- Upload ZIP Archive </h4>
        </section>
...
//		document.getElementById('itemrecord_useorgname').disabled = true;
		...
		$('#aib_submit_button').prop('disabled',true);
		$('#aib_submit_button').hide();
	});
	...
	var UploadDataSet = {};
	var FileUploadsCompleted = 0;
	var TotalFileQueueCount = 0;
	...
	function trigger_upload()
	{
		...
		TotalFileQueueCount = TotalFileQueueCount + QueuedCount;
		if (TotalFileQueueCount > 0)
		{
			$('#fileupload').hide();
			$('#fileupload').prop('disabled',true);
		}

		if (FileUploadsCompleted < 1)
		{
		...
		{
			$('#aib_submit_button').prop('disabled',false);
			$('#aib_submit_button').show();
		}
	}
...
//		LocalData.submit();
	...
	progress: function(LocalEvent,LocalData) {
		...
		if (Progress >= 100)
		{
			...
			$('#aib_submit_button').prop('disabled',false);
			$('#aib_submit_button').show();
			$('#input-box-itemrecord_upload').hide();
			$('#title-box-itemrecord_upload').hide();
...
<?php
	...
	$FieldDef = array(
		...
		"itemrecord_visible" => array("title" => "Visible To Public?:", "type" => "dropdown", "display_width" => "20",
			...
			"desc" => "If 'Yes', then imported records can be seen by public users.", "help_function_name" => "itemrecord_visible_help_popup"),
		...
		"itemrecord_upload_field" => array(
			"title" => "Select ZIP Archive:", "type" => "custom", "display_width" => "64",
		...
		"itemrecord_default_tags" => array(
			...
			"desc" => "Tags for all imported files.  Enter words or phrases separated by commas", "help_function_name" => false),
		...
		"itemrecord_url_list" => array(
			...
			"desc" => ""),
		"itemrecord_ocr" => array("title" => "OCR?:", "type" => "dropdown", "display_width" => "20",
			"field_name" => "itemrecord_ocr", "field_id" => "itemrecord_ocr",
			"desc" => "If 'Yes', then imported records will be processed with OCR.", "help_function_name" => "itemrecord_visible_help_popup"),
	...
	$FieldDef["itemrecord_ocr"]["option_list"] = array("N" => "No", "Y" => "Yes");
	$FieldDef["itemrecord_ocr"]["value"] = "N";
...
*/
					...
					$OutBuffer[] = "<div class='aib-parent-folder-title'><span class='aib-parent-folder-title-span'>You Are Importing Records Into: <b>$ParentTitle</b></span></div>";
					...
					if ($ParentFolderType == AIB_ITEM_TYPE_ITEM || $ParentFolderType == AIB_ITEM_TYPE_RECORD)
					{
						...
						$OutBuffer[] = "<input type='hidden' name='opcode' value='map'>";
					}
					else
					{
			...
			switch($OpCode)
			{
				...
				case false:
					...
					<div class='upload-container'>
						<input id=\"fileupload\" type=\"file\" name=\"files[]\" accept='application/zip' data-url=\"../server/php/\" >  <button type='button' onclick='trigger_upload()' id='aib_start_upload_button'>Start ZIP Archive Upload</button>
					</div>
					";
...
//					$OutBuffer[] = aib_draw_input_row_separator();
					...
					$OutBuffer[] = aib_draw_input_row_separator();
					$OutBuffer[] = aib_draw_dropdown_field($FieldDef["itemrecord_ocr"]);
					$OutBuffer[] = aib_draw_input_row_separator();
...
*/
					$OutBuffer[] = aib_draw_input_row_separator();
					$OutBuffer[] = aib_draw_input_row_separator();
					$OutBuffer[] = aib_draw_form_submit("CLICK HERE TO CONTINUE IMPORT","");
					$OutBuffer[] = "</table>";
					$OutBuffer[] = "</form>";
					break;
			}
...
<?php
	...
	$ValidationDef = array("_form" => array("function" => "post_process_form"));
